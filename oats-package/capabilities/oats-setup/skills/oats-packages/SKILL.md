...
name: oats-packages
description: >-
  Use for OATS package acquisition, exact restore/update/remove, package identity
  versus capability IDs, catalog discovery, executable trust and v1/v2 migration;
  also when reviewing a portable preparation artifact set. Keep immutable retained
  approval separate from classic mutable installed-store operations.
---

# OATS packages and exact approvals

Moved from the existing package skill; command baseline **0.24.0**.

## Source-complete preparation and retained approval

Retention, selection and approval are distinct. A captured resolution selects
immutable source, capability and resource trees. Current provider/native readiness
is checked again per action; availability or officialness is never that proof.

For a new source/workspace selection use **oats-workspace-setup**. Only after the
operator reviews and authorizes the exact executable artifacts returned by
preparation, use the existing prospective-artifact approval route:

```bash
oats prepare --request <absolute-preparation-json> --json
oats trust <capability-id> --deployment <absolute-deployment> \
  --artifact-set <sha256-id> --json
oats prepare --request <absolute-preparation-json> --json
oats inspect --deployment <absolute-deployment> --resolution <sha256-id> --json
```

For an already complete retained record, trust accepts that exact
`--deployment` / `--resolution` pair instead. Do not derive approval from a
current catalog, mutable lock or package directory. Resource-only capabilities
have no executable approval surface, but still need exact source/integrity and
explicit selection. Never approve automatically from this procedure.

The reviewed `package-catalog.json` in the oats repository defines the official
marketplace; it is not another registry. Discovery is not installation, and
installation is not activation or executable trust. Newly announced capabilities
need an actual published source/catalog revision before an install-by-ID promise.
Package identity and capability IDs are distinct: updating/locking a distribution
is not renaming every capability it exports.

## Classic installed-store compatibility — 0.24
...
> captured resolution. Use the exact retained approval procedure above; never restore or advance captured code through today's
...
framework repo; `docs/capabilities.md` for the user-level walkthrough. These are
optional framework-release references, not paths under this installed skill.
Use the installed CLI's help when no matching source checkout is available.
