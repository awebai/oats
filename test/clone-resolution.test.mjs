...
test("R2: ensureRoot with an oats-local.yaml above but no agents/ → E_NO_DEPLOYMENT whose remedy is `mkdir <deployment>/agents (or run oats sync)`, never a v1 verb", () => {
  ...
  try {
    ...
    try {
      ...
      const r = spawnSync(process.execPath, [CLI, "spawn", "platform-engineer", "--dir", dep, "--no-launch", "--json"], { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"], env: { ...process.env, PATH: inertHarnessPath(base), PI_AGENTS_ROOT: "", PI_AGENT_HOME: "", OATS_HOME: "", HOME: join(base, "home") } });
