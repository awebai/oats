...
/**
 * A workspace-model deployment (test/helpers/v2-deployment.mjs) whose one member
 * carries the REAL capabilities/oats-okf (capability oats.okf) bound to the
 * knowledge slot by the workspace defaults, and a `source` soul. Host settings
 * (bindings-file, harvest-*) are the deployment's oats-local.yaml `settings:`.
...
export function fixture(t, { git = false, settings = {}, register = true } = {}) {
  ...
  // The knowledge slot: oats.okf from this member, for every soul (workspace default).
  const wsFile = join(fx.member, 'oats-workspace.yaml');
  const wsDoc = YAML.parse(fs.readFileSync(wsFile, 'utf8'));
  wsDoc.defaults = { ...(wsDoc.defaults || {}), knowledge: { 'oats.okf': { from: fx.key } } };
  fx.commit({ 'oats-workspace.yaml': { yaml: wsDoc } }, 'bind the knowledge slot to oats.okf');
