...
// The in-process planner resolves a harness's default binary on THIS process's PATH (`which`), not on the
...
test("configuration args and environment render literally, references by reference; the parser and the description read them back", () => {
  ...
  const shadow = { harness: "claude", executable: "/usr/bin/printenv", args: [], env: { A: "override", B: { fromEnv: "A" } }, model: null, hooks: hooks({}, { A: "hook" }) };
  ...
  const pi = renderLaunchRecipe({ harness: "pi", executable: "/p", args: ["--x", "1"], env: {}, model: null, hooks: hooks({ pi: "--hook" }, {}) }, { home, instance: "n" });
...
test("selection rules: a named configuration is a unit; models never cross harnesses; frozen recipes keep or drop their configuration as asked", () => {
  const launchConfigs = Object.assign(Object.create(null), {
    fast: { name: "fast", harness: "codex", executable: "/c", args: ["--a"], env: {}, model: "gpt-5.5", yolo: true, source: "/s" },
    cl: { name: "cl", harness: "claude", args: [], env: {}, source: "/s" },
  });
  const agent = { harness: "pi", model: "anthropic/claude-opus-5:high", yolo: false, "launch-config": "cl" };
  let s = resolveLaunchSelection({ launchConfigs, agent, selection: {} });
  assert.deepEqual([s.config.name, s.harness, s.model, s.modelSource], ["cl", "claude", "", "native default (harness differs from the soul's)"], "the soul's default configuration runs another harness: its pi model preference is not carried");
  s = resolveLaunchSelection({ launchConfigs, agent: { ...agent, harness: "claude" }, selection: {} });
  assert.deepEqual([s.config.name, s.harness, s.model, s.modelSource], ["cl", "claude", "claude-opus-5", "soul default"], "same harness as the soul: the soul model, translated");
  s = resolveLaunchSelection({ launchConfigs, agent, selection: { launchConfig: "none" } });
  assert.deepEqual([s.config, s.harness, s.model], [null, "pi", "anthropic/claude-opus-5:high"]);
  s = resolveLaunchSelection({ launchConfigs, agent, selection: { launchConfig: "fast" } });
  assert.deepEqual([s.config.name, s.harness, s.model, s.modelSource, s.configuredYolo], ["fast", "codex", "gpt-5.5", "launch-config fast", true]);
  s = resolveLaunchSelection({ launchConfigs, agent: { harness: "claude", model: "sonnet" }, selection: { launchConfig: "cl2" === "cl2" ? "fast" : "" } });
  assert.deepEqual([s.harness, s.model, s.modelSource], ["codex", "gpt-5.5", "launch-config fast"]);
  s = resolveLaunchSelection({ launchConfigs: Object.assign(Object.create(null), { p: { name: "p", harness: "codex", args: [], env: {}, source: "/s" } }), agent: { harness: "claude", model: "sonnet" }, selection: { launchConfig: "p" } });
  assert.deepEqual([s.harness, s.model, s.modelSource], ["codex", "", "native default (harness differs from the soul's)"], "a soul's bare alias is not passed to another harness");
  s = resolveLaunchSelection({ launchConfigs, agent: { harness: "claude", model: "sonnet" }, selection: { harness: "codex" } });
  assert.deepEqual([s.harness, s.model], ["codex", ""]);
  assert.throws(() => resolveLaunchSelection({ launchConfigs, agent, selection: { launchConfig: "fast", harness: "claude" } }), (e) => e.code === "E_LAUNCH_CONFIG_MISMATCH");
  s = resolveLaunchSelection({ launchConfigs, agent, selection: { launchConfig: "fast", harness: "codex", model: "openai/gpt-5.5-mini" } });
  assert.deepEqual([s.harness, s.model, s.modelSource], ["codex", "gpt-5.5-mini", "explicit"], "same harness override is fine; an explicit model wins over the configured one");
  ...
  // changed or vanished; only an explicit name applies the current one; --harness alone leaves it all behind.
  const frozen = { harness: "codex", launchConfig: "fast", launchConfigSource: "/s", model: "gpt-5.5", executable: "/old/wrapper", executableDeclared: "./old/wrapper", args: ["--old"], env: { K: { fromEnv: "S" } } };
  s = resolveLaunchSelection({ launchConfigs: Object.create(null), agent, frozen, selection: {} });
  assert.deepEqual([s.config.name, s.config.frozen, s.config.executablePath, s.config.args, s.config.env, s.harness, s.model, s.modelSource], ["fast", true, "/old/wrapper", ["--old"], { K: { fromEnv: "S" } }, "codex", "gpt-5.5", "recorded"], "the recorded configuration, with a scope that no longer declares it");
  ...
  s = resolveLaunchSelection({ launchConfigs, agent, frozen, selection: { harness: "claude" } });
  assert.deepEqual([s.config, s.harness, s.model, s.modelSource], [null, "claude", "", "native default (harness changed)"], "no executable, args or model carried across the harness change");
  s = resolveLaunchSelection({ launchConfigs, agent, frozen: { harness: "claude", launchConfig: null, model: "claude-sonnet-5" }, selection: { yolo: true } });
  assert.deepEqual([s.harness, s.model, s.modelSource], ["claude", "claude-sonnet-5", "recorded"], "same harness keeps the recorded model");
  assert.deepEqual(resolveLaunchSelection({ launchConfigs, agent, frozen: { harness: "codex", launchConfig: "gone", executable: "/g" }, selection: {} }).config.name, "gone", "a recorded configuration the scope forgot still starts as recorded");
  for (const [name, entry, why] of [["none", { harness: "pi" }, /cannot be named none/], ["p", { harness: "codex", env: { OATS_INSTANCE_HOME: "/wrong" } }, /set by the kernel/], ["p", { harness: "codex", env: { PI_AGENTS_ROOT: "/x" } }, /set by the kernel/], ["p", { harness: "codex", env: { OATS_LAUNCH_REF_X: "1" } }, /set by the kernel/], ["p", { harness: "pi", env: { A: { fromEnv: "OATS_LAUNCH_REF_A" } } }, /alias/]]) {
...
test("spawn records the recipe: a configuration's executable, args and references land in instance.json and the command; missing references, unknown or mismatched configurations refuse before a home exists", async (t) => {
  ...
  const spawnDev = async (purpose, { launchConfig, harness, extra = {} } = {}) => {
    const saved = { ...process.env }; Object.assign(process.env, { HOME: fx.env.HOME, OATS_REMOTE_CACHE: fx.env.OATS_REMOTE_CACHE }, extra);
    try { const json = { ok: true, result: await fx.spawn("dev", { purpose, ...(launchConfig ? { launchConfig } : {}), ...(harness ? { harness } : {}) }) }; return { json, stdout: JSON.stringify(json) }; }
  ...
  write(join(base, "personal.json"), JSON.stringify({ harness: "claude", executable: "./tools/claude-wrapper.sh", args: ["--settings", "/abs/settings.json", "a b"], env: { LIT: "plain", KEY: { fromEnv: "LAUNCH_TEST_SRC" } }, model: "claude-opus-5", yolo: true }));
  ...
  r = await spawnDev("p1", { launchConfig: "personal", harness: "codex", extra: { LAUNCH_TEST_SRC: "s3cret" } });
  ...
  assert.equal(meta.harness, "claude"); assert.equal(meta.model, "claude-opus-5"); assert.equal(meta.yolo, true);
  assert.deepEqual([meta.launch.version, meta.launch.launchConfig, meta.launch.launchConfigSource, meta.launch.executable, meta.launch.executableDeclared, meta.launch.args, meta.launch.env, meta.launch.model, meta.launch.yolo, meta.launch.prompt], [2, "personal", repo, wrapper, "./tools/claude-wrapper.sh", ["--settings", "/abs/settings.json", "a b"], { KEY: { fromEnv: "LAUNCH_TEST_SRC" }, LIT: "plain" }, "claude-opus-5", true, { kind: "task-file", file: "TASK.md" }]);
  ...
  // `status --json` is not enveloped: the roster is the top-level document.
  ...
  // Without a configuration: the default harness (a v2 soul declares none) with a spawn --model, no configuration fields, same command shape as before recipes.
  r = await spawnDev("p2", { extra: {} });
  assert.equal(r.json.ok, true, r.stdout);
  const plain = JSON.parse(readFileSync(join(instancesDir, "dev-p2", "instance.json"), "utf8"));
  assert.deepEqual([plain.launch.harness, plain.launch.launchConfig, plain.launch.args, plain.launch.env], ["pi", null, [], {}]);
  assert.ok(plain.command.endsWith(`--approve --name 'dev-p2' '@TASK.md'`), plain.command);
  // Preview of the existing home: frozen as recorded; a switch to codex renders codex defaults with no wrapper or args; a disagreeing --harness is refused.
  const h1 = join(instancesDir, "dev-p1");
  r = oats(["launch-config", "preview", "--home", h1], { extra: { LAUNCH_TEST_SRC: "s3cret" } });
  assert.equal(r.json.ok, true, r.stdout);
  let v = r.json.result;
  assert.deepEqual([v.selection.source, v.harness, v.launchConfig, v.executable.path, v.model, v.yolo, v.ok], ["frozen", "claude", "personal", wrapper, "claude-opus-5", true, true]);
  ...
  write(join(base, "personal2.json"), JSON.stringify({ harness: "claude", args: ["--changed"], model: "claude-sonnet-5" }));
  ...
  write(join(base, "clash.json"), JSON.stringify({ harness: "claude", env: { AWEB_DELIVERY: "channel" } }));
  ...
  r = oats(["launch-config", "preview", "--home", h1, "--harness", "codex"]);
  v = r.json.result;
  assert.deepEqual([v.selection.source, v.harness, v.launchConfig, v.executable.path, v.model, v.modelSource, v.argv.includes("a b")], ["config", "codex", null, join(binDir, "codex"), null, "native default (harness changed)", false]);
  assert.equal(v.preflight.find((c) => c.check === "environment").ok, true, "no references left once the configuration is dropped");
  r = oats(["launch-config", "preview", "--home", h1, "--launch-config", "personal", "--harness", "codex"]);
  ...
  v = r.json.result;
  assert.deepEqual([v.selection.source, v.harness, v.launchConfig, v.executable.path, v.ok, v.hooks.pending], ["config", "claude", "personal", wrapper, true, true], JSON.stringify(r.json).slice(0, 400));
  r = oats(["launch-config", "preview", "--soul", "dev", "--dir", repo, "--launch-config", "none"]);
  assert.deepEqual([r.json.result.harness, r.json.result.launchConfig, r.json.result.model], ["pi", null, null]);
  // A legacy home (no recipe): described as is; a selection is refused (E_LAUNCH_LEGACY): re-spawn it.
  const legacy = join(instancesDir, "dev-legacy"); mkdirSync(legacy, { recursive: true });
  write(join(legacy, "instance.json"), JSON.stringify({ agent: "dev", instance: "dev-legacy", home: legacy, repo, harness: "claude", model: "claude-x", launched: false, command: `OATS_INSTANCE='dev-legacy' OATS_INSTANCE_HOME=${shq(legacy)} AWEB_DELIVERY='session' '/opt/homebrew/bin/claude' --model 'claude-x' -- "$(cat TASK.md)"` }));
  ...
  r = oats(["launch-config", "preview", "--home", legacy, "--harness", "codex"]);
  assert.equal(r.json.error?.code, "E_LAUNCH_LEGACY", r.stdout);
  assert.match(r.json.error.message, /dev-legacy records no launch recipe.*re-spawn it from the deployment; nothing was changed/);
  assert.equal(readFileSync(join(legacy, "instance.json"), "utf8"), legacyBytes, "the refusal writes nothing");
});
...
test("an UNNAMED frozen recipe starts exactly as recorded: its wrapper, args and references, whatever the scope says now; an unknown recipe shape is refused before anything", () => {
  ...
  const meta = { instance: "u", agent: "dev", home, harness: "claude", launch: { version: 2, harness: "claude", launchConfig: null, launchConfigSource: null, executable: "/usr/bin/true", executableDeclared: null, executableResolvedFrom: "oats-claude-config", args: ["--saved"], env: { KEY: { fromEnv: "MISSING_REVIEW_ENV" } }, hooks: { launch: {}, env: {}, contributions: [] }, model: null, yolo: false, prompt: { kind: "task-file", file: "TASK.md" } } };
  const plan = planLaunch({ home, meta, contextDir: base, agentLike: { harness: "claude" }, selection: {}, launchConfigs: Object.create(null), env: {}, preview: true });
  ...
  const yoloOnly = planLaunch({ home, meta, contextDir: base, agentLike: { harness: "claude" }, selection: { yolo: true }, launchConfigs: Object.create(null), env: { MISSING_REVIEW_ENV: "v" }, preview: true });
  ...
  const switched = planLaunch({ home, meta, contextDir: base, agentLike: { harness: "claude" }, selection: { harness: "codex" }, launchConfigs: Object.create(null), env: {}, preview: true });
