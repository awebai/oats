...
test("the YAML subset reads block sequences and quoted scalars with their escapes; a comment never cuts a quoted value", () => {
  ...
  assert.deepEqual(cfg["launch-configs"].fast, { harness: "claude", executable: "/opt/x/claude wrapper", args: ["--flag", HOSTILE], env: { A: "lit # x", B: { fromEnv: "SRC" } }, model: "it's", yolo: true });
  assert.deepEqual(cfg["launch-configs"].plain, { harness: "pi" });
...
test("validation names the fault; the deployment's oats-local.yaml is the one source, with no shadowing", () => {
  const ok = { harness: "pi", args: ["x"], env: { A: "1", B: { fromEnv: "S" } } };
  assert.equal(validateLaunchConfig("n1", ok), ok);
  for (const [name, entry, why] of [
    ["bad name!", { harness: "pi" }, /invalid name/],
    ["n", { harness: "bash" }, /needs harness/],
    ["n", { harness: "pi", extra: 1 }, /unsupported key "extra"/],
    ["n", { harness: "pi", args: "x" }, /args must be a list/],
    ["n", { harness: "pi", env: { "1A": "x" } }, /not a valid environment variable name/],
    ["n", { harness: "pi", env: { A: { fromEnv: "bad-name" } } }, /string or \{fromEnv: NAME\}/],
    ["n", { harness: "pi", env: { A: { fromEnv: "S", extra: 1 } } }, /string or \{fromEnv: NAME\}/],
    ["n", { harness: "pi", yolo: "yes" }, /yolo must be true or false/],
    ["n", { harness: "pi", executable: "" }, /executable must be non-empty/],
  ...
  assert.deepEqual({ ...found.fast }, { name: "fast", harness: "pi", args: ["--x"], env: { A: "1" }, source: dep, shadows: [] }, "found walking up; source is the deployment");
  assert.deepEqual({ ...found.slow }, { name: "slow", harness: "codex", args: [], env: {}, source: dep, shadows: [] });
...
test("launch-config set/list/remove rewrite only the launch-configs block, round-trip hostile values, redact literals, and keep env on request", () => {
  ...
  const def = { harness: "claude", executable: "./bin/claude wrapper.sh", args: ["--flag", HOSTILE], env: { KEY: { fromEnv: "SRC" }, FOO: "lit # val" }, model: "claude-opus-5", yolo: false };
  ...
  assert.deepEqual(readLocal(join(scope, "oats-local.yaml"))["launch-configs"].fast, { harness: "claude", executable: "./bin/claude wrapper.sh", args: ["--flag", HOSTILE], env: { FOO: "lit # val", KEY: { fromEnv: "SRC" } }, model: "claude-opus-5", yolo: false }, "the kernel reads back exactly what was set");
  ...
  assert.deepEqual(r.json.result.configurations.map((c) => [c.name, c.harness, c.env, c.source, c.shadows]), [["fast", "claude", { FOO: { redacted: true }, KEY: { fromEnv: "SRC" } }, scope, []]]);
  ...
  write(join(base, "fast2.json"), JSON.stringify({ harness: "claude", args: ["--other"], model: "claude-sonnet-5" }));
  r = oats(["launch-config", "set", "fast", "--file", join(base, "fast2.json"), "--keep-env", "--dir", scope]);
  assert.equal(r.json.ok, true, r.stdout);
  assert.deepEqual(readLocal(join(scope, "oats-local.yaml"))["launch-configs"].fast, { harness: "claude", args: ["--other"], env: { FOO: "lit # val", KEY: { fromEnv: "SRC" } }, model: "claude-sonnet-5" });
  ...
  write(join(base, "fast3.json"), JSON.stringify({ harness: "pi", env: { X: "1" } }));
  ...
  write(join(base, "n.json"), JSON.stringify({ harness: "pi" }));
  ...
  write(join(base, "bad.json"), JSON.stringify({ harness: "bash" }));
...
test("configuration names that are Object.prototype properties work everywhere: merge, set, list, keep-env, remove", () => {
  ...
  for (const name of ["constructor", "toString", "__proto__".replace("__proto__", "hasOwnProperty")]) {
    write(join(base, `${name}.json`), JSON.stringify({ harness: "pi", env: { A: "1" } }));
    ...
    write(join(base, `${name}2.json`), JSON.stringify({ harness: "codex" }));
  ...
  assert.deepEqual([read.constructor.harness, read.constructor.source], ["pi", scope]);
...
test("the launch-configs block replacement leaves every other byte alone and handles inline and quoted key forms", () => {
  ...
  write(join(scope, "oats-local.yaml"), original);
  write(join(base, "b.json"), JSON.stringify({ harness: "pi" }));
  ...
  for (const form of ["launch-configs: {}\n", "launch-configs: {x: {harness: codex}}\n", "\"launch-configs\":\n  x:\n    harness: codex\n"]) {
    ...
    assert.deepEqual(back.b, { harness: "pi" }); if (form.includes("x")) assert.deepEqual(back.x, { harness: "codex" });
  }
...
test("an unrouted --server never reads or writes a local scope; a bad --file is refused without echoing its text", () => {
  ...
  write(join(scope, "oats-local.yaml"), LOCAL);
  write(join(base, "g.json"), JSON.stringify({ harness: "pi" }));
  ...
  write(join(base, "bad.json"), `{"harness": "pi", "env": {"K": "${secret}"} trailing`);
...
test("a top-level comment after the launch-configs block is not part of it: set and remove leave it where it is", () => {
  ...
  write(join(scope, "oats-local.yaml"), doc);
  write(join(base, "c-b.json"), JSON.stringify({ harness: "codex" }));
  ...
  write(join(base, "c-personal.json"), JSON.stringify({ harness: "codex" }));
  let r = oats(["launch-config", "set", "personal", "--file", join(base, "c-personal.json"), "--dir", scope]);
  assert.equal(r.json.ok, true, r.stdout);
  text = readFileSync(join(scope, "oats-local.yaml"), "utf8");
  assert.equal(text, `${LOCAL}launch-configs:\n  personal:\n    harness: codex\n\n# after the block\nsouls:\n  disabled: []\n`, text);
  assert.deepEqual(readLocal(join(scope, "oats-local.yaml"))["launch-configs"], { personal: { harness: "codex" } });
});
...
test("an oats-config.yaml in a deployment is a migration error (E_CONFIG_BROKEN), and set with no deployment in reach writes nothing", () => {
  ...
  for (const argv of [["launch-config", "list", "--dir", scope, "--json"], ["launch-config", "set", "x", "--file", join(base, "m.json"), "--dir", scope]]) {
    write(join(base, "m.json"), JSON.stringify({ harness: "pi" }));
    r = oats(argv);
