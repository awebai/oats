...
for (const runtime of RUNTIME_STUBS) {
  test(`spawn --no-launch without a ${runtime} executable is refused with nothing created; the inert stub is the only difference`, t => {
    ...
    const spawn = (PATH, purpose) => fx.cli(["spawn", "dev", "--purpose", purpose, "--runtime", runtime, "--no-launch", "--json"], { env: { PATH } }).json();
    ...
    const stubbed = spawn(`${inertRuntimeDir(fx.base)}:${minimal}`, "stubbed");
