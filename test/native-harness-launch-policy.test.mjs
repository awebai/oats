...
// module's hooks only, with tripwires in place of runtime/backend executables.
...
test("ordinary native launch retains complete OATS homes for Claude and Codex: module skills and inject, required spawn hook, normal retire", async t => {
  ...
  // Tripwires in place of every runtime/backend executable; node and git are the real ones.
  ...
  try {
    for (const runtime of ["claude", "codex"]) {
      ...
      const result = await fx.spawn("probe", { instance, runtime, task });
