...
test("desktop server: /api/cli reports discovery status; compatible fake CLI accepted via OATS_DESKTOP_OATS_BIN", async () => {
  ...
  try {
    ...
    assert.deepEqual(s.required, { desktopApi: 1, range: ">=0.25.8 <0.30.0" });
...
test("desktop server: 0.25.8 through 0.29.x CLIs are ACCEPTED and 0.30.0 is REJECTED at the band ceiling", async () => {
  for (const version of ["0.25.8", "0.26.0", "0.26.4", "0.27.0", "0.28.0", "0.29.0"]) {
  ...
  const next = fakeCli(badDir, { version: "0.30.0" });
  ...
  try {
    ...
    assert.equal(s.ok, false, "0.30.0 is past the exclusive ceiling and must not become the mutation binary");
    const tried = s.tried.find((t) => t.path === next.real);
    assert.ok(tried, "the rejected candidate is in diagnostics");
    assert.match(tried.reason, /outside >=0\.25\.8 <0\.30\.0/);
    assert.equal(tried.version, "0.30.0");
  } finally { b.proc.kill(); }
});
...
test("desktop server: remote roster, souls and harvest stay on the saved host route", async () => {
  ...
  try {
    ...
    const spawned = await fetch(`${base}/api/spawn`, { method: "POST", headers: { "content-type": "application/json" },
      body: JSON.stringify({ agent: "dev", agentsRoot: "/remote/project/agents", serverId: "host", harness: "codex" }) });
    ...
    assert.equal(spawnCall.argv[spawnCall.argv.indexOf("codex") - 1], "--runtime", "the local kernel's flag (no feature harness): its --server route translates");
