...
export async function readinessDocument(t, { selector = null, remoteOptions, catalog = null } = {}) {
  ...
  else if (d.standalone === true) member.push(item(`member ${t.soul.repoKey}`, "unknown", { producer: "workspace discovery", reason: `standalone view (${d.standaloneReason ?? "explicit"}): the workspace this repository declares could not be read, so its membership is declared, not confirmed`, evidence: { repoKey: t.soul.repoKey, workspace: d.key } }));
  else {
