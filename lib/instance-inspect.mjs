...
import { discoverOrStandalone, findSoulEntry, prepareInstance } from "./instance-resolution.mjs";
...
export function runProviderCheck(t, mod, dir, { timeoutMs = 30000 } = {}) {
  ...
  if (doc.ok !== true || !obj(res) || !PROVIDER_STATUSES.has(res.status) || !validProblems(res.problems)
    || (res.warnings !== undefined && !validProblems(res.warnings))) return { outcome: "unknown", problems: [{ code: "provider-unavailable", message: `${mod.name} check answered an unrecognised result` }] };
