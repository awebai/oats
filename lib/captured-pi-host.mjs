...
export function verifyCapturedPiHost(options, env = process.env) {
  ...
  if (!admitted || admitted.attempt !== intent.attempt || admitted.capability !== null || admitted.action.kind !== "session"
    || !["start", "restart"].includes(admitted.action.name) || !["running", "completed", "unconfirmed"].includes(admitted.state)) refuse("Pi host execution is not an admitted native action");
  const pending = parseStrictJson(readPortableBytes(join(home, ".oats-start-pending.json")));
  if (pending.id !== intent.executionId || !same(pending.capturedIntent, intent) || typeof pending.nativeRecordId !== "string" || !/^[a-f0-9-]{36}$/.test(pending.nativeRecordId)
    || pending.runtime !== "pi" || pending.model !== options.model) refuse("Pi host lacks the original native dispatch receipt");
  ...
  assertCapturedInstanceCustody(row);
  assertCapturedPiStart(home, pending.nativeRecordId, { incarnationId: row.incarnationId, intent, sessionDir });
