...
export function loadCapturedAction({ deployment, resolution, action }, codecs) {
  ...
  } else objectAt(action, action.kind === "hook" ? ["kind", "capability", "name"] : ["kind", "slot", "name"], action.kind === "hook" ? ["kind", "capability", "name"] : ["kind", "slot", "name"]);
  if (action.name !== undefined) stringAt(action.name, "/action/name");
  ...
  if (action.namespace !== undefined) {
    stringAt(action.namespace, "/action/namespace");
  ...
  if (bindings.length) {
    ...
    codecs.bindings(bindings, capability, record);
  }
  ...
  return { ...base, action, capability, operation, executable: { file, args } };
}
