...
export async function materialize(resolution, home, options = {}) {
  ...
  try {
    ...
    for (const { module, source } of sources) {
      ...
      assertPlainTree(stagedModule, module.name);
...
function assertPlainTree(dir, module) {
  for (const name of readdirSync(dir)) {
    const p = join(dir, name), st = lstatSync(p);
    if (st.isSymbolicLink()) throw fail("E_REMOTE_TREE_UNSAFE", `module ${module}: ${p} is a symlink`, { module, path: p, why: "symlink" });
    if (st.isDirectory()) { assertPlainTree(p, module); continue; }
