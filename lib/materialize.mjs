 ...
 *   <home>/instance.json.
 *
 * TRANSACTION: everything is built under <home>/.oats/.staging-<pid>/ and then renamed
 * into place. Any failure before the commit removes the staging directory and leaves
 * the home exactly as it was (a `.oats/` directory created only for staging is removed
 * too). The commit phase is a short sequence of renames whose targets were proven
 * absent up front; should one of them fail regardless, the renames already done are
 * undone before the error propagates.
 *
 * SOURCES. A member module is fetched from `<from.repoKey>@<from.commit>` at
 * `capabilities/<name>`; a package module from the package's repository at the LOCKED
 * commit (`from.commit`, which must equal the lock entry's `commit`) at
 * `<lock.path>/capabilities/<name>`. The repository of a package is `from.repoKey` when
 * the resolver recorded it, else derived from the lock entry's `source` ("git:<key>@<ref>"
 * directly; "catalog:<id>" through `options.catalog[id].url`). A repository that cannot
 * be determined is E_MATERIALIZE_SOURCE — nothing is guessed.
 *
...
function moduleSource(module, { lock, catalog, remote }) {
  ...
  if (from.kind === "member") {
    return { key: from.repoKey, ref: refForKey(from.repoKey), commit: from.commit, dir: `capabilities/${name}` };
  }
  ...
  return { key, ref: refForKey(key), commit: from.commit, dir: `${pkgPath}/capabilities/${name}` };
}
...
function soulBody(home, options) {
  ...
  candidates.push(join(home, "soul", "AGENTS.md"));
 ...
 *   soulAgentsMd      the soul's canonical AGENTS.md text (else read from options.soulDir or <home>/soul/AGENTS.md)
...
export async function materialize(resolution, home, options = {}) {
  ...
  for (const rel of [MODULES_DIR, SKILLS_DIR, ".agents", ".claude"]) {
    const p = join(homeAbs, rel);
    let st = null;
    try { st = lstatSync(p); } catch {}
    if (st && !st.isDirectory()) throw fail("E_MATERIALIZE_HOME", `${p} exists and is not a directory`, { home: homeAbs, path: p });
  }

  const oats = ensureOatsDir(homeAbs);
  const staging = join(oats.dir, `.staging-${process.pid}`);
  const discard = () => {
  ...
  let outcome;
  try {
    rmSync(staging, { recursive: true, force: true }); // a stale dir from a crashed run with our pid
    ...
    for (const { module, source } of sources) {
      ...
      let fetched;
      try { fetched = await fetch(source.ref, source.commit, source.dir, stagedModule, options.remoteOptions); }
      catch (e) {
      ...
      const digest = remote.contentDigest(stagedModule);
    ...
    // ── commit: renames whose targets were proven absent above ──
    const placed = [];
    try {
      mkdirSync(modulesRoot, { recursive: true, mode: 0o755 });
      mkdirSync(skillsRoot, { recursive: true, mode: 0o755 });
      for (const { module } of sources) {
        renameSync(join(staging, "modules", module.name), join(modulesRoot, module.name));
        placed.push(join(modulesRoot, module.name));
        if (existsSync(join(staging, "skills", module.name))) {
          renameSync(join(staging, "skills", module.name), join(skillsRoot, module.name));
          placed.push(join(skillsRoot, module.name));
        }
      }
      renameSync(join(staging, "AGENTS.md"), join(homeAbs, "AGENTS.md"));
      renameSync(join(staging, "instance.json"), instanceFile);
    } catch (e) {
      for (const p of placed) rmSync(p, { recursive: true, force: true });
      throw e;
    }
    rmSync(staging, { recursive: true, force: true });

    // Aliases (relative symlinks), only when absent — the canonical-plus-alias construction stays.
    const claudeMd = join(homeAbs, "CLAUDE.md");
    if (!linkPresent(claudeMd)) symlinkSync("AGENTS.md", claudeMd);
    const claudeSkills = join(homeAbs, ".claude", "skills");
    mkdirSync(join(homeAbs, ".claude"), { recursive: true, mode: 0o755 });
    if (!linkPresent(claudeSkills)) symlinkSync(join("..", ".agents", "skills"), claudeSkills);
