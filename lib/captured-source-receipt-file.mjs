...
export function withCapturedSourceReceiptFile(home, receipt, run) {
  ...
  const owned = lstatSync(directory), file = join(directory, "receipt.json"); let primary;
  try {
    writeFileSync(file, canonicalJson(receipt, CAPTURED_SOURCE_RECEIPT_LIMITS), { flag: "wx", mode: 0o400 });
    return run({ OATS_SOURCE_RECEIPT_FILE: file });
