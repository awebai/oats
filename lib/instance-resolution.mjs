 ...
 *  are refused here as well as in resolve (defense in depth). */
export function parseProviderFlags(pairs) {
  ...
  for (const [cap, kv] of pairs) {
    ...
    if (!/^[A-Za-z0-9_][A-Za-z0-9_.-]{0,127}$/.test(key) || ["__proto__", "constructor", "prototype"].includes(key)) throw err("E_BAD_ARGS", `--provider ${cap}: invalid key ${JSON.stringify(key)}`);
    out[cap] ??= {};
    // dotted keys nest: identity.source=retained:x → { identity: { source: "retained:x" } }
    const parts = key.split(".");
    let cur = out[cap];
    for (const p of parts.slice(0, -1)) { if (["__proto__", "constructor", "prototype"].includes(p)) throw err("E_BAD_ARGS", `--provider ${cap}: invalid key ${JSON.stringify(key)}`); cur[p] = cur[p] && typeof cur[p] === "object" ? cur[p] : {}; cur = cur[p]; }
...
export async function prepareInstance(contextDir, soulName, { spawn = {}, remoteOptions, local: localOverride, discovery: discoveryOverride } = {}) {
  ...
  const discovery = discoveryOverride ?? await discoverOrStandalone(local, { remoteOptions });
  const soulEntry = findSoulEntry(discovery, soulName);
  const resolution = await resolveSoul(discovery, soulEntry, { local, lock, spawn, remoteOptions });
 ...
 * member of a privately hosted workspace, a lone repo), fall back to the repo's
 * own souls and capabilities. `standalone: <repo ref>` in oats-local.yaml asks
 * for the standalone view explicitly (no workspace lookup at all).
 */
export async function discoverOrStandalone(local, { remoteOptions } = {}) {
  if (typeof local.standalone === "string" && local.standalone) {
    const repo = await discoverRepo(local.standalone, { remoteOptions });
    return standaloneRepo(local.standalone, repo.commit, repo);
  }
  try { return await discoverWorkspace(local.workspace, { local, remoteOptions }); }
  catch (e) {
    ...
    const repo = await discoverRepo(local.workspace, { remoteOptions });
    if (!repo.membership) throw e;
    try { await discoverWorkspace(repo.membership.workspace, { local, remoteOptions }); }
    catch (inner) { if (inner?.code !== "E_REMOTE_UNREADABLE" && inner?.code !== "E_REMOTE_UNREACHABLE") throw inner; return standaloneRepo(local.workspace, repo.commit, repo); }
    // Readable after all: the operator pointed oats-local.yaml at a member; use its workspace.
    return discoverWorkspace(repo.membership.workspace, { local, remoteOptions });
  }
}
