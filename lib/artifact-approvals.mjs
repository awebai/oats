...
function manifestFor(verified, id) {
  const resource = verified.record.dispatch.providerManifests[id];
  return parseStrictJson(readPortableBytes(verified.resources.get(resource), { invalidCode: "invalid-resolution" }));
}
...
export function inspectCapturedApprovals(scope, reference) {
  ...
  const capabilities = Object.entries(verified.record.artifacts.capabilities).map(([id, row]) => {
