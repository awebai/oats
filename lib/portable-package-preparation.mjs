...
import { chmodSync, existsSync, lstatSync, mkdirSync, mkdtempSync, readdirSync, realpathSync } from "node:fs";
...
import { copyTreeSafe, removeOwnedStaging } from "./artifact-tree.mjs";
...
function copyLocalPayload(source, destination) {
  const root = realpathSync(source), parent = realpathSync(join(destination, ".."));
  if (!lstatSync(root).isDirectory() || !outside(root, parent)) throw oatsError("invalid-source", "package input must be a directory outside its staging tree");
  // Local authoring is explicit, not permission to sweep live deployment/auth
  // state. Use a clean package subtree or Git observation for a live repo root.
  for (const name of [".aw", ".agents", ".oats-state"]) if (existsSync(join(root, name))) {
    throw oatsError("source-incomplete", "local package root contains private deployment state; select a clean package subtree or Git snapshot");
  }
  const agents = join(root, "agents");
  if (existsSync(agents) && lstatSync(agents).isDirectory()) for (const entry of readdirSync(agents, { withFileTypes: true })) {
    if (entry.isDirectory() && existsSync(join(agents, entry.name, "instances"))) throw oatsError("source-incomplete", "local package source includes instance homes");
  }
  mkdirSync(destination, { mode: 0o700 });
  for (const name of readdirSync(root)) if (name.toLowerCase() !== ".git") copyTreeSafe(join(root, name), join(destination, name));
  chmodSync(destination, lstatSync(root).mode & 0o7777);
}
...
export function preparePackageArtifacts({ requests, deployment, directory, repositories, kernel, observations = [], catalog, allowLocalPaths = false }) {
  ...
  const cached = new Map(); let counter = 0, closed = false;
  const cleanup = () => {
    if (closed) return;
  ...
  const observe = (source, selector, origin) => observations.find((value) => value.source.remote === source
    && (value.source.commit === selector || value.source.selector === selector)) ?? repositories.observe(source, { ...(selector === undefined ? {} : { revision: selector }), origin });
  const readPackage = (request) => {
    ...
    const manifest = kernel.loadPackageManifestAt(payload);
    for (const cap of manifest._capabilities) parseStrictJson(readPortableBytes(join(cap.dir, "oats.json")));
