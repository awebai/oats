...
export { validateProjectionPath } from "./source-projection.mjs";
    ...
    materialize(observation, roots, destination) {
      ...
      try {
        for (const entry of rows) {
          const bytes = blob(data, entry, maxBytes);
          if (entry.mode === "120000") projection.link(entry.path, decodeUtf8(bytes, "source link target"));
          else if (["100644", "100755"].includes(entry.mode)) projection.file(entry.path, bytes, entry.mode === "100755" ? 0o755 : 0o644);
