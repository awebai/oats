...
import { linkSync, lstatSync, mkdirSync, mkdtempSync, readdirSync, realpathSync, rmSync, writeFileSync } from "node:fs";
...
export function listResolutionEvidence(scope, { maxRecords = 256, maxBytes = 32 * 1024 * 1024, includeDocuments = false } = {}) {
  ...
  const entries = readdirSync(root, { withFileTypes: true }).filter((entry) => entry.name !== ".gitignore" && !entry.name.startsWith(".evidence-"));
