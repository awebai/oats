...
import { linkSync, lstatSync, mkdirSync, mkdtempSync, opendirSync, realpathSync, rmSync, writeFileSync } from "node:fs";
...
/** Bounded diagnostic enumeration of immutable evidence. Staging and managed
 * ignore metadata are not records; every other entry must be one valid file. */
export function listResolutionEvidence(scope, { maxEntries = 512, maxRecords = 256, maxBytes = 32 * 1024 * 1024, includeDocuments = false } = {}) {
  if (!Number.isSafeInteger(maxEntries) || maxEntries < 1 || maxEntries > 10_000
      || !Number.isSafeInteger(maxRecords) || maxRecords < 1 || maxRecords > 10_000
      || !Number.isSafeInteger(maxBytes) || maxBytes < 1 || maxBytes > 256 * 1024 * 1024
      || typeof includeDocuments !== "boolean") {
    throw oatsError("invalid-evidence", "invalid resolution evidence listing options");
  }
  let root;
  try { root = store(scope, false); }
  catch (error) { if (error.code === "evidence-not-found") return []; throw error; }
  const entries = [];
  let visited = 0;
  ...
  try {
    for (;;) {
      ...
      if (entry === null) break;
      if (++visited > maxEntries) throw oatsError("resource-limit", "resolution evidence visited-entry limit exceeded");
      if (entry.name === ".gitignore" || entry.name.startsWith(".evidence-")) continue;
      if (entries.length >= maxRecords) throw oatsError("resource-limit", "resolution evidence record limit exceeded");
      entries.push(entry);
    }
  ...
  for (const entry of entries.sort((a, b) => compareUtf8(a.name, b.name))) {
    const match = /^(sha256-[a-f0-9]{64})\.json$/.exec(entry.name);
    if (!entry.isFile() || !match) throw oatsError("invalid-evidence", "resolution evidence store contains an unsupported entry");
    const reference = { schemaVersion: 1, id: match[1] }, document = decode(join(root, entry.name), reference);
    totalBytes += Buffer.byteLength(canonicalJson(document));
    if (totalBytes > maxBytes) throw oatsError("resource-limit", "aggregate resolution evidence byte limit exceeded");
    results.push({ reference, target: document.target, status: document.status, unresolved: document.unresolved.length,
      ...(includeDocuments ? { document } : {}) });
  }
  return results;
}
...
/** Persist a narrow verified home→historical capability association as
...
export function commitHistoricalHomeCapabilityEvidence(scope, inventory, request, adapters = {}) {
  ...
  const proof = { kind: candidate.artifact.package ? "home-capability-v2" : "home-capability-v1", home: candidate.home.path,
    capability: { id: candidate.artifact.capability.id, version: candidate.artifact.capability.row.version,
      package: candidate.artifact.package?.id ?? null, path: candidate.artifact.package ? candidate.artifact.capability.row.path : null },
