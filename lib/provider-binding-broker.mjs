...
export function invokeProviderBinding(options,codecs) {
  ...
  objectAt(options,['deployment','artifacts','capability','phase','settings','input'],['deployment','artifacts','capability','phase','settings','input']);
  const {deployment,artifacts,capability,phase,settings,input}=options;
  ...
  try {
    result=spawnSync(process.execPath,[file,...args],{cwd:root,env:environment,input:canonicalJson(request,BINDING_LIMITS),
      timeout:30000,maxBuffer:BINDING_LIMITS.maxBytes,stdio:['pipe','pipe','pipe'],shell:false});
