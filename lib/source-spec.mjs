...
import { canonicalJson, scalarString } from "./portable-values.mjs";
...
/** Package dependency inputs retain existing CLI conveniences, but never infer
 * an ambient local base or allow an unpinned Git dependency. Soul requirements
 * continue to use parsePortableSource and cannot borrow catalog nicknames. */
export function parsePackageDependency3(spec, { localBase, allowLocalPaths = false } = {}) {
  text(spec);
  if (spec.startsWith("repo:") || spec.startsWith("path:")) return parsePortableSource(spec, { localBase, allowLocalPaths });
  if (spec.startsWith(".") || spec.startsWith("/") || spec.startsWith("~")) return parsePortableSource(`path:${spec}`, { localBase, allowLocalPaths });
  if (spec.startsWith("git:") && !spec.startsWith("git://")) return parsePortableSource(spec);
  if (/^(https?:\/\/|ssh:\/\/|git:\/\/|git@)/.test(spec)) return parsePortableSource(`git:${spec}`);
  const catalog = /^(?:catalog:)?([a-z0-9][a-z0-9._-]*)(?:@([^\s#]+))?$/.exec(spec);
  if (!catalog) refuse("invalid package dependency source");
  const source = `catalog:${catalog[1]}${catalog[2] ? `@${catalog[2]}` : ""}`;
  parseLockedSource3(source, "oats-package");
  return { kind: "catalog", source, id: catalog[1], ...(catalog[2] ? { selector: catalog[2] } : {}) };
}

/** Repository-relative selections carry the exact declaring observation. */
export function validateRepoAnchor(anchor) {
  canonicalJson(anchor);
  if (!anchor || Object.keys(anchor).sort().join(",") !== "revision,source") refuse("invalid repository snapshot anchor");
  if (typeof anchor.source === "string" && anchor.source.startsWith("path:")) {
    parseLockedSource3(anchor.source, ".");
    if (anchor.revision !== "local") refuse("local source anchor requires a local observation");
  } else if (parseRepositorySource(anchor.source).normalized !== anchor.source || typeof anchor.revision !== "string" || !/^[a-f0-9]{40}$/.test(anchor.revision)) {
    refuse("repository source anchor requires its exact observation");
  }
  return anchor;
}

/** Normalized choice values cannot disagree with their own source/path/url fields.
 * This is still data validation, not acquisition or local-path authorization. */
export function validateSelectionSource(value) {
  const encoded = canonicalJson(value);
  let expected;
  if (value?.kind === "git") {
    portablePath(value.path, { allowRoot: true });
    expected = parsePortableSource(`${value.source}#${value.path}`);
  } else if (value?.kind === "path") expected = parsePortableSource(value.source, { allowLocalPaths: true });
  else if (value?.kind === "repo") expected = { ...parsePortableSource(`repo:${value.path}`), anchor: validateRepoAnchor(value.anchor) };
  else refuse("invalid normalized selection source");
  if (canonicalJson(expected) !== encoded) refuse("normalized selection source fields disagree");
  return value;
}
