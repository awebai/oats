...
import { constants, closeSync, fstatSync, linkSync, lstatSync, mkdirSync, mkdtempSync, openSync, readSync, readlinkSync, realpathSync, rmSync, writeFileSync } from "node:fs";
...
function readBytes(path, missingCode = "resolution-not-found") {
  const stat = exists(path);
  if (!stat) throw oatsError(missingCode, "captured metadata is absent");
  if (!stat.isFile()) throw oatsError("invalid-resolution", "captured resolution must be a regular file");
  if (stat.size > MAX_RECORD_BYTES) throw oatsError("resource-limit", "captured resolution byte limit exceeded");
  const fd = openSync(path, constants.O_RDONLY | (constants.O_NOFOLLOW ?? 0));
  try {
    const before = fstatSync(fd);
    if (!before.isFile() || before.dev !== stat.dev || before.ino !== stat.ino || before.size !== stat.size) throw oatsError("integrity-drift", "resolution changed during opening");
    const buffer = Buffer.alloc(stat.size + 1);
    let count = 0, size;
    while (count < buffer.length && (size = readSync(fd, buffer, count, buffer.length - count, null)) > 0) count += size;
    const after = fstatSync(fd);
    if (count !== before.size || after.size !== before.size || after.mtimeMs !== before.mtimeMs || after.ctimeMs !== before.ctimeMs) throw oatsError("integrity-drift", "resolution changed during reading");
...
function verifySoulConstraints(record, root, definition) {
  ...
  const parsed = parsePortableSoul(readBytes(definition, "resource-not-found"), { localBase, allowLocalPaths: true });
  ...
  const check = (id, selection, pointer) => {
    ...
    for (const [key, expected] of Object.entries(selection.settings ?? {})) {
      ...
      if (!choiceKey || !Object.hasOwn(record.choices, choiceKey)
...
export function verifyResolutionInputs(scope, reference, { draft, maxRecords = 256 } = {}) {
  ...
  const verify = (ref, supplied, depth) => {
    ...
    try {
      ...
      for (const [id, resourceKey] of Object.entries(record.dispatch.providerManifests)) {
        const manifest = parseStrictJson(readBytes(resources.get(resourceKey), "resource-not-found"));
        const row = record.artifacts.capabilities[id];
        if (manifest.capability !== id || manifest.version !== row.version) throw oatsError("invalid-resolution", "retained manifest identity/version differs from its captured row");
        ...
        if (row.origin.kind === "package") {
          const pkg = record.artifacts.packages[row.origin.package];
          const provenance = parseStrictJson(readBytes(join(rootOf(row.artifact), ".oats-installation.json"), "resource-not-found"));
...
export function commitCapturedResolution(scope, draft) {
  ...
  verifyResolutionInputs(scope, reference, { draft });
  const root = store(scope, true), target = join(root, `${reference.id}.json`);
  if (exists(target)) { decodeRecord(target, reference); return reference; }
