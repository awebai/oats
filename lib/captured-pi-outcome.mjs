...
import { closeSync, constants, fstatSync, fsyncSync, lstatSync, openSync, realpathSync, renameSync, writeSync } from "node:fs";
...
export function readPiOutcome(kind, authority, check) {
  ...
  check();
  try { if (lstatSync(path).size > limits.maxBytes) bad("Pi outcome exceeds its bound"); }
  catch (error) { if (error.code !== "ENOENT") throw error; }
  const bytes = readPortableBytes(path, { allowMissing: true });
  check();
  return bytes === null ? null : validateReceipt(parseStrictJson(bytes, limits), kind, authority);
}
