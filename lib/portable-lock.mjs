...
import { portableScope, withPortableStateWrite } from "./portable-state.mjs";
...
export function writeLock3(scope, expectedPreviousIntegrity, next) {
  ...
  validateLock3(next);
  return withPortableStateWrite(scope, ({ deployment, root }) => {
    ...
    const staging = mkdtempSync(join(root, ".selection-")), candidate = join(staging, "lock.json");
    let failed = false, primary;
    try {
      writeFileSync(candidate, canonicalJson(next), { flag: "wx", mode: 0o600 });
      const target = join(deployment, "oats-lock.json");
      if (previous.lock === null) {
        try { linkSync(candidate, target); }
        catch (error) { if (error.code === "EEXIST") throw oatsError("selection-changed", "a selection lock appeared during first publication"); throw error; }
      ...
      try { rmSync(staging, { recursive: true, force: true }); }
        ...
        const error = failed ? new AggregateError([primary, cleanup], "selection write and staging cleanup failed", { cause: primary })
          : new Error("selection staging cleanup failed", { cause: cleanup });
