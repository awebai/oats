...
export function uninstallHostUnit({ exec, os = platform() } = {}) {
  const p = hostUnitPaths(os);
  if (p.kind === "launchd") {
    const before = hostUnitStatus({ exec, os });
    if (before.active) {
      const r = run(exec, ["launchctl", "bootout", `gui/${userInfo().uid}/${LAUNCHD_LABEL}`]);
      if (!r.ok || hostUnitStatus({ exec, os }).active) throw scheduleError("E_SCHEDULE_HOST", `launchctl bootout did not unload ${LAUNCHD_LABEL}: ${r.err || "still loaded"}; the unit file is kept`);
    }
  ...
  if (p.kind === "systemd") {
    ...
    if (before.active) {
      ...
      if (!r.ok || hostUnitStatus({ exec, os }).active) throw scheduleError("E_SCHEDULE_HOST", `systemctl --user disable --now ${SYSTEMD_UNIT}.timer failed: ${r.err || "still active"}; the unit files are kept`);
    }
