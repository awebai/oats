...
import { validateOrigin, validateProviderBinding, validateMessagingChoice, validateCapturedChoices, validateCapturedContext } from './resolution-shape.mjs';
import { validateCapturedInvocationContext } from './captured-invocation-context.mjs';
...
export function validateBindingRequest(request) {
  ...
  if (request.phase === 'normalize') {
    ...
    objectAt(input,['binding','context','action','invocation'],['binding','context','action']);
    ...
    if (Object.hasOwn(input,'invocation')) {
      const invocation=validateCapturedInvocationContext(input.invocation);
      if (invocation.capability !== request.capability || !same(invocation.context,input.context) || !same(invocation.action,input.action)) fail();
      if (request.slot === 'messaging' && (!invocation.messagingChoice.enabled || invocation.messagingChoice.privateKey.provider !== request.capability)) fail();
    }
  }
  validateCapturedContext(input.context);
  return request;
}
...
export function decodeBindingResponse(bytes,request,{reasons}={}) {
  ...
  try {
    ...
    reasons=validateBindingReasons(reasons === undefined ? providerReasons({capability:request.capability}) : reasons);
