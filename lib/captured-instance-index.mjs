...
const STATUSES = new Set(["scaffolded-hooks-pending", "spawn-hooks-running", "spawned-launch-pending", "spawned-cleanup-required", "spawn-failed-cleanup-required", "retire-running", "retire-failed-cleanup-required", "start-running", "start-dispatched", "start-failed-cleanup-required"]);
...
function validateAdmittedAction(action,capability) {
  if(action?.kind==='session'){
    objectAt(action,['kind','name'],['kind','name']);
    if(!['start','restart'].includes(action.name)||capability!==null)throw oatsError('invalid-declaration','invalid kernel session admission');
  }else{
    validateCapturedAction(action);
    if(!isMaterializedCapabilityId(capability)||action.capability!==undefined&&action.capability!==capability)throw oatsError('invalid-declaration','invalid intent capability owner');
  }
}
function validateIntent(intent) {
  ...
  validateAdmittedAction(intent.action,intent.capability);
  stringAt(intent.executionId,"/executionId",{pattern:/^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$/});
  if (!["command","operation","hook","session"].includes(intent.action.kind) || !INTENT_STATES.has(intent.state)) throw oatsError("invalid-declaration","invalid admitted action/state");
...
export function validateCapturedInstanceReference(row) {
  objectAt(row,["home","instance","agent","kind","status","executionBinding","responsibleHuman","incarnationId","custody","intents","custodyFailure","nativeScaffold"],
  ...
  if(row.nativeScaffold!==undefined){
    objectAt(row.nativeScaffold,['schemaVersion','directories'],['schemaVersion','directories']);
    if(row.nativeScaffold.schemaVersion!==1)throw oatsError('invalid-declaration','invalid native scaffold witness');
    const names=['historyRoot','history','retirementRoot','baselines'];objectAt(row.nativeScaffold.directories,names,names);
    for(const value of Object.values(row.nativeScaffold.directories))directoryIdentity(value);
  }
...
function sameAuthority(a,b){return AUTHORITY_FIELDS.every(key=>same(a[key],b[key]))&&same(a.nativeScaffold??null,b.nativeScaffold??null);}
...
export function recordCapturedCustodyFailure(original,{meta={},intents={},code="integrity-drift",message="captured home/work custody changed"}={}) {
  ...
  return withPortableStateWrite(original.executionBinding.deployment,context=>{
    ...
    // This reporter owns only the activation transition it acquired before
    // hooks. Check it before changing ANY receipt/status, even on failure.
...
export function markCapturedNativeScaffold(deployment,home,proof) {
  return withPortableStateWrite(deployment,context=>{
    const index=readCapturedInstanceIndex(context.deployment),{row,metadata}=matchedRow(index,home);
    if(row.nativeScaffold||row.status!=='scaffolded-hooks-pending'||row.intents.length)throw oatsError('selection-changed','native scaffold cannot be backfilled after admission');
    if(!same(metadata.captured.nativeScaffold,proof))throw oatsError('invalid-resolution','native scaffold proof differs from owned metadata');
    row.nativeScaffold=proof;save(context,index);return row;
  });
}
...
export function admitCapturedInstanceAction({deployment,home,executionBinding,capability,action,input={},retryExecutionId,expectedStatus}) {
  validateExecutionBinding(executionBinding);
  if(expectedStatus!==undefined&&!STATUSES.has(expectedStatus))throw oatsError('invalid-declaration','invalid admission source state');
  validateAdmittedAction(action,capability);canonicalJson(input,{maxBytes:128*1024,maxDepth:24,maxEntries:8192});
  ...
  return withPortableStateWrite(deployment,context=>{
    ...
    if(expectedStatus!==undefined&&(row.status!==expectedStatus||metadata.captured.lifecycle!==expectedStatus)){
      const error=oatsError('selection-changed','native admission no longer owns matching source lifecycle');error.home=row.home;
      error.capturedCustody={incarnationId:row.incarnationId,executionBinding:row.executionBinding,indexedStatus:row.status,metadataStatus:metadata.captured.lifecycle,expectedStatus,held:true};
      throw error;
    }
    let intent;
    if(retryExecutionId!==undefined){
      ...
      const receipt=previous?previous.receipt:capability!==null&&metadata.capabilityMeta&&Object.hasOwn(metadata.capabilityMeta,capability)?metadata.capabilityMeta[capability]:null;
...
export function failCapturedNativeIntent(original,ref,receipt) {
  canonicalJson(receipt,RECEIPT_LIMITS);
  return withPortableStateWrite(original.executionBinding.deployment,context=>{
    const index=readCapturedInstanceIndex(context.deployment),row=index.instances.find(item=>item.incarnationId===original.incarnationId);
    if(!row||!sameAuthority(original,row))throw oatsError('selection-changed','native failure lost original indexed authority');
    if(row.status!=='start-running')throw oatsError('selection-changed','native failure no longer owns lifecycle state');
    const intent=matchedIntent(row,ref);
    if(intent.action.kind!=='session'||intent.capability!==null)throw oatsError('invalid-resolution','native failure is not its session intent');
    if(intent.state!=='completed'){intent.state='unconfirmed';intent.receipt=receipt;intent.replayable=false;}
    row.status='start-failed-cleanup-required';save(context,index);return intentRef(row,intent);
  });
}
/** An allocated native target is retained under the running logical intent
 * BEFORE launch. It is an observation, not completion or a new identity. */
export function observeCapturedNativeTarget(original,ref,receipt) {
  validateCapturedInstanceReference(original);canonicalJson(receipt,RECEIPT_LIMITS);
  return withPortableStateWrite(original.executionBinding.deployment,context=>{
    const index=readCapturedInstanceIndex(context.deployment),{row}=matchedRow(index,original.home);
    if(!sameAuthority(original,row)||row.status!=='start-running')throw oatsError('selection-changed','native target observation lost original lifecycle custody');
    const intent=matchedIntent(row,ref);
    if(intent.capability!==null||intent.action.kind!=='session'||intent.state!=='running')throw oatsError('selection-changed','native target observation needs the running admitted session');
    intent.receipt=receipt;intent.replayable=false;save(context,index);return intentRef(row,intent);
  });
}
