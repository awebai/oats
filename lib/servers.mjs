...
export function validateServer(id, s) {
  ...
  for (const k of ["oatsPath", "herdrPath", "label"]) {
    if (s[k] !== undefined && s[k] !== null && typeof s[k] !== "string") throw serverError("E_SERVER_INVALID", `server ${id}: ${k} must be a string`);
  }
  for (const k of Object.keys(s)) {
    if (!["label", "sshHost", "workspace", "oatsPath", "herdrPath"].includes(k)) throw serverError("E_SERVER_INVALID", `server ${id}: unknown field ${JSON.stringify(k)} (a registration holds label, sshHost, workspace, oatsPath, herdrPath and nothing else — never keys or passwords)`);
  }
  return true;
}
...
export function sshArgv(target, oatsArgs) {
  const cmd = [target.oatsPath, ...oatsArgs].map(remoteQuote).join(" ");
