...
export function routeCommand(serverId, cmd, oatsArgs, io = {}) {
  ...
  if (OPERATIONS_COMMANDS.has(cmd)) {
    ...
    const scoped = oatsArgs.includes("--dir") || oatsArgs.includes("--home") ? oatsArgs : [...oatsArgs, "--dir", target.workspace];
    const { envelope, stderr } = runRemote(target, json([cmd, ...scoped]), io);
    return { envelope: envelope.result && typeof envelope.result === "object" ? { ...envelope, result: { ...envelope.result, server: serverId } } : envelope, stderr };
  }
...
export const OPERATIONS_REMOTE_VERSION = "0.22.15";
