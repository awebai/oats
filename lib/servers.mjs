...
export function targetKey(target) {
  return createHash("sha256").update(`${target.sshHost}\0${target.workspace}\0${target.oatsPath || "oats"}`).digest("hex").slice(0, 12);
}
...
export function routeCommand(serverId, cmd, oatsArgs, io = {}) {
  ...
  if (cmd === "retire") {
    ...
    checkRemote(target, io); // a mutation: version and envelope first, like spawn
  ...
  if (cmd === "harvest") {
    ...
    const name = instanceArg;
    const snap = name ? readSnapshot(serverId, name) : undefined;
    ...
    const { envelope, stderr } = runRemote(snap.target || target, ["okf", "harvest", "--json"], { ...io, cwd: snap.home });
    return { envelope: envelope.result ? { ...envelope, result: { ...envelope.result, server: serverId, instance: name, home: snap.home } } : envelope, stderr };
  }
  if (cmd === "status") {
...
export const ROSTER_PER_TARGET_TIMEOUT_MS = 60000;
...
/** The remote roster, grouped by saved route target: one bounded status
 *  pull per target, registrations unioned with saved routes so a removed or
 *  changed registration keeps its group (registrationPresent false) and its
 *  instances from the snapshots (running null when the probe fails). Remote
 *  state is pulled, never cached; the saved route stays the action authority. */
export function rosterGroups({ server, io = {} } = {}) {
  ...
  const add = (serverId, target, registrationPresent, label) => {
    ...
    if (!groups.has(key)) groups.set(key, { id: key, server: serverId, label: label || serverId, registrationPresent, target, probe: null, agentsRoot: undefined, souls: [], instances: [], _snapshots: [] });
  ...
  for (const g of groups.values()) {
    let status;
    try { status = runRemote(g.target, ["status", "--json", "--dir", g.target.workspace], { ...io, timeoutMs: perTargetTimeoutMs }).envelope; }
    ...
    if (status?.ok) {
      ...
      for (const a of status.result.agents || []) {
        ...
        for (const i of a.instances || []) {
          const snap = bySnapshot.get(i.instance);
          g.instances.push({
            server: g.server, instance: i.instance, agent: a.name, home: i.home || snap?.home, agentsRoot: status.result.root,
            runtime: i.runtime, backend: i.sessionTarget?.backend || (i.tmux ? "tmux" : undefined),
            ...
            retirePending: !!i.retirePending, savedRoute: !!snap,
          });
          bySnapshot.delete(i.instance);
        }
      }
    }
    for (const snap of bySnapshot.values()) {
      g.instances.push({ server: g.server, instance: snap.instance, agent: snap.agent, home: snap.home, agentsRoot: snap.agentsRoot, running: null, savedRoute: true, ...(g.probe?.ok ? { runtimeError: "not in the remote roster: retired there, or its home removed" } : {}) });
    }
    delete g._snapshots;
  }
  return { groups: [...groups.values()], bounds: { perTargetTimeoutMs, elapsedMs: Date.now() - started } };
}
