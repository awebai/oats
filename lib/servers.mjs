...
export function routeCommand(serverId, cmd, oatsArgs, io = {}) {
  ...
  if (cmd === "retire") {
    ...
    // that resolves --home retires that one, never a same-named twin.
    const homeArgs = snap?.home && Array.isArray(remote?.features) && remote.features.includes("retire-home") && !oatsArgs.includes("--home") ? ["--home", snap.home] : [];
  ...
  if (cmd === "harvest") {
    ...
    // envelope first, and the remote must route harvest itself, since an
    // older okf there answers outside the envelope.
    const remote = checkRemote(target, io);
...
export function rosterGroups({ server, io = {} } = {}) {
  ...
  for (const g of groups.values()) {
    ...
    for (const snap of bySnapshot.values()) {
      g.instances.push({ server: g.server, instance: snap.instance, agent: snap.agent, home: snap.home, agentsRoot: snap.agentsRoot, runtime: snap.runtime || null, running: null, retirePending: false, rollbackIncomplete: false, savedRoute: true, missingRemotely: g.probe?.ok === true });
    }
    delete g._snapshots;
  }
