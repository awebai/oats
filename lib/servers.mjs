...
function requireSessionRemote(target, io) {
  const remote = checkRemote(target, io);
  if (compareSemver(remote.version, SESSION_REMOTE_VERSION) < 0) {
    throw serverError("E_REMOTE_INCOMPATIBLE", `remote oats ${remote.version} at ${target.sshHost} has no \`oats session\` commands (they ship in ${SESSION_REMOTE_VERSION}); upgrade it there, or attach with ssh -t ${target.sshHost} tmux attach -t oats`);
  }
  return remote;
}
