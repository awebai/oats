...
export function buildCapturedInvocationContext(input) {
  ...
  if (instance !== null) {
    ...
    const metadata = parseStrictJson(readPortableBytes(join(home, "instance.json")));
    ...
    priorReceipt = metadata.capabilityMeta && Object.hasOwn(metadata.capabilityMeta, loaded.capability.id) ? metadata.capabilityMeta[loaded.capability.id] : null;
  }
