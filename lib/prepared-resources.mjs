...
export function completePreparedResources({ seed, plan, manifests, mode, deployment, directory }, kernel) {
  ...
  for (const [id, manifest] of manifests) {
    ...
    if (manifest.layer) problems.push({ code: "provider-not-qualified", message: `provider ${id} needs its binding adapter before complete preparation`, origins: [] });
  ...
  const build = (subject, workMode, helpers) => {
    ...
    return { ...seed, subject, bindings: {}, messagingChoice: { schemaVersion: 1, enabled: false }, resources, resourceBundles: [bundle], helpers,
