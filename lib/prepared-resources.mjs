...
export function completePreparedResources({ seed, plan, manifests, mode, deployment, directory }, kernel) {
  ...
  const injectionFiles = ["oats-portable.md", "instance-boundary.md", ...kernel.workModes.map((item) => `work-${item}.md`)];
  ...
  for (const { key, subject, helperMode } of helperPlans) helpers[key] = commitCapturedResolution(deployment, build(subject, helperMode, {}));
