...
export function contentDigest(dir) {
  ...
  const walk = (abs, rel) => {
    for (const name of readdirSync(abs)) {
      ...
      if (s.isSymbolicLink()) throw fail("E_REMOTE_TREE_UNSAFE", `${r} is a symlink`, { path: r, why: "symlink" });
  ...
  for (const f of files) d.add(f.path, f.mode, readFileSync(f.abs));
...
export async function fetchRemoteTree(refText, commit, dir, destDir, options = {}) {
  ...
  for (const e of entries) {
    ...
    if (e.mode === "120000") throw fail("E_REMOTE_TREE_UNSAFE", `${shown} is a symlink`, { path: shown, why: "symlink", key: ref.key, commit });
