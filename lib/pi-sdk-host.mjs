...
/** Non-authorizing metadata from actual PUBLIC SDK/session objects. No prompt,
 * content, raw error, credential/profile or request-as-result material is copied. */
export function observePiSdkSession(session, sdkVersion) {
  const text = value => typeof value === "string" && value.length > 0 && value.length <= 4096 && !value.includes("\0") ? value : null;
  const manager = session.sessionManager, header = manager.getHeader();
  const messages = session.messages, last = messages.at(-1);
  const entry = manager.getBranch().findLast(value => value.type === "message");
  const native = entry?.message;
  const matchesLast = last?.role === "assistant" && native?.role === "assistant"
    && native.timestamp === last.timestamp && native.provider === last.provider
    && native.model === last.model && native.stopReason === last.stopReason;
  const finalAssistant = matchesLast && text(entry.id) && text(native.provider) && text(native.model)
    && Number.isFinite(native.timestamp) && native.timestamp >= 0
    && ["stop", "length", "toolUse", "error", "aborted"].includes(native.stopReason)
    ? { entryId: entry.id, provider: native.provider, model: native.model,
      responseModel: text(native.responseModel), stopReason: native.stopReason, timestamp: native.timestamp } : null;
  return {
    sdkVersion: text(sdkVersion),
    header: header?.type === "session" && Number.isSafeInteger(header.version) && text(header.id) && text(header.cwd) && text(header.timestamp)
      ? { type: "session", version: header.version, id: header.id, cwd: header.cwd, timestamp: header.timestamp } : null,
    sessionId: text(manager.getSessionId()), sessionFile: text(manager.getSessionFile()),
    model: text(session.model?.provider) && text(session.model?.id) ? { provider: session.model.provider, id: session.model.id } : null,
    finalAssistant,
  };
}
...
export async function runPiSdkHost(argv, { verifyContext, onOutcome, importSdk = url => import(url), cwd = process.cwd() } = {}) {
  ...
  let handedToPrint = false, observation = null;
  try {
    if (onOutcome) runtime.setBeforeSessionInvalidate(() => {
      // Public synchronous hook: capture real session data BEFORE disposal, never
      // synthesize a header/model/final message from the requested launch values.
      try { observation = observePiSdkSession(runtime.session, sdk.VERSION); }
      catch { observation = null; } // unavailable evidence cannot qualify success
    });
    await verify();
    ...
    const verified = await verify(); // no successful host outcome after observed custody drift
    if (!Number.isInteger(exitCode) || exitCode < 0 || exitCode > 255) fail("E_PI_HOST_OUTCOME", "native print mode returned no valid status");
    if (onOutcome) await onOutcome(options, { exitCode, observation }, verified);
    return exitCode;
  } finally {
