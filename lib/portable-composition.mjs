...
export function planSoftwareChoices(input) {
  ...
  const requirements = soulConstraints(soul), candidates = [];
  ...
  const collect = (parsed, policy, pointer, kind, sourceContext) => {
    const selection = (id, item, at, key) => {
      add(parsed, key, item === false || item === "none" ? null : sourceSelectionValue(id, parsed, at, sourceContext), at, kind);
      if (item && typeof item === "object") for (const [name, value] of Object.entries(item.settings ?? {})) {
        add(parsed, settingChoiceKey(id, name), value, `${at}/settings/${pointerKey(name)}`, kind);
      }
    };
  ...
  if (operator) {
    objectAt(operator, ["policy", "document", "localBase", "allowLocalPaths", "sourceContext"], ["policy", "document"]);
  ...
  return { ...result, status: problems.some((problem) => problem.code === "requirement-conflict") ? "conflict" : result.status,
    requirements: allRequirements, candidates, capabilities, providers, settings, problems,
