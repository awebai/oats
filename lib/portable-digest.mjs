...
export function validateIntegrity(value, formats = [TREE_FORMAT, PACKAGE_FORMAT, JSON_FORMAT, BYTES_FORMAT]) {
  const invalid = () => { throw oatsError("invalid-artifact-reference", "expected explicit supported digest format and full SHA-256"); };
  if (!value || typeof value !== "object" || Array.isArray(value)) invalid();
  const prototype = Object.getPrototypeOf(value), fields = Object.getOwnPropertyDescriptors(value);
  if ((prototype !== null && prototype !== Object.prototype) || Reflect.ownKeys(fields).length !== 2) invalid();
  for (const key of ["format", "value"]) {
    if (!Object.hasOwn(fields, key) || !Object.hasOwn(fields[key], "value") || !fields[key].enumerable) invalid();
  }
  if (!formats.includes(fields.format.value) || typeof fields.value.value !== "string"
      || !/^sha256-[a-f0-9]{64}$/.test(fields.value.value)) invalid();
  return value;
}
