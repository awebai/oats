...
export function localEntry(kind, def, { invalid = null } = {}) {
  const enabled = def.enabled !== false;
  return { kind, id: localId(def.id), name: def.id, member: LOCAL, description: null, runsOn: null, owner: null, enabled, origin: { kind: "local", path: "oats-schedules.json" }, definition: def, ...(invalid ? { invalid } : {}), placement: { runsHere: enabled && !invalid, reason: null, enabledHere: enabled } };
}
...
export function automationContext(dep, { local, io, now = new Date(), refresh = false, cache = new Map() } = {}) {
  ...
  const place = (list) => (list || []).map((a) => ({ ...a, placement: placementOf(a, { host, io, cache }) }));
  return { snapshot, host, ...Object.fromEntries(KIND_NAMES.map((k) => [`${k}s`, place(snapshot?.[`${k}s`])])), refresh: refreshed };
}
