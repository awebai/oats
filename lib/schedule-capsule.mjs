 ...
 *
 * A captured definition stores content identity (`capsuleIntegrity`). Each
 * admitted attempt then receives a fresh opaque `executionId`. Two attempts of
 * identical retained bytes are therefore distinct intents without inventing a
 * different software identity. This module performs no preparation, approval,
 * launch, or ambient resolution. */
...
const CAPTURE_FIELDS = Object.freeze(["schemaVersion", "capsuleIntegrity", "resolution", "deployment", "action", "target", "inputRefs", "responsibleHuman"]);
const CAPSULE_FIELDS = Object.freeze([...CAPTURE_FIELDS, "executionId"]);
...
function contentIntegrity(value) { return jsonIntegrity(capturePayload(value)); }

export function buildCommandExecutionCapture({ cwd, argv, inputRefs = {}, responsibleHuman = null }) {
  ...
  return { ...payload, capsuleIntegrity: contentIntegrity(payload) };
}

export function validateExecutionCapture(value) {
  canonicalJson(value, { maxBytes: 1024 * 1024, maxDepth: 64, maxEntries: 20_000 });
  exactObject(value, CAPTURE_FIELDS, "captured execution");
  if (value.schemaVersion !== EXECUTION_CAPSULE_VERSION) throw oatsError("unsupported-wire-version", "unsupported captured execution version");
  validateIntegrity(value.capsuleIntegrity, [JSON_FORMAT]);
  ...
  if (parsed.deployment !== value.deployment || !same(parsed.resolution, value.resolution)
      || !same(parsed.action, value.action)) throw oatsError("invalid-declaration", "captured execution disagrees with its saved command selectors");
  if (!same(contentIntegrity(value), value.capsuleIntegrity)) throw oatsError("invalid-declaration", "capsule integrity does not match its immutable contents");
  return value;
}

export function admitExecutionCapture(capture, { executionId = randomUUID() } = {}) {
  validateExecutionCapture(capture);
  ...
  const capsule = { ...copied(capture), executionId };
  return validateExecutionCapsule(capsule);
}
...
export function validateExecutionCapsule(value) {
  ...
  const { executionId: _executionId, ...capture } = value;
  validateExecutionCapture(capture);
  return value;
}

export function capturedDispatchAction(value) {
  const capture = Object.hasOwn(value, "executionId") ? (() => { validateExecutionCapsule(value); const { executionId: _executionId, ...rest } = value; return rest; })() : validateExecutionCapture(value);
  return capturedCommandTarget(capture.target).dispatchAction;
}
