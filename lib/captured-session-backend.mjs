...
import { HERDR_PROTOCOLS, isSupportedHerdrProtocol, validHerdrTarget } from './herdr.mjs';
...
export function validateCapturedSessionBackend(value) {
  ...
  if(value.backend==='herdr'){
    if(!isSupportedHerdrProtocol(value.protocol))throw oatsError('needs-configuration',`captured Herdr requires an explicitly selected protocol in ${HERDR_PROTOCOLS.join(', ')}`);
