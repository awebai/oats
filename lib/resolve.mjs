...
export async function resolveSoul(discovery, soulEntry, { local = null, lock = null, spawn = {}, catalog = null, remote: injected, remoteOptions, kernel = KERNEL_VERSION } = {}) {
  ...
  for (const m of modules) {
    ...
    if (m.layer === "messaging" && isObject(workspace?.messaging)) {
      ...
      // Validation is unchanged: the primary's entry may nest no byTeam and carry no hostOnly key (it still reaches the provider, in OATS_TEAMS).
      if (team !== null && isObject(byTeam) && isObject(byTeam[team])) { assertNoReservedKey(byTeam[team], `/messaging/byTeam/${team}`); assertNoHostOnlyKey(byTeam[team], `/messaging/byTeam/${team}`, hostOnly, m.name); }
    }
  ...
  return deepFreeze({ ...decl, payloads, payloadOrigins: origins, teams, declRevision, payloadRevision, revision });
}
