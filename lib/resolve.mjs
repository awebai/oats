...
export async function resolveSoul(discovery, soulEntry, { local = null, lock = null, spawn = {}, catalog = null, remote: injected, remoteOptions } = {}) {
  ...
  for (const m of modules) {
    ...
    if (m.layer === "messaging" && isObject(workspace?.messaging)) {
      ...
      if (team !== null && isObject(byTeam) && isObject(byTeam[team])) { assertNoReservedKey(byTeam[team], `/messaging/byTeam/${team}`); committed(byTeam[team], `/messaging/byTeam/${team}`, { kind: "workspace-team", at: `oats-workspace.yaml#/messaging/byTeam/${team}` }); }
    }
  ...
  // bind, so it does not enter either fingerprint.
  return deepFreeze({ ...decl, payloads, payloadOrigins: origins, declRevision, payloadRevision, revision });
}
