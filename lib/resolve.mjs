...
import { bindRemote, packageProviding, readPackageManifests } from "./packages.mjs";
...
const choiceOf = (value, path) => {
  if (value === "off") return "off";
  if (isObject(value) && typeof value.from === "string" && value.from) return { from: value.from };
...
export function composeCapabilities(workspace, soulDefinition, { team = null } = {}) {
  ...
  const apply = (entries, via, path) => {
    for (const [name, value] of Object.entries(entries || {})) {
      const choice = choiceOf(value, `${path}/${name}`);
...
/** The repo ref a locked package is read from: the catalog's url for catalog ids, the key for git refs. */
function packageRef(id, entry, catalog, remote) {
  ...
  if (entry.source === `catalog:${id}`) {
    ...
    throw fail("E_PACKAGE_MISSING", `${id}: locked from the catalog, but no catalog entry (or lock url) says which repo to read it from — pass { catalog } to resolveSoul`, { id, source: entry.source, reason: "no-catalog" });
  }
...
export async function resolveSoul(discovery, soulEntry, { local = null, lock = null, spawn = {}, catalog = null, remote: injected, remoteOptions } = {}) {
  ...
  for (const [cap, range] of Object.entries(isObject(definition.compatibility) ? definition.compatibility : {})) {
    ...
    if (!satisfiesRange(m.from.version, range)) {
      throw fail("E_COMPATIBILITY", `${cap}: package ${m.from.package} is pinned at v${m.from.version}, below the soul's floor ${show(range)} — bump packages.${m.from.package} in the workspace`, { capability: cap, package: m.from.package, version: m.from.version, range, soul: soul.name });
    }
  }
