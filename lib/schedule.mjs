...
function launchSpawn(ws, def, minute, io) {
  ...
  const opts = { task: def.task.trimEnd() + scheduleBlock(def, minute), purpose, harness: def.harness, model: def.model, yolo: def.yolo, backend: def.backend, repo: def.repo, launch: io?.noLaunch === true ? false : true };
  let r;
...
export function tickHost({ now = new Date(), io, dryRun = false } = {}) {
  ...
  const body = () => {
    ...
    for (const ws of wsList) {
      try { considered.push(...tickTriggers(ws, { now, io, dryRun })); }
...
export function runNow(ws, id, { now = new Date(), io, force = false } = {}) {
  const defs = readDefinitions(ws);
  const def = defs.jobs[id];
  if (!def || def.kind === "trigger") throw scheduleError("E_SCHEDULE_UNKNOWN", `no schedule ${JSON.stringify(id)} in ${ws}`);
...
export function describe(ws, id, io, { defs, st, now = new Date() } = {}) {
  ...
  if (def.kind === "trigger") throw scheduleError("E_SCHEDULE_UNKNOWN", `${id} is a trigger, not a schedule — use oats trigger show ${id}`);
...
export function listSchedules(ws, io, { now = new Date() } = {}) {
  ...
  const schedules = Object.keys(defs.jobs).filter((id) => defs.jobs[id]?.kind !== "trigger").sort().map((id) => {
  ...
  const triggers = Object.values(defs.jobs).filter((d) => d?.kind === "trigger").length;
  return { scope: ws, scheduleApi: 2, scheduleHistoryApi: SCHEDULE_HISTORY_API, integrity: scheduleIntegrity(ws), schedules, triggers: { count: triggers, command: "oats trigger list" }, scheduler: schedulerStatus(ws, io) };
}
