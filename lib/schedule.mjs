...
/** Session-start refusals after which nothing was allocated: the slot can
 *  be given back. Any other failure (E_SESSION_START_FAILED, an unexpected
 *  error) may have allocated a runtime: the slot stays until observation of
 *  the start receipt proves it stopped or absent. */
const START_ALLOCATED_NOTHING = new Set(["E_BAD_ARGS", "E_RUNTIME_ENDPOINT_UNKNOWN", "E_RUNTIME_AUTHORITY_MISMATCH", "E_SESSION_START_BUSY", "E_INSTANCE_RETIRING", "E_SESSION_RUNNING", "E_SESSION_UNKNOWN"]);
function performWake(def, io, { startIfStopped = true, canStart = true, reserve, release } = {}) {
  ...
  if (seen.outcome === "stopped") {
    ...
    // The slot is persisted BEFORE the start call: a start that allocates a
    // runtime and then fails (or a process death in between) leaves a held
    // slot that the receipt observation releases once absence is proven.
    ...
    try {
      ...
      if (START_ALLOCATED_NOTHING.has(e.code)) {
        release?.();
      ...
      return { kind: "wake", action: "skipped", reason: `start could not be confirmed (${e.code || "error"}): ${e.message}; the slot is kept until the session is observed stopped or absent`, error: e.message, pending: true, startedRuntime: "unconfirmed" };
    }
  }
...
export function tickWorkspace(ws, { now = new Date(), io, reg, wsList, dryRun = false, only, observeOnly = false, candidates } = {}) {
  ...
  for (const id of order) {
    ...
    if (def.kind === "wake" && js.pendingWake && def.enabled && !dryRun && !only) {
      ...
      const run = performWake(def, io, { startIfStopped: dueNow, canStart, reserve: () => held || acquireJobLock(ws, id, { scheduledFor: js.pendingWake.scheduledFor, wake: true, home: def.home }), release: () => { if (!held) releaseJobLock(ws, id); } });
    ...
    if (def.kind === "wake") {
      ...
      const run = performWake(def, io, { startIfStopped: true, canStart: holdsSlot || liveLocks(wsAll) < bound, reserve: () => holdsSlot || acquireJobLock(ws, id, { scheduledFor: minute.toISOString(), wake: true, home: def.home }), release: () => { if (!holdsSlot) releaseJobLock(ws, id); } });
      if (run.startedRuntime) js.lastLaunchedAt = now.toISOString();
      if (!existsSync(def.home)) releaseJobLock(ws, id);
    ...
    try { run = def.kind === "command" ? launchCommand(ws, def, io) : launchSpawn(ws, def, minute, io); }
      ...
      // unconfirmed as well.
      const incomplete = /INCOMPLETE|quarantin|retain|could not (?:be )?(?:verif|confirm)/i.test(String(e.message));
      run = { kind: def.kind, launched: false, error: e.message, errorCode: e.code, unconfirmed: def.kind !== "spawn" || incomplete };
    }
    if (run.unconfirmed) {
