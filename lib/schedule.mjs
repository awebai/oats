...
export function tickWorkspace(ws, { now = new Date(), io, reg, wsList, dryRun = false, only, observeOnly = false, candidates } = {}) {
  ...
  for (const id of order) {
    ...
    if (def.kind === "wake" && js.pendingWake && def.enabled && !dryRun && !only) {
      ...
      if (run.startedRuntime) { js.lastLaunchedAt = now.toISOString(); if (!jobLockInfo(ws, id)) acquireJobLock(ws, id, { scheduledFor: js.pendingWake.scheduledFor, wake: true, home: def.home }); }
    ...
    if (def.kind === "wake") {
      ...
      if (run.startedRuntime) { js.lastLaunchedAt = now.toISOString(); if (!holdsSlot) acquireJobLock(ws, id, { scheduledFor: minute.toISOString(), wake: true, home: def.home }); }
      if (!existsSync(def.home)) releaseJobLock(ws, id);
...
export function updateSchedule(ws, id, spec, io) {
  ...
  return withScopeLock(ws, () => {
    ...
    // What a run is tracked by cannot change under a running or unresolved
    // job; timing and text can.
    const identity = (d) => JSON.stringify([d.kind, d.agent, d.agentsRoot, d.repo, d.home, d.cwd, d.argv]);
    const busy = !!jobLockInfo(ws, id) || !!readState(ws).jobs[id]?.attempt;
    if (busy && identity(defs.jobs[id]) !== identity(def)) throw scheduleError("E_SCHEDULE_RUNNING", `schedule ${id} is running or has an unresolved attempt; its kind, agent, agentsRoot, repo, home, cwd and argv cannot change until it ends (cron, tz, task, message, purpose, runtime, model and enabled can)`);
