...
export function scheduleExecutionStatus(def, intent, lock) {
  const versioned = def?.definitionVersion !== undefined || def?.recurrencePolicy !== undefined || def?.execution !== undefined;
  let result;
...
export function shapeError(def) {
  ...
  if (def.definitionVersion !== undefined || def.recurrencePolicy !== undefined || def.execution !== undefined) {
    ...
    } else if (def.execution !== undefined) return "prepare-on-tick cannot carry a pinned execution template";
  }
  return null;
}
...
export function validateDefinition(ws, def, { checkAgent = true } = {}) {
  ...
  if (versioned) {
    ...
    } else if (def.execution !== undefined) throw scheduleError("E_SCHEDULE_INVALID", "execution: prepare-on-tick captures only when a future tick is admitted", { field: "execution" });
  }
  return out;
}
...
function admitExecution(def, io, { createIntent = true } = {}) {
  ...
  if (def.recurrencePolicy === "prepare-on-tick") throw scheduleError("migration-required", "prepare-on-tick needs the preparation transaction adapter before this tick can be admitted");
  const execution = validateExecutionTemplate(def.execution), action = capturedDispatchAction(execution);
  if (action.kind === "wake") {
...
export function updateSchedule(ws, id, spec, io) {
  ...
  return withHostLock(() => withScopeLock(ws, () => {
    ...
    if (def.definitionVersion === SCHEDULE_DEFINITION_VERSION) defs.version = 2;
    const identity = (d) => JSON.stringify([d.kind, d.agent, d.agentsRoot, d.repo, d.purpose, d.home, d.cwd, d.argv, d.operation, d.definitionVersion, d.recurrencePolicy, d.execution]);
