...
import { ensureRoot, findAgent, spawnInstance, inspectInstanceSession, inputInstanceSession, startInstanceSession, retirePendingMarkerPath } from "./core.mjs";
...
export function validateDefinition(ws, def, { checkAgent = true } = {}) {
  ...
  if (def.kind === "spawn") {
    ...
    if (def.repo !== undefined) { if (typeof def.repo !== "string" || !def.repo.trim()) throw scheduleError("E_SCHEDULE_INVALID", "repo: path", { field: "repo" }); out.repo = def.repo; }
...
/** The soul a spawn job launches: found in the agents root of the job's
 *  repo (or the workspace) with the same lookup `oats spawn` uses. */
export function resolveScheduledAgent(ws, def) {
  let root;
  try { root = ensureRoot(def.repo ? resolve(ws, def.repo) : ws); }
  catch (e) { throw scheduleError("E_SCHEDULE_INVALID", `agent: ${e.message}`, { field: def.repo ? "repo" : "agent" }); }
...
function launchSpawn(ws, def, minute, io) {
  ...
  const opts = { task: def.task.trimEnd() + scheduleBlock(def, minute), purpose, runtime: def.runtime, model: def.model, yolo: def.yolo, backend: def.backend, launch: true };
...
/** One wake: start the home when it is not running, deliver when it is active. */
function performWake(def, io) {
  const seen = observeHome(def.home, io);
  if (seen.outcome === "ended") return { kind: "wake", action: "skipped", reason: "home is gone or retiring" };
  if (seen.outcome === "unknown") return { kind: "wake", action: "skipped", reason: `cannot observe the session: ${seen.error}` };
  if (seen.outcome === "stopped") {
    try {
      const r = (io?.start || startInstanceSession)(def.home);
      return { kind: "wake", action: "started", instance: r.instance, target: r.target, reason: "session was stopped; message not delivered this minute" };
    } catch (e) {
      if (e.code === "E_SESSION_START_BUSY") return { kind: "wake", action: "skipped", reason: "session is starting" };
      return { kind: "wake", action: "skipped", reason: `start refused: ${e.code || ""} ${e.message}`.trim(), error: e.message };
    }
  }
  try { (io?.input || inputInstanceSession)(def.home, def.message); return { kind: "wake", action: "delivered" }; }
  catch (e) { return { kind: "wake", action: "skipped", reason: `input refused: ${e.message}` }; }
}
...
export function tickWorkspace(ws, { now = new Date(), io, reg, wsList, dryRun = false, only } = {}) {
  ...
  for (const [id, def] of Object.entries(defs.jobs)) {
    ...
    record(def.kind === "wake" ? run.action : run.launched ? "launched" : "launch-failed", { ...(run.reason ? { reason: run.reason } : {}), ...(run.error ? { error: run.error } : {}), ...(run.instance ? { instance: run.instance } : {}) });
  }
  if (!dryRun) writeState(ws, st);
  return considered;
}
...
export function describe(ws, id, io, { defs, st, now = new Date() } = {}) {
  ...
  if (!def) throw scheduleError("E_SCHEDULE_UNKNOWN", `no schedule ${JSON.stringify(id)} in ${ws}`);
  const js = st.jobs[id] || {};
  return { ...def, nextRun: def.enabled ? nextRunAfter(def, now) : null, lastRun: js.lastRun || null, running: !!jobLockInfo(ws, id), ...(js.attempt ? { attempt: js.attempt } : {}) };
}
