...
import { RECURRENCE_POLICIES, SCHEDULE_ATTEMPT_VERSION, SCHEDULE_DEFINITION_VERSION, admitExecutionCapture, buildCommandExecutionCapture, capturedDispatchAction, validateExecutionCapsule, validateExecutionCapture } from "./schedule-capsule.mjs";
...
export function scheduleExecutionStatus(def, attempt, lock) {
  const versioned = def?.definitionVersion !== undefined || def?.recurrencePolicy !== undefined || def?.capturedExecution !== undefined || def?.execution !== undefined;
  let result;
  if (!versioned) result = { kind: "legacy", capture: "unknown", migrationRequired: true };
    ...
    else result = { kind: "captured", capture: "recorded", migrationRequired: false, schemaVersion: def.capturedExecution.schemaVersion, capsuleIntegrity: def.capturedExecution.capsuleIntegrity, resolution: def.capturedExecution.resolution };
  }
  if (!attempt) return result;
  try {
    const execution = validateAttemptExecution(attempt, lock);
    return { ...result, attempt: execution ? { kind: "captured", capture: "recorded", migrationRequired: false, schemaVersion: attempt.schemaVersion, executionId: execution.executionId, resolution: execution.resolution }
      : { kind: "legacy", capture: "unknown", migrationRequired: true } };
  } catch (error) {
    return { ...result, attempt: { kind: "invalid", capture: "unknown", migrationRequired: true, reason: error.message, errorCode: error.code } };
  }
}
...
export function shapeError(def) {
  ...
  if (def.definitionVersion !== undefined || def.recurrencePolicy !== undefined || def.capturedExecution !== undefined || def.execution !== undefined) {
    if (def.execution !== undefined) return "definition executionId is obsolete: definitions store capturedExecution content, attempts store execution intent";
    ...
    if (def.kind !== "command") return "versioned execution currently supports command definitions only";
    if (def.recurrencePolicy === "capture") {
      try {
        validateExecutionCapture(def.capturedExecution);
        const expected = buildCommandExecutionCapture({ cwd: def.cwd, argv: def.argv, inputRefs: def.capturedExecution.inputRefs, responsibleHuman: def.capturedExecution.responsibleHuman });
        if (expected.capsuleIntegrity.value !== def.capturedExecution.capsuleIntegrity.value) return "captured execution differs from the saved command target";
      } catch (error) { return `captured execution is invalid: ${error.message}`; }
    } else if (def.capturedExecution !== undefined) return "prepare-on-tick cannot carry previously captured content";
  }
  return null;
}
...
export function validateDefinition(ws, def, { checkAgent = true } = {}) {
  ...
  const versioned = def.definitionVersion !== undefined || def.recurrencePolicy !== undefined || def.capturedExecution !== undefined || def.execution !== undefined;
  if (versioned) {
    if (def.execution !== undefined) throw scheduleError("migration-required", "definition executionId is obsolete: definitions store capturedExecution content and attempts mint execution intent", { field: "execution" });
    ...
    if (def.kind !== "command") throw scheduleError("E_SCHEDULE_INVALID", "versioned execution currently supports command jobs through the exact captured CLI; spawn, operation and wake await their captured consumer adapters", { field: "recurrencePolicy" });
    out.definitionVersion = SCHEDULE_DEFINITION_VERSION;
    out.recurrencePolicy = def.recurrencePolicy;
    if (def.recurrencePolicy === "capture") {
      let capturedExecution;
      try {
        capturedExecution = buildCommandExecutionCapture({ cwd: out.cwd, argv: out.argv,
          inputRefs: def.inputRefs ?? def.capturedExecution?.inputRefs ?? {},
          responsibleHuman: def.responsibleHuman ?? def.capturedExecution?.responsibleHuman ?? null });
        if (def.capturedExecution !== undefined) {
          validateExecutionCapture(def.capturedExecution);
          if (def.capturedExecution.capsuleIntegrity.value !== capturedExecution.capsuleIntegrity.value) throw new Error("capture differs from cwd, argv, inputs or responsible human");
        }
      } catch (error) { throw scheduleError("E_SCHEDULE_INVALID", `capturedExecution: ${error.message}`, { field: "capturedExecution" }); }
      out.capturedExecution = capturedExecution;
    } else if (def.capturedExecution !== undefined) throw scheduleError("E_SCHEDULE_INVALID", "capturedExecution: prepare-on-tick captures only when a future tick is admitted", { field: "capturedExecution" });
  }
  return out;
}
...
function admitExecution(def, io, { createIntent = true } = {}) {
  ...
  const captured = validateExecutionCapture(def.capturedExecution), action = capturedDispatchAction(captured);
  const request = { deployment: captured.deployment, resolution: captured.resolution, action };
  (io?.admit || loadCapturedDispatch)(request);
  return createIntent ? admitExecutionCapture(captured) : undefined;
}
...
export function describe(ws, id, io, { defs, st, now = new Date() } = {}) {
  ...
  const lock = jobLockInfo(ws, id);
  return { ...def, executionStatus: scheduleExecutionStatus(def, js.attempt, lock), nextRun, lastRun: js.lastRun || null, running: !!lock, ...(js.attempt ? { attempt: js.attempt } : {}), ...(js.pendingWake ? { pendingWake: js.pendingWake } : {}) };
}
...
export function updateSchedule(ws, id, spec, io) {
  ...
  return withHostLock(() => withScopeLock(ws, () => {
    ...
    if (def.definitionVersion === SCHEDULE_DEFINITION_VERSION) defs.version = 2;
    const identity = (d) => JSON.stringify([d.kind, d.agent, d.agentsRoot, d.repo, d.purpose, d.home, d.cwd, d.argv, d.operation, d.definitionVersion, d.recurrencePolicy, d.capturedExecution, d.execution]);
