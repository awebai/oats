...
export function scheduleExecutionStatus(def) {
  const versioned = def?.definitionVersion !== undefined || def?.recurrencePolicy !== undefined || def?.execution !== undefined;
  if (!versioned) return { kind: "legacy", capture: "unknown", migrationRequired: true };
  const invalid = shapeError(def);
  if (invalid) return { kind: "invalid", capture: "unknown", migrationRequired: true, reason: invalid };
  if (def.recurrencePolicy === "prepare-on-tick") return { kind: "prepare-on-tick", capture: "not-captured", migrationRequired: true, reason: "preparation adapter unavailable" };
  return { kind: "captured", capture: "recorded", migrationRequired: false, schemaVersion: def.execution.schemaVersion, executionId: def.execution.executionId, resolution: def.execution.resolution };
}
...
export function shapeError(def) {
  ...
  if (def.definitionVersion !== undefined || def.recurrencePolicy !== undefined || def.execution !== undefined) {
    ...
    if (def.recurrencePolicy === "capture") {
      if (def.kind !== "command") return "capture policy currently supports command definitions only";
      try {
...
export function validateDefinition(ws, def, { checkAgent = true } = {}) {
  ...
  if (versioned) {
    ...
    if (def.recurrencePolicy === "capture") {
      if (def.kind !== "command") throw scheduleError("E_SCHEDULE_INVALID", "recurrencePolicy capture currently supports command jobs through the exact captured CLI; spawn, operation and wake await their captured consumer adapters", { field: "recurrencePolicy" });
      let execution;
      try {
...
export function describe(ws, id, io, { defs, st, now = new Date() } = {}) {
  ...
  return { ...def, executionStatus: scheduleExecutionStatus(def), nextRun, lastRun: js.lastRun || null, running: !!jobLockInfo(ws, id), ...(js.attempt ? { attempt: js.attempt } : {}), ...(js.pendingWake ? { pendingWake: js.pendingWake } : {}) };
}
