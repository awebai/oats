...
export function scheduleKind({ dep = null } = {}) {
  return {
    ...
    bodyKeys: ["run", "cron", "tz", "agent", "task", "purpose", "harness", "model", "yolo", "backend", "wake", "argv", "cwd"],
    parseBody: (body) => {
...
function launchSpawn(ws, def, minute, io) {
  ...
  const opts = { discover, task: def.task.trimEnd() + scheduleBlock(def, minute), purpose, harness: def.harness, model: def.model, yolo: def.yolo, backend: def.backend, repo: def.repo, launch: io?.noLaunch === true ? false : true };
  let r;
...
export function scopeAutomations(ws, { io, now = new Date(), refresh = false, cache = new Map() } = {}) {
  ...
  return { ws, local, ...automationContext(ws, { local, io: { ...(io || {}), childEnv }, now, refresh, cache }) };
}
...
export function describe(ws, qid, io, { defs, st, now = new Date(), ctx = null } = {}) {
  ...
  return scheduleRow(ws, entry ?? localEntry("schedule", { ...def, id: name }), base, ctx);
}
...
function scheduleRow(ws, entry, base, ctx) {
  ...
  return {
    ...
    teams: [], harness: def.harness ?? null, model: def.model ?? null, concurrency: null,
 ...
 *  disabled on this host through oats-local.yaml automations.disabled). */
