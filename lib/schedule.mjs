/** OATS schedules: legacy v1 plus explicit captured-execution v2,
 * workspace-scoped definitions and host-owned execution.
 *
...
import { RESERVED_LAUNCH_ENV, MAX_INSTANCE_NAME, findAgent, findInstanceHomes, inspectInstanceSession, inputInstanceSession, startInstanceSession, retirePendingMarkerPath, loadCapturedDispatch, prepareCapturedComposition } from "./core.mjs";
...
export function scheduleExecutionStatus(def, intent, lock) {
  const versioned = def?.definitionVersion !== undefined || def?.recurrencePolicy !== undefined || def?.execution !== undefined || def?.preparation !== undefined;
  let result;
  if (!versioned) result = { kind: "legacy", capture: "unknown", migrationRequired: true };
  else {
    const invalid = shapeError(def);
    if (invalid) result = { kind: "invalid", capture: "unknown", migrationRequired: true, reason: invalid };
    else if (def.recurrencePolicy === "prepare-on-tick") result = { kind: "prepare-on-tick", capture: "not-captured", migrationRequired: false, reason: "explicit preparation runs at admission" };
    else result = { kind: "captured", capture: "recorded", migrationRequired: false, schemaVersion: def.execution.schemaVersion, contentIntegrity: executionContentIntegrity(def.execution), resolution: def.execution.resolution };
  }
  if (!intent) return result;
  try {
    const execution = validateAttemptExecution(intent, lock);
    return { ...result, intent: execution ? { kind: "captured", capture: "recorded", migrationRequired: false, schemaVersion: intent.schemaVersion, executionId: execution.executionId, resolution: execution.resolution }
      : { kind: "legacy", capture: "unknown", migrationRequired: true } };
  } catch (error) {
...
export function shapeError(def) {
  ...
  if (def.definitionVersion !== undefined || def.recurrencePolicy !== undefined || def.execution !== undefined || def.preparation !== undefined) {
    if (def.definitionVersion !== SCHEDULE_DEFINITION_VERSION) return `definitionVersion must be ${SCHEDULE_DEFINITION_VERSION}`;
    if (!RECURRENCE_POLICIES.includes(def.recurrencePolicy)) return "recurrencePolicy must be capture or prepare-on-tick";
    if (!["command", "wake"].includes(def.kind)) return "versioned execution currently supports command and captured wake definitions only";
    if (def.kind === "wake" && def.recurrencePolicy !== "capture") return "wake definitions require capture policy";
    if (def.recurrencePolicy === "capture") {
      if (def.preparation !== undefined) return "capture policy cannot carry prepare-on-tick input";
      try {
        validateExecutionTemplate(def.execution);
        const expected = def.kind === "command"
          ? buildCommandExecutionTemplate({ cwd: def.cwd, argv: def.argv, inputRefs: def.execution.inputRefs, responsibleHuman: def.execution.responsibleHuman })
          : buildWakeExecutionTemplate({ home: def.home, message: def.message,
            executionBinding: { schemaVersion: 1, deployment: def.execution.deployment, resolution: def.execution.resolution },
            inputRefs: def.execution.inputRefs, responsibleHuman: def.execution.responsibleHuman });
        if (executionContentIntegrity(expected).value !== executionContentIntegrity(def.execution).value) return "execution template differs from the saved target";
      } catch (error) { return `execution template is invalid: ${error.message}`; }
    } else {
      if (def.execution !== undefined) return "prepare-on-tick cannot carry a pinned execution template";
      try { validatePreparationInput(def.preparation); } catch (error) { return `preparation input is invalid: ${error.message}`; }
      try { commandArgvWithExecutionBinding(def.argv, { schemaVersion: 1, deployment: def.preparation.deployment, resolution: { schemaVersion: 1, id: `sha256-${"0".repeat(64)}` } }); }
      catch (error) { return `prepare-on-tick command is invalid: ${error.message}`; }
    }
  }
  return null;
}
...
export function validateDefinition(ws, def, { checkAgent = true } = {}) {
  ...
  const versioned = def.definitionVersion !== undefined || def.recurrencePolicy !== undefined || def.execution !== undefined || def.preparation !== undefined;
  if (versioned) {
    if (def.definitionVersion !== SCHEDULE_DEFINITION_VERSION) throw scheduleError("E_SCHEDULE_INVALID", `definitionVersion: ${SCHEDULE_DEFINITION_VERSION} required for captured schedules`, { field: "definitionVersion" });
    if (!RECURRENCE_POLICIES.includes(def.recurrencePolicy)) throw scheduleError("E_SCHEDULE_INVALID", "recurrencePolicy: capture or prepare-on-tick", { field: "recurrencePolicy" });
    if (!["command", "wake"].includes(def.kind)) throw scheduleError("E_SCHEDULE_INVALID", "versioned execution currently supports command and captured wake jobs; spawn and operation await their captured consumer adapters", { field: "recurrencePolicy" });
    if (def.kind === "wake" && def.recurrencePolicy !== "capture") throw scheduleError("E_SCHEDULE_INVALID", "versioned wake jobs require capture policy", { field: "recurrencePolicy" });
    out.definitionVersion = SCHEDULE_DEFINITION_VERSION;
    out.recurrencePolicy = def.recurrencePolicy;
    if (def.recurrencePolicy === "capture") {
      if (def.preparation !== undefined) throw scheduleError("E_SCHEDULE_INVALID", "preparation belongs only to prepare-on-tick", { field: "preparation" });
      let execution;
      try {
        const responsibleHuman = Object.hasOwn(def, "responsibleHuman") ? def.responsibleHuman : def.execution?.responsibleHuman;
        if (responsibleHuman === undefined) throw new Error("responsibleHuman must be explicit for captured execution (null means messaging disabled)");
        if (def.kind === "command") execution = buildCommandExecutionTemplate({ cwd: out.cwd, argv: out.argv,
          inputRefs: def.inputRefs ?? def.execution?.inputRefs ?? {}, responsibleHuman });
        else {
          const meta = readJson(join(out.home, "instance.json"), null);
          if (!meta?.executionBinding) throw scheduleError("migration-required", "wake target has no captured executionBinding", { field: "executionBinding" });
          validateExecutionBinding(meta.executionBinding);
          execution = buildWakeExecutionTemplate({ home: out.home, message: out.message, executionBinding: meta.executionBinding,
            inputRefs: def.inputRefs ?? def.execution?.inputRefs ?? {}, responsibleHuman });
        }
        if (def.execution !== undefined) {
          validateExecutionTemplate(def.execution);
          if (executionContentIntegrity(def.execution).value !== executionContentIntegrity(execution).value) throw new Error("template differs from target, binding, inputs or responsible human");
        }
      } catch (error) {
        if (error.code === "migration-required") throw error;
        throw scheduleError("E_SCHEDULE_INVALID", `execution: ${error.message}`, { field: "execution" });
      }
      out.execution = execution;
    } else {
      if (def.execution !== undefined) throw scheduleError("E_SCHEDULE_INVALID", "execution: prepare-on-tick captures only when a future tick is admitted", { field: "execution" });
      try {
        out.preparation = validatePreparationInput(def.preparation);
        commandArgvWithExecutionBinding(out.argv, { schemaVersion: 1, deployment: out.preparation.deployment, resolution: { schemaVersion: 1, id: `sha256-${"0".repeat(64)}` } });
      } catch (error) { throw scheduleError("E_SCHEDULE_INVALID", `preparation: ${error.message}`, { field: "preparation" }); }
    }
  }
  return out;
}
 ...
 * before the scheduler acquires a slot or invokes a child. Legacy definitions
 * retain their old release boundary; prepare-on-tick remains fail-closed until
 * the preparation owner supplies its transaction API. */
function homeExecutionBinding(home) {
  const meta = readJson(join(home, "instance.json"), null);
  if (!meta?.executionBinding) throw scheduleError("migration-required", "wake target has no captured executionBinding");
  try { return validateExecutionBinding(meta.executionBinding); }
  catch (error) { throw scheduleError("E_SCHEDULE_INVALID", `wake target executionBinding is invalid: ${error.message}`); }
}
function validateAttemptExecution(attempt, lock) {
  if (attempt?.execution === undefined) {
    if (attempt?.schemaVersion !== undefined) throw scheduleError("migration-required", "versioned scheduled attempt has no captured execution");
    if (lock?.executionId !== undefined) throw scheduleError("E_SCHEDULE_INVALID", "captured job lock has no matching admitted capsule");
    return undefined;
  }
  if (attempt.schemaVersion !== SCHEDULE_ATTEMPT_VERSION) throw scheduleError("migration-required", "scheduled attempt uses an unsupported version");
  const execution = validateExecutionCapsule(attempt.execution);
  if (lock && lock.executionId !== execution.executionId) throw scheduleError("E_SCHEDULE_INVALID", "job lock does not name the admitted execution intent");
  return execution;
}
function validateActiveExecution(js, lock) {
  ...
  if (lock && (js.lastRun?.execution || lock.executionId !== undefined)) {
    return validateAttemptExecution(js.lastRun?.execution ? { schemaVersion: SCHEDULE_ATTEMPT_VERSION, execution: js.lastRun.execution } : {}, lock);
  }
}
function receiptMatchesExecution(receipt, execution) {
  return !!receipt?.execution && receipt.execution.executionId === execution.executionId
    && executionContentIntegrity(validateExecutionCapsule(receipt.execution)).value === executionContentIntegrity(execution).value;
}
function admitExecution(def, io, { createIntent = true } = {}) {
  if (def.definitionVersion === undefined) return undefined;
  if (def.definitionVersion !== SCHEDULE_DEFINITION_VERSION) throw scheduleError("migration-required", "scheduled definition uses an unsupported execution version");
  let execution;
  if (def.recurrencePolicy === "prepare-on-tick") {
    if (!createIntent) return undefined; // dry-run never prepares or publishes
    const prepare = io && Object.hasOwn(io, "prepare") ? io.prepare : prepareCapturedComposition;
    if (typeof prepare !== "function") throw scheduleError("migration-required", "prepare-on-tick needs the preparation transaction adapter before this tick can be admitted");
    const request = validatePreparationInput(def.preparation);
    const requestedDeployment = portableScope(request.deployment); // Freeze identity before preparation, including platform path aliases.
    const prepared = prepare(request);
    if (!prepared || typeof prepared !== "object" || !prepared.executionBinding || !Object.hasOwn(prepared, "responsibleHuman")) {
      throw scheduleError("needs-configuration", "preparation did not produce a complete executionBinding and responsibleHuman", { problems: prepared?.problems || [] });
    }
    const binding = validateExecutionBinding(prepared.executionBinding);
    let sameDeployment = false;
    try { sameDeployment = portableScope(binding.deployment) === requestedDeployment && portableScope(request.deployment) === requestedDeployment; } catch { /* absent/changed scope is not the admitted deployment */ }
    if (!sameDeployment) throw scheduleError("E_SCHEDULE_INVALID", "prepared executionBinding changed the requested deployment");
    binding.deployment = requestedDeployment; // Persist canonical authority, not a retargetable spelling.
    if (prepared.resolution && prepared.resolution.id !== binding.resolution.id) throw scheduleError("E_SCHEDULE_INVALID", "prepared resolution differs from executionBinding");
    const argv = commandArgvWithExecutionBinding(def.argv, binding);
    execution = buildCommandExecutionTemplate({ cwd: def.cwd, argv, inputRefs: prepared.inputRefs ?? {}, responsibleHuman: prepared.responsibleHuman });
  } else execution = validateExecutionTemplate(def.execution);
  const action = capturedDispatchAction(execution);
  if (action.kind === "wake") {
    const binding = homeExecutionBinding(execution.target.home);
    if (binding.deployment !== execution.deployment || binding.resolution.id !== execution.resolution.id) throw scheduleError("E_SCHEDULE_INVALID", "wake target executionBinding differs from its captured template");
  }
  const request = { deployment: execution.deployment, resolution: execution.resolution, action };
  (io?.admit || loadCapturedDispatch)(request);
  if (action.kind === "wake") throw scheduleError("migration-required", "captured wake lifecycle dispatch is not integrated yet");
  return createIntent ? admitExecutionTemplate(execution) : undefined;
}
...
function observePrevious(ws, id, def, js, io, now, { mutate }) {
  ...
  if (js.attempt || shapeError(def)) return; // Unconfirmed effects need reconcile.
  const lr = js.lastRun;
...
export function tickWorkspace(ws, { now = new Date(), io, reg, wsList, dryRun = false, only, observeOnly = false, candidates } = {}) {
  ...
  for (const id of order) {
    ...
    const js = stateOf(id);
    validateActiveExecution(js, jobLockInfo(ws, id)); // Also covers host pre-observed candidates.
    let due, cronError;
    ...
    // A captured wake is bound to the target home's exact executionBinding,
    // but remains gated until lifecycle exposes captured wake dispatch. Keep
    // this before both pending delivery and due-start branches so neither can
    // fall through to the legacy home path.
    const versionedWakeDue = def.kind === "wake" && def.definitionVersion !== undefined && !dryRun
      && (def.enabled || only) && ((js.pendingWake && !only) || only || (due && js.lastAttemptedMinute !== key));
    if (versionedWakeDue) {
      let error;
      try { admitExecution(def, io, { createIntent: false }); } catch (caught) { error = caught; }
      if (error) {
        if (only || due) js.lastAttemptedMinute = key;
        js.lastRun = { scheduledFor: minute.toISOString(), startedAt: now.toISOString(), kind: "wake", launched: false, outcome: "blocked", error: error.message, errorCode: error.code || "E_SCHEDULE_ADMISSION" };
        record(id, "blocked", { error: error.message, errorCode: error.code || "E_SCHEDULE_ADMISSION" });
        continue;
      }
      throw scheduleError("migration-required", "captured wake unexpectedly passed its lifecycle integration gate");
    }
    ...
    if (dryRun) {
      let admission;
      if (!js.attempt && !jobLockInfo(ws, id) && (def.kind === "wake" || liveLocks(wsAll) < bound)) {
        try { admitExecution(def, io, { createIntent: false }); } catch (error) { admission = error; }
      }
      const reason = js.attempt ? "an earlier launch has no recorded result; run oats schedule reconcile" : jobLockInfo(ws, id) && def.kind !== "wake" ? "still running" : liveLocks(wsAll) >= bound && def.kind !== "wake" ? "host busy" : admission ? `admission blocked (${admission.code || "error"}): ${admission.message}` : undefined;
      record(id, reason ? (admission ? "blocked" : "skipped") : "due", reason ? { reason, ...(admission ? { errorCode: admission.code } : {}) } : {});
      continue;
    }
    ...
    if (js.attempt) {
      ...
      if (!js.attempt.execution || receiptMatchesExecution(js.lastRun, js.attempt.execution)) {
    ...
    let execution;
    try { execution = admitExecution(def, io); }
    catch (error) {
      js.lastRun = { scheduledFor: minute.toISOString(), startedAt: now.toISOString(), kind: def.kind, launched: false, outcome: "blocked", error: error.message, errorCode: error.code || "E_SCHEDULE_ADMISSION" };
      record(id, "blocked", { error: error.message, errorCode: error.code || "E_SCHEDULE_ADMISSION" });
      continue;
    }
    // Persist the admitted authority before even reserving the launch slot.
    // A crash in the following gap leaves an inspectable unresolved attempt,
    // never an ownerless lock whose intended software cannot be recovered.
    js.attempt = { ...(execution ? { schemaVersion: SCHEDULE_ATTEMPT_VERSION } : {}), scheduledFor: minute.toISOString(), startedAt: now.toISOString(), wallClock: new Date().toISOString(), ...(execution ? { execution } : {}) };
    writeState(ws, st);
    if (!acquireJobLock(ws, id, { scheduledFor: minute.toISOString(), ...(execution ? { executionId: execution.executionId } : {}) })) {
      delete js.attempt; writeState(ws, st); record(id, "skipped", { reason: "still running" }); continue;
    }
    js.lastLaunchedAt = now.toISOString();
    writeState(ws, st);
    let run;
    const admitted = execution ? { ...def, cwd: execution.target.cwd, argv: execution.target.argv } : def;
    try { run = def.kind === "spawn" ? launchSpawn(ws, def, minute, io) : launchCommand(ws, def.kind === "operation" ? operationAsCommand(def) : admitted, io); }
    catch (e) {
    ...
    if (run.unconfirmed) {
      ...
      js.lastRun = { scheduledFor: minute.toISOString(), startedAt: now.toISOString(), ...(execution ? { execution } : {}), ...run, outcome: "unknown" };
      record(id, "unknown", { error: run.error });
      continue;
    }
    delete js.attempt;
    const outcome = run.launched ? "launched" : "launch-failed";
    js.lastRun = { scheduledFor: minute.toISOString(), startedAt: now.toISOString(), ...(execution ? { execution } : {}), ...run, outcome };
...
export function reconcile(ws, id, { io, now = new Date(), clear = false } = {}) {
  ...
  return withHostLock(() => {
    const st = readState(ws);
    const js = st.jobs[id] || (st.jobs[id] = {});
    const attempt = js.attempt || (js.lastRun?.outcome === "unknown" ? { ...(js.lastRun.execution ? { schemaVersion: SCHEDULE_ATTEMPT_VERSION } : {}), scheduledFor: js.lastRun.scheduledFor, startedAt: js.lastRun.startedAt, ...(js.lastRun.execution ? { execution: js.lastRun.execution } : {}) } : undefined);
    ...
    catch (error) { throw scheduleError("E_SCHEDULE_INVALID", `admitted execution capsule is invalid: ${error.message}`); }
    const attemptKind = attempt.execution?.action.kind || def.kind;
    ...
    if (attemptKind === "spawn" && attempt.scheduledFor) {
      ...
      if (attempt.execution) {
        // Only this admission's receipt may establish effects. Never borrow the
        // previous run's name or consult current-config roster discovery.
        if (receiptMatchesExecution(js.lastRun, attempt.execution) && typeof js.lastRun.home === "string" && isAbsolute(js.lastRun.home)) {
          const home = resolve(js.lastRun.home), meta = readJson(join(home, "instance.json"), null);
          if (meta?.instance === js.lastRun.instance && typeof meta.home === "string" && resolve(meta.home) === home) named = [{ instance: meta.instance, home }];
        }
      } else if (js.lastRun?.instance) named = findHomesInScope(ws, js.lastRun.instance).map((home) => ({ instance: js.lastRun.instance, home }));
      ...
      else if (!clear) return { schedule: describe(ws, id, io), reconciled: "unknown", remedy: `the command's effects cannot be proven from a matching admitted receipt; check the roster and the host by hand, then run oats schedule reconcile ${id} --clear to record launch-failed and free the slot, or oats schedule remove --force ${id}` };
    }
    ...
    if (adopted) {
      ...
      js.lastRun = { scheduledFor: attempt.scheduledFor, startedAt: js.lastRun?.startedAt || attempt.startedAt, kind: attemptKind, ...(attempt.execution ? { execution: attempt.execution } : {}), launched: true, ...adopted, outcome: seen.outcome, ...(seen.error ? { error: seen.error } : {}) };
      if (seen.outcome !== "ended") { if (!jobLockInfo(ws, id)) acquireJobLock(ws, id, { scheduledFor: attempt.scheduledFor, reconciled: true, ...(attempt.execution ? { executionId: attempt.execution.executionId } : {}) }); } else releaseJobLock(ws, id);
    } else {
      js.lastRun = { scheduledFor: attempt.scheduledFor, startedAt: js.lastRun?.startedAt || attempt.startedAt, kind: attemptKind, ...(attempt.execution ? { execution: attempt.execution } : {}), launched: false, outcome: "launch-failed", error: clear ? "cleared by the operator after checking by hand" : "no instance found in the scope's roster for the unrecorded attempt" };
      releaseJobLock(ws, id);
    }
    writeState(ws, st);
...
export function addSchedule(ws, spec, io) {
  ...
  return withScopeLock(ws, () => {
    ...
    if (defs.version === 2 && def.definitionVersion === undefined) throw scheduleError("migration-required", "version 2 schedule files require an explicit recurrence policy for new definitions");
    if (def.definitionVersion === SCHEDULE_DEFINITION_VERSION) defs.version = 2;
...
export function updateSchedule(ws, id, spec, io) {
  ...
  return withHostLock(() => withScopeLock(ws, () => {
    ...
    if (defs.jobs[id].definitionVersion !== undefined && def.definitionVersion === undefined) throw scheduleError("migration-required", "a versioned schedule definition cannot be downgraded to legacy execution");
    if (def.definitionVersion === SCHEDULE_DEFINITION_VERSION) defs.version = 2;
...
export function saveWakeForHome(ws, { instance, home, wake, executionBinding, responsibleHuman }) {
  ...
  if (executionBinding !== undefined || responsibleHuman !== undefined) {
    if (executionBinding === undefined || responsibleHuman === undefined) throw scheduleError("needs-configuration", "captured wake creation needs both executionBinding and responsibleHuman");
    const binding = validateExecutionBinding(executionBinding), actual = homeExecutionBinding(home);
    if (binding.deployment !== actual.deployment || binding.resolution.id !== actual.resolution.id) throw scheduleError("E_SCHEDULE_INVALID", "spawn result executionBinding differs from the new home");
    Object.assign(spec, { definitionVersion: SCHEDULE_DEFINITION_VERSION, recurrencePolicy: "capture", responsibleHuman });
  }
  return addSchedule(ws, spec); // Same whole-file version/policy gate as manual add.
}
