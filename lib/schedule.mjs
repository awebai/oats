...
export function registerWorkspace(ws) { const reg = readRegistry(); const abs = resolve(ws); if (!reg.workspaces.includes(abs)) { reg.workspaces.push(abs); writeRegistry(reg); } return reg; }
export function unregisterWorkspace(ws) { const reg = readRegistry(); reg.workspaces = reg.workspaces.filter((w) => w !== resolve(ws)); writeRegistry(reg); return reg; }
...
export function withHostLock(fn) { return withDirLock(join(hostScheduleDir(), "host.lock"), "the host scheduler", fn); }
...
export function observeHome(home, io) {
  ...
  try {
    ...
    return retiring ? { outcome: "active", note: "retiring: home present, teardown pending" } : { outcome: "stopped" };
...
/** Homes anywhere in the scope's roster created after `since`. */
function homesCreatedAfter(ws, since) {
  const out = [];
  const t = new Date(since).getTime();
  for (const root of scopeRoots(ws)) {
    for (const base of [root, join(dirname(root), "local-agents")]) {
      if (!existsSync(base)) continue;
      for (const a of readdirSync(base, { withFileTypes: true })) {
        if (!a.isDirectory() || a.name.startsWith(".")) continue;
        const inst = join(base, a.name, "instances");
        if (!existsSync(inst)) continue;
        for (const i of readdirSync(inst, { withFileTypes: true })) {
          if (!i.isDirectory() || i.name.startsWith(".")) continue;
          const home = join(inst, i.name);
          try { if (existsSync(join(home, "instance.json")) && statSync(join(home, "instance.json")).mtimeMs >= t) out.push({ instance: i.name, home }); } catch { /* skip */ }
        }
      }
    }
  }
...
function launchCommand(ws, def, io) {
  ...
  if (io?.command) envelope = io.command({ cwd: def.cwd, argv });
    ...
    if (!envelope && !timedOut) envelope = { ok: false, error: { code: r.status === null ? "E_SIGNAL" : "E_ENVELOPE", message: (String(r.stderr || "").trim() || `exit ${r.status}`).slice(0, 400) } };
  }
  if (timedOut || !envelope) return { kind: "command", launched: false, unconfirmed: true, error: timedOut ? "command timed out; its side effects are unconfirmed" : "command answered no envelope; its side effects are unconfirmed" };
...
function performWake(def, io, { startIfStopped = true, canStart = true } = {}) {
...
/** Observe the previous run of one job. A wake job is observed whenever it
 *  holds a slot (its started runtime), whatever its last action was. */
function observePrevious(ws, id, def, js, io, now, { mutate }) {
  const lr = js.lastRun;
  const home = def.kind === "wake" ? def.home : lr?.home;
  const tracked = def.kind === "wake" ? !!jobLockInfo(ws, id) : !!home && ["launched", "active", "stopped", "unknown"].includes(lr?.outcome);
  if (!tracked || !home) return;
  const seen = observeHome(home, io);
  if (!mutate) return;
  if (def.kind === "wake") {
    if (seen.outcome === "ended") { releaseJobLock(ws, id); delete js.pendingWake; if (lr) js.lastRun = { ...lr, endedAt: now.toISOString(), note: "started runtime ended" }; }
    return;
  }
  js.lastRun = { ...lr, outcome: seen.outcome, ...(seen.error ? { error: seen.error } : {}), ...(seen.outcome === "ended" ? { endedAt: now.toISOString() } : {}) };
...
export function tickWorkspace(ws, { now = new Date(), io, reg, wsList, dryRun = false, only } = {}) {
  ...
  for (const [id, def] of Object.entries(defs.jobs)) observePrevious(ws, id, def, stateOf(id), io, now, { mutate: !dryRun });
  const order = Object.keys(defs.jobs).filter((id) => !only || id === only).sort((a, b) => String(st.jobs[a]?.lastRun?.startedAt || "") .localeCompare(String(st.jobs[b]?.lastRun?.startedAt || "")) || a.localeCompare(b));
  for (const id of order) {
    const def = defs.jobs[id];
    const js = stateOf(id);
    let due, cronError;
    try { due = only ? true : dueAt(def, minute); } catch (e) { cronError = e.message; due = false; }
    ...
    if (def.kind === "wake" && js.pendingWake && def.enabled && !dryRun && !only) {
      ...
      const canStart = !!jobLockInfo(ws, id) || liveLocks(wsAll) < bound;
      const run = performWake(def, io, { startIfStopped: dueNow, canStart });
      if (run.startedRuntime && !jobLockInfo(ws, id)) acquireJobLock(ws, id, { scheduledFor: js.pendingWake.scheduledFor, wake: true });
    ...
    if (def.kind === "wake") {
      ...
      const run = performWake(def, io, { startIfStopped: true, canStart: holdsSlot || liveLocks(wsAll) < bound });
      if (run.startedRuntime && !holdsSlot) acquireJobLock(ws, id, { scheduledFor: minute.toISOString(), wake: true });
      if (!existsSync(def.home)) releaseJobLock(ws, id);
    ...
    try { run = def.kind === "command" ? launchCommand(ws, def, io) : launchSpawn(ws, def, minute, io); }
      ...
      // spawnInstance compensates its own failures (no instance survives a
      // thrown spawn); a launch that threw for any other reason after a
      // process could have started is unconfirmed.
      run = { kind: def.kind, launched: false, error: e.message, errorCode: e.code, unconfirmed: def.kind === "command" || !(e.code || String(e.message).includes("compensat")) && def.kind !== "spawn" };
    }
    if (run.unconfirmed) {
...
export function tickHost({ now = new Date(), io, dryRun = false } = {}) {
  ...
  const body = () => {
    const considered = [];
    for (const ws of wsList) {
      try { considered.push(...tickWorkspace(ws, { now, io, reg, wsList, dryRun })); }
 ...
 *  a spawn attempt adopts the instance named for its minute; a command
 *  attempt adopts the ONE home the roster shows created after the attempt;
 *  a proven absence clears it; anything ambiguous stays unknown with the remedy. */
export function reconcile(ws, id, { io, now = new Date() } = {}) {
  const defs = readDefinitions(ws);
  const def = defs.jobs[id];
  if (!def) throw scheduleError("E_SCHEDULE_UNKNOWN", `no schedule ${JSON.stringify(id)} in ${ws}`);
  return withHostLock(() => {
    ...
    if (def.kind === "spawn" && attempt.scheduledFor) {
      ...
      const candidates = named.length ? named : homesCreatedAfter(ws, attempt.wallClock || attempt.startedAt || attempt.scheduledFor);
      if (candidates.length === 1) adopted = candidates[0];
      else if (candidates.length > 1) ambiguous = `${candidates.length} homes appeared after the attempt: ${candidates.map((c) => c.instance).join(", ")}`;
    }
    if (ambiguous) return { schedule: describe(ws, id, io), reconciled: "unknown", remedy: `${ambiguous}; retire or adopt by hand, then remove the attempt from ${statePath(ws)} or run oats schedule remove --force ${id}` };
    delete js.attempt;
    if (adopted) {
      ...
      js.lastRun = { scheduledFor: attempt.scheduledFor, startedAt: js.lastRun?.startedAt || attempt.startedAt, kind: def.kind, launched: false, outcome: "launch-failed", error: "no instance found in the scope's roster for the unrecorded attempt" };
