...
import { RECURRENCE_POLICIES, SCHEDULE_ATTEMPT_VERSION, SCHEDULE_DEFINITION_VERSION, buildCommandExecutionCapsule, capturedDispatchAction, validateExecutionCapsule } from "./schedule-capsule.mjs";
...
export function scheduleExecutionStatus(def, attempt) {
  ...
  if (!versioned) result = { kind: "legacy", capture: "unknown", migrationRequired: true };
    ...
    else result = { kind: "captured", capture: "recorded", migrationRequired: false, schemaVersion: def.execution.schemaVersion, executionId: def.execution.executionId, resolution: def.execution.resolution };
  }
...
export function shapeError(def) {
  ...
  if (def.definitionVersion !== undefined || def.recurrencePolicy !== undefined || def.execution !== undefined) {
    ...
    if (def.recurrencePolicy === "capture") {
      try {
        validateExecutionCapsule(def.execution);
        const expected = buildCommandExecutionCapsule({ cwd: def.cwd, argv: def.argv, inputRefs: def.execution.inputRefs, responsibleHuman: def.execution.responsibleHuman });
        if (expected.executionId !== def.execution.executionId) return "execution capsule differs from the saved command target";
      } catch (error) { return `execution capsule is invalid: ${error.message}`; }
    } else if (def.execution !== undefined) return "prepare-on-tick cannot carry a previously captured execution";
  }
  return null;
}
...
export function validateDefinition(ws, def, { checkAgent = true } = {}) {
  ...
  if (versioned) {
    ...
    if (def.recurrencePolicy === "capture") {
      let execution;
      try {
        execution = buildCommandExecutionCapsule({ cwd: out.cwd, argv: out.argv,
          inputRefs: def.inputRefs ?? def.execution?.inputRefs ?? {},
          responsibleHuman: def.responsibleHuman ?? def.execution?.responsibleHuman ?? null });
        if (def.execution !== undefined) {
          validateExecutionCapsule(def.execution);
          if (def.execution.executionId !== execution.executionId) throw new Error("capsule differs from cwd, argv, inputs or responsible human");
        }
      } catch (error) { throw scheduleError("E_SCHEDULE_INVALID", `execution: ${error.message}`, { field: "execution" }); }
      out.execution = execution;
    } else if (def.execution !== undefined) throw scheduleError("E_SCHEDULE_INVALID", "execution: prepare-on-tick creates a new capsule only when a future tick is admitted", { field: "execution" });
  }
  return out;
}
...
function admitExecution(def, io) {
  ...
  const execution = validateExecutionCapsule(def.execution), action = capturedDispatchAction(execution);
  ...
  for (const id of order) {
    ...
    if (dryRun) {
      ...
      if (!js.attempt && !jobLockInfo(ws, id) && (def.kind === "wake" || liveLocks(wsAll) < bound)) {
        try { admitExecution(def, io); } catch (error) { admission = error; }
      }
