...
import { RECURRENCE_POLICIES, SCHEDULE_DEFINITION_VERSION, buildCommandExecutionCapsule, capturedDispatchAction, validateExecutionCapsule } from "./schedule-capsule.mjs";
...
export function tickWorkspace(ws, { now = new Date(), io, reg, wsList, dryRun = false, only, observeOnly = false, candidates } = {}) {
  ...
  for (const id of order) {
    ...
    js.attempt = { scheduledFor: minute.toISOString(), startedAt: now.toISOString(), wallClock: new Date().toISOString(), ...(execution ? { execution } : {}) };
...
export function reconcile(ws, id, { io, now = new Date(), clear = false } = {}) {
  ...
  return withHostLock(() => {
    const st = readState(ws);
    const js = st.jobs[id] || (st.jobs[id] = {});
    const attempt = js.attempt || (js.lastRun?.outcome === "unknown" ? { scheduledFor: js.lastRun.scheduledFor, startedAt: js.lastRun.startedAt, ...(js.lastRun.execution ? { execution: js.lastRun.execution } : {}) } : undefined);
    if (!attempt) return { schedule: describe(ws, id, io), reconciled: "nothing to reconcile" };
    if (attempt.execution) {
      try { validateExecutionCapsule(attempt.execution); }
