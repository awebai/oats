...
import { RESERVED_LAUNCH_ENV, ensureRoot, findAgent, findCapabilityAgent, findInstanceHomes, teamAgentRoots, configChain, spawnInstance, inspectInstanceSession, inputInstanceSession, startInstanceSession, retirePendingMarkerPath, loadCapturedDispatch } from "./core.mjs";
...
export function scheduleExecutionStatus(def, intent, lock) {
  ...
  if (!versioned) result = { kind: "legacy", capture: "unknown", migrationRequired: true };
    ...
    else if (def.recurrencePolicy === "prepare-on-tick") result = { kind: "prepare-on-tick", capture: "not-captured", migrationRequired: true, reason: "preparation adapter unavailable" };
...
function admitExecution(def, io, { createIntent = true } = {}) {
  ...
  if (def.recurrencePolicy === "prepare-on-tick") {
    ...
    if (typeof io?.prepare !== "function") throw scheduleError("migration-required", "prepare-on-tick needs the preparation transaction adapter before this tick can be admitted");
    const prepared = io.prepare(validatePreparationInput(def.preparation));
