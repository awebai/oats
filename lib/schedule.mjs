...
export function scheduleExecutionStatus(def, attempt) {
  ...
  try {
    const execution = validateAttemptExecution(attempt);
...
function validateAttemptExecution(attempt) {
  ...
  return validateExecutionCapsule(attempt.execution);
}
...
export function tickWorkspace(ws, { now = new Date(), io, reg, wsList, dryRun = false, only, observeOnly = false, candidates } = {}) {
  ...
  for (const id of order) {
    ...
    if (js.attempt) {
      let invalid;
      try { validateAttemptExecution(js.attempt); } catch (error) { invalid = error; }
...
export function reconcile(ws, id, { io, now = new Date(), clear = false } = {}) {
  ...
  return withHostLock(() => {
    ...
    try { validateAttemptExecution(attempt); }
...
export function describe(ws, id, io, { defs, st, now = new Date() } = {}) {
  ...
  return { ...def, executionStatus: scheduleExecutionStatus(def, js.attempt), nextRun, lastRun: js.lastRun || null, running: !!jobLockInfo(ws, id), ...(js.attempt ? { attempt: js.attempt } : {}), ...(js.pendingWake ? { pendingWake: js.pendingWake } : {}) };
}
