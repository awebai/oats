...
export function standaloneRepo(ref, commit, discovery, { remote: injected } = {}) {
  ...
  const souls = (source.souls || []).filter((s) => s.repoKey === key).map((soul) => {
    ...
    // even when the workspace (and its defaults) cannot be read; a soul may say off.
    const kept = {};
    const declaredCaps = soul.definition.capabilities || {};
    if (declaredCaps[STANDALONE_DEFAULT_CAPABILITY] === undefined) kept[STANDALONE_DEFAULT_CAPABILITY] = { from: "package" };
