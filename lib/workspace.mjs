...
export function validateWorkspace(value, { remote = defaultRemote } = {}) {
  ...
  if (isObject(value.defaults)) {
    if (isObject(value.defaults.byTeam)) for (const label of Object.keys(value.defaults.byTeam)) {
...
export function validateSoul(value) { return validateAgainst(schemaFor("soul"), value); }
export function validateLocal(value) { return validateAgainst(schemaFor("local"), value); }
...
function schemaError(kind, origin, problems, value) {
  ...
  return fail("E_WORKSPACE_SCHEMA", `${FILE_KINDS[kind].file} at ${where} is invalid${schemaHint(kind, value)}: ${problems.map((p) => `${p.path || "/"}: ${p.message}`).join("; ")}`, {
    path: origin.path, repoKey: origin.repoKey, commit: origin.commit, problems,
  });
}
...
export async function observeWorkspace(ref, { at, remote: injected, remoteOptions } = {}) {
  ...
  try { ({ bytes } = await remote.readRemoteFile(ref, obs.commit, path)); }
    ...
    if (e?.code === "E_REMOTE_PATH_MISSING") throw fail("E_WORKSPACE_SCHEMA", `${obs.key}@${obs.commit.slice(0, 12)} has no ${path}: it is not a workspace host`, { path, repoKey: obs.key, commit: obs.commit, problems: [{ path: "", message: `${path} is missing` }], cause: e.code });
    throw e;
  }
...
/**
 * For a readable member whose workspace cannot be read: its souls with from:here
 * capabilities only. `discovery` is the repo's own enumeration (discoverRepo(...)
 * or a discoverWorkspace(...) result containing the repo); workspace defaults do
 * not apply and are not known here.
 */
export function standaloneRepo(ref, commit, discovery, { remote: injected } = {}) {
  ...
  const souls = (source.souls || []).filter((s) => s.repoKey === key).map((soul) => {
    const kept = {};
    for (const [name, choice] of Object.entries(soul.definition.capabilities || {})) {
