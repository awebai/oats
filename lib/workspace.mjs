...
export async function discoverWorkspace(ref, { at, local, lock = null, remote: injected, remoteOptions } = {}) {
  ...
  for (const memberRef of workspace.members || []) {
    ...
    const row = { key: confirmation.key, ref: memberRef, commit: confirmation.commit ?? null, confirmed: confirmation.confirmed, team: confirmation.team ?? null, labels: confirmation.labels ?? [], souls: [], capabilities: [], publishes: null };
 ...
 *  messaging provider falls back to the workspace's default team for it. (An undeclared label is E_TEAM_UNKNOWN.)
 *  ONE warning per unmapped label, naming its souls (a workspace on its default team alone would otherwise print
...
function unmappedLabelWarnings(workspace, members, externalRows, teams) {
  ...
  return [...byLabel.keys()].sort(byCodepointOrder).map((label) => {
    ...
    return { code: "unmapped-team-label", label, souls: sorted, paths: order.map((i) => paths[i]),
      message: `team ${show(label)} has no messaging.byTeam entry; its souls (${sorted.join(", ")}) fall back to the workspace's default team for it` };
  });
}
