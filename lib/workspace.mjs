...
async function enumerateRepo(remote, ref, key, commit, { defaultLabels = [], teams = null } = {}) {
  ...
  for (const entry of await list("souls")) {
    ...
    souls.push({ name: definition.name, path, repoKey: key, commit, team: labels[0] ?? null, labels, private: definition.private === true, definition });
  }
...
export async function discoverWorkspace(ref, { at, local, remote: injected, remoteOptions } = {}) {
  ...
  for (const [index, entry] of (workspace.external || []).entries()) {
    ...
    externalRows.push({ source: entry.source, key, commit, soul: { name: read.value.name, path: entry.soul, repoKey: key, commit, team: labels[0] ?? null, labels, private: read.value.private === true, definition: read.value } });
  }
  return { workspace, key: wsObs.key, url: wsObs.url, commit: wsObs.commit, observedAt: wsObs.observedAt, local: local ?? null, members, external: externalRows, problems, warnings: unmappedLabelWarnings(workspace, members, externalRows, teams) };
}
