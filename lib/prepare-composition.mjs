...
import { bindingDeclaration, operatorBindingDeclaration, prepareProviderBindings } from "./prepared-bindings.mjs";
...
export function prepareComposition(input, { repositories, kernel, previous: suppliedPrevious }) {
  canonicalJson(input);
  objectAt(input, ["deployment", "directory", "source", "origin", "workspace", "member", "operator", "mode", "allowLocalPaths", "standaloneContextKey"], ["deployment", "directory", "source", "origin"]);
  ...
  const hasStandaloneContext = Object.hasOwn(input, "standaloneContextKey"), standaloneContextKey = input.standaloneContextKey;
  if (hasWorkspace && hasStandaloneContext) throw oatsError("invalid-declaration", "workspace and standaloneContextKey are mutually exclusive");
  if (hasStandaloneContext && standaloneContextKey !== null
      && (typeof standaloneContextKey !== "string" || !standaloneContextKey || standaloneContextKey.includes("\0") || Buffer.byteLength(standaloneContextKey, "utf8") > 256)) {
    throw oatsError("invalid-declaration", "standaloneContextKey must be explicit null or non-empty opaque text up to 256 bytes");
  }
  ...
  const workspace = hasWorkspace ? discovery.readWorkspace(workspaceRequest) : null;
  ...
  try {
    ...
    const context = workspace ? { kind: "workspace", identity: workspace.identity,
      observation: { ...workspace.parsed.origins[""], kind: "workspace-default" } }
      : { kind: "standalone", key: hasStandaloneContext ? standaloneContextKey : null };
    let seed = { schemaVersion: 1, capture: "prepared", subject: { kind: "persistent", soul }, context, artifacts,
      choices: plan.choices, evidence: imported.provenance };
    const declarations = [bindingDeclaration("soul", imported.soul.declaration, imported.soul.origins)];
    if (workspace) declarations.push(bindingDeclaration("workspace", workspace.parsed.declaration, workspace.parsed.origins));
    for (const adoption of adoptions) {
      const value = adoption.parsed.reference.adoption;
      if (value) declarations.push(bindingDeclaration("adoption", value, adoption.origins, `${adoption.pointer}/adoption`));
    }
    ...
    const completion = bound.problems.length ? { record: null, problems: bound.problems }
      : kernel.complete({ seed, plan, manifests, mode, deployment, directory });
    let resolution = null;
    ...
    return { status: resolution ? (approvals.some((entry) => entry.status === "approval-required") ? "approval-required" : "prepared") : completion.problems?.some(problem => problem.code === "requirement-conflict") ? "conflict" : "needs-configuration",
