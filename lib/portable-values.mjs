...
export function decodeUtf8(input, where = "input") {
  ...
  if (!Buffer.isBuffer(input) && !(input instanceof Uint8Array)) {
    throw oatsError("invalid-declaration", `expected text or UTF-8 bytes at ${where}`);
  }
...
export function parseStrictJson(input, options) {
  ...
  if ((typeof input === "string" ? Buffer.byteLength(input) : input?.byteLength) > limits.maxBytes) {
