// The Desktop's kernel line (F2b): the version band is >=0.25.8 <0.27.0 so it
...
test('the version band alone never decides: 0.26.x without the feature is refused too; below 0.25.8 is not a kernel at all', () => {
  ...
  assert.equal(acceptProbe(probe('0.27.0', [...released.features, 'packages-no-approval'])).ok, false);
});
