...
import { spawnApplySupported, spawnApplyFailure, spawnApplyView } from '../renderer/spawn-apply-contract.mjs';
import { PREVIEW_ONLY } from '../renderer/spawn-preview-contract.mjs';
...
function http() {
  ...
  const c = applyContext(), reads = [], calls = [], legacy = []; let nonce = 0;
  ...
  const deps = { createServer: fn => fn, spawnApplySupported, spawnApplyFailure, spawnApplyRequest: broker, PREVIEW_ONLY,
    spawnAgent: async body => { legacy.push(body); return { instance: 'dev-1', home: '/fixture/dev-1', agent: 'dev', launched: true }; },
    spawnErrorPayload: () => assert.fail('unexpected legacy error'), workspaces: () => [c.workspace], cliState: c.cli,
    agentsData: () => ({ agents: c.agents }), snapshot: { byWs: new Map([['team', { instances: c.instances }]]) } };
  const handler = new Function(...Object.keys(deps), `${source.slice(start, end)}\nreturn server;`)(...Object.values(deps));
  return { c, reads, calls, legacy, async request({ url = '/api/spawn?ws=team', method = 'POST', body = draft(), headers = { host: 'localhost:4820', origin: 'http://localhost:4820' } } = {}) {
...
test('shipped HTTP enforces host/origin, method, query and bounded strict JSON before effect', async () => {
  ...
  for (const url of ['/api/spawn', '/api/spawn?ws=', '/api/spawn?ws=team&ws=team', '/api/spawn?ws=team&other=1']) assert.equal((await h.request({ url })).status, 400);
  for (const body of ['{', 'null', '[]', ' '.repeat(65537)]) assert.equal((await h.request({ body })).status, 400);
  assert.equal(h.calls.length + h.reads.length + h.legacy.length, 0);
...
test('HTTP prepare/ref-only apply/result is the real broker, not the legacy spawn path', async () => {
  ...
  assert.equal(h.calls.length, 1); assert.equal(h.legacy.length, 0);
});
test('only full capable-local raw requests require a plan; truthy fake remote values cannot bypass', async () => {
  const h = http(), body = { agent: 'dev', agentsRoot: '/team/agents', task: 'ordinary' };
  ...
  assert.equal((await h.request({ body: { ...body, serverId: 'remote-host' } })).body.spawned, true); assert.equal(h.legacy.length, 1);
  delete h.c.cli.spawnApplyApi;
  assert.equal((await h.request({ body })).body.spawned, true); assert.equal(h.legacy.length, 2, 'API2-only ordinary spawn preserved');
  for (const extra of [{ branch: 'x' }, { base: 'HEAD' }, { idempotencyKey: ref }, { spawnRef: ref }, { decision: {} }, { model: '@native-default' }]) assert.equal((await h.request({ body: { ...body, ...extra } })).body.code, 'E_PREVIEW_ONLY');
  assert.equal(h.legacy.length, 2); assert.equal((await h.request()).body.reason.code, 'E_APPLY_UNAVAILABLE');
});
...
test('main composition normalizes all spawn aliases to specialized guard, including legacy body', async () => {
...
for (const action of ['prepare', 'apply', 'result', 'legacy']) for (const reject of [false, true]) for (const change of ['frame', 'navigation', 'epoch', 'base']) test(`${action} guards late ${reject ? 'rejection' : 'success'} after ${change}`, async () => {
  const d = deferred(), f = proxy(() => d.promise);
  const body = action === 'prepare' ? draft() : action === 'legacy' ? { agent: 'dev', agentsRoot: '/team/agents' } : { action, spawnRef: ref };
  ...
  const r = await pending, mutation = action === 'apply' || action === 'legacy';
...
test('workspace duplicates survive pinning and are refused before fetch; output targets/refs are bound', async () => {
  assert.deepEqual(apiUrl('/api/spawn?ws=team&ws=other', 'http://localhost:4820', 'team').searchParams.getAll('ws'), ['team', 'other']);
  const f = proxy(assert.fail);
  for (const path of ['/api/spawn?ws=team&ws=other', '/api/spawn?ws=team&extra=1', '//evil/api/spawn']) assert.equal((await proxySpawnApply(f.event, path, f.opts, f.deps)).ok, false);
...
test('ordinary legacy response survives, new typed output cannot leak recipes/private extras', async () => {
  const legacy = { spawned: true, instance: 'dev-1', home: '/fixture/dev-1', launched: true };
  const f = proxy(async () => ({ ok: true, status: 200, text: async () => JSON.stringify(legacy) }));
  assert.deepEqual((await proxySpawnApply(f.event, '/api/spawn', { method: 'POST', body: { agent: 'dev', agentsRoot: '/team/agents' } }, f.deps)).body, legacy);
  const p = prepared(); p.task = 'PRIVATE'; p.preview.env = 'PRIVATE'; p.preview.decision.effective.recipe = 'PRIVATE';
  assert.doesNotMatch(JSON.stringify(spawnApplyView(p, { workspace: 'team', selector })), /PRIVATE/);
});
