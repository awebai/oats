...
// test/fixtures/workspace-v2/automations (main, scratch Northwind; host fixture-laptop,
...
const doc = name => JSON.parse(readFileSync(new URL(`./fixtures/workspace-v2/automations/${name}.json`, import.meta.url), 'utf8'));
...
test('list, enable/disable, test and run: the captured argv, the kernel\'s document verbatim', async () => {
  for (const [name, request] of [['trigger-list', { kind: 'trigger', action: 'list' }], ['schedule-list', { kind: 'schedule', action: 'list' }],
    ['trigger-disable', { kind: 'trigger', action: 'disable', id: 'agents/pr-review' }], ['trigger-enable', { kind: 'trigger', action: 'enable', id: 'agents/pr-review' }],
    ['schedule-disable', { kind: 'schedule', action: 'disable', id: 'agents/nightly' }], ['trigger-test-here', { kind: 'trigger', action: 'test', id: 'agents/pr-review' }],
    ['trigger-test-mismatch', { kind: 'trigger', action: 'test', id: 'agents/triage' }]]) {
  ...
  const mismatch = await automationsRequest({ kind: 'trigger', action: 'test', id: 'agents/triage' }, { workspace, cli, invoke: replay('trigger-test-mismatch').invoke });
...
test('kernel refusals keep their own code and message (E_AUTOMATION_NOT_HERE, E_AUTOMATION_WORKSPACE)', async () => {
  const run = await automationsRequest({ kind: 'schedule', action: 'run', id: 'agents/weekly' }, { workspace, cli, invoke: replay('schedule-run-elsewhere').invoke });
...
test('gates and admission: no call without the feature, a known local workspace and a valid verb/id', async () => {
  ...
  const cases = [
    ...
    [{ kind: 'trigger', action: 'run', id: 'agents/pr-review' }, cli, workspace, 'E_BAD_ARGS'],
    [{ kind: 'schedule', action: 'test', id: 'agents/nightly' }, cli, workspace, 'E_BAD_ARGS'],
    [{ kind: 'schedule', action: 'remove', id: 'local/digest' }, cli, workspace, 'E_BAD_ARGS'],
    [{ kind: 'trigger', action: 'list', id: 'x' }, cli, workspace, 'E_BAD_ARGS'],
    [{ kind: 'trigger', action: 'enable' }, cli, workspace, 'E_BAD_ARGS'],
    [{ kind: 'trigger', action: 'enable', id: 'agents/pr-review', extra: 1 }, cli, workspace, 'E_BAD_ARGS'],
