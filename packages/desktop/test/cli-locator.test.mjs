...
test("the human-readable band is derived from the enforced numbers", () => {
  ...
  assert.match(acceptProbe(PROBE("0.25.0")).reason, new RegExp(ACCEPT_RANGE_TEXT.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
});
...
test("band edges: released 0.22.x through 0.24.x are accepted, 0.25.0 and the pre-0.22 floor are not", () => {
  assert.deepEqual(ACCEPT_RANGE, { min: [0, 22, 0], maxExclusive: [0, 25, 0] });
  assert.equal(ACCEPT_RANGE_TEXT, ">=0.22.0 <0.25.0");
  assert.equal(DESKTOP_API, 1, "Desktop API stays v1 across the v0.24.0 kernel bump");
  // inside — including both edges of the newly admitted minor
  for (const v of ["0.22.0", "0.22.1", "0.22.12", "0.23.0", "0.23.1", "0.23.99", "0.24.0", "0.24.1", "0.24.99", "0.24.0+build.7"]) {
    assert.equal(acceptProbe(PROBE(v)).ok, true, `${v} must be accepted`);
  }
  // outside — the exclusive ceiling and everything above it
  for (const v of ["0.21.9", "0.25.0", "0.25.1", "1.0.0"]) {
    const r = acceptProbe(PROBE(v));
    assert.equal(r.ok, false, `${v} must be rejected`);
    assert.match(r.reason, /outside >=0\.22\.0 <0\.25\.0/, v);
  }
