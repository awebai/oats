// Import the instance git line-count capture (kernel #238, 0.29.1): `oats instance git` for a spawned
// worktree home with a modified text file, a staged new file, a modified committed binary and an
// untracked file (capture-git.mjs + provenance.json). NEVER runs a CLI. <base>/<oats> → fixture paths.
...
const documents = ['version', 'instance-git'];
const sha = bytes => createHash('sha256').update(bytes).digest('hex');
const captured = JSON.parse(readFileSync(join(source, 'provenance.json'), 'utf8'));
const provenance = { source: `${captured.capturedBy}; test/fixtures/northwind/build.mjs; capture-git.mjs`, kernel: captured.kernel,
