// Import the Desktop facts capture (kernel #217, feature desktop-facts, OATS 0.29.0): souls,
// capabilities, workspace status, inspect --soul and status (a spawned home) on a scratch
// Northwind from the kernel tree's own build.mjs; one soul disabled in oats-local.yaml; a spawn
// with --model (capture-facts.mjs + provenance.json). NEVER runs a CLI, runtime or native probe.
// <base>/<oats> placeholders become absolute fixture paths.
...
const documents = ['version', 'souls-before-disable', 'souls', 'capabilities', 'workspace-status', 'inspect-soul', 'preview', 'apply', 'status', 'inspect-home'];
const sha = bytes => createHash('sha256').update(bytes).digest('hex');
const captured = JSON.parse(readFileSync(join(source, 'provenance.json'), 'utf8'));
const provenance = { source: `${captured.capturedBy}; test/fixtures/northwind/build.mjs; capture-facts.mjs`, kernel: captured.kernel,
