...
export function workspaceStatusData(document, deployment) {
  ...
  const members = array(data.members).map(row => {
  ...
  const packages = array(data.packages).map(row => {
  ...
  return flags(data, ['standalone'], { workspaceStatusApi: 1, workspace, members, packages,
    ...
    problems: array(data.problems).map(row => fields(row, ['code', 'message', 'repoKey', 'path'])) });
}
