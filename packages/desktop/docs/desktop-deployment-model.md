...
`ACCEPT_RANGE` is `>=0.25.8 <0.30.0`: the floor admits main's kernel before
0.26.0 was tagged, the ceiling admits 0.27 (the harness rename, gated on feature
`harness`) through 0.29 (workspace automations and desktop facts, gated on
features `automations` and `desktop-facts`), and the `packages-no-approval`
fence is the real gate.
