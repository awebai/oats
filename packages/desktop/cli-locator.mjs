...
// Desktop accepts desktopApi === 1 and semver >=0.22.0 <0.26.0 (the band is
...
// Widened through 0.24.x for the retained-execution first cut. Desktop CLI API
// v1 and the 0.22.x floor remain unchanged; individual capability/refusal gates
// still apply. Version acceptance does not assert captured UI/backend parity.
// Widened through 0.25.x for the workspace model (v0.25.0): the Desktop v1
// surface (`version --json` probe, `spawn --json`, the retained lifecycle
// verbs — test/cli-json-contract.test.mjs) is unchanged; the removed verbs
// (`catalog`, install/use/…) surface as typed E_USAGE through the adapter.
export const ACCEPT_RANGE = { min: [0, 22, 0], maxExclusive: [0, 26, 0] };
...
export async function discover(io, probe) {
  ...
  for (const src of candidateSources(io)) {
    ...
    for (const raw of paths) {
      ...
      if (a.ok) return { ok: true, bin: path, source: src.source, version: payload.version, ...(Array.isArray(payload.runtimes) ? { runtimes: payload.runtimes } : {}), ...(Array.isArray(payload.sessionBackends) ? { sessionBackends: payload.sessionBackends } : {}), ...(Array.isArray(payload.launchOptions) ? { launchOptions: payload.launchOptions } : {}), ...([1, 2].includes(payload.scheduleApi) ? { scheduleApi: payload.scheduleApi } : {}), ...(payload.operationsApi === 1 ? { operationsApi: 1 } : {}), ...(payload.lifecycleApi === 1 ? { lifecycleApi: 1 } : {}), ...(payload.readinessApi === 1 ? { readinessApi: 1 } : {}), ...(payload.spawnPreviewApi === 2 ? { spawnPreviewApi: 2 } : {}), ...(payload.spawnApplyApi === 1 ? { spawnApplyApi: 1 } : {}), ...(payload.eventsApi === 2 ? { eventsApi: 2 } : {}), ...(payload.scheduleHistoryApi === 3 ? { scheduleHistoryApi: 3 } : {}), ...(Array.isArray(payload.features) ? { features: payload.features } : {}), ...(Array.isArray(payload.remote) ? { remote: payload.remote } : {}) };
