...
export async function discover(io, probe) {
  ...
  for (const src of candidateSources(io)) {
    ...
    for (const raw of paths) {
      ...
      if (a.ok) return { ok: true, bin: path, source: src.source, version: payload.version, ...(Array.isArray(payload.runtimes) ? { runtimes: payload.runtimes } : {}), ...(Array.isArray(payload.sessionBackends) ? { sessionBackends: payload.sessionBackends } : {}), ...(Array.isArray(payload.launchOptions) ? { launchOptions: payload.launchOptions } : {}), ...([1, 2].includes(payload.scheduleApi) ? { scheduleApi: payload.scheduleApi } : {}), ...(payload.operationsApi === 1 ? { operationsApi: 1 } : {}), ...(Array.isArray(payload.features) ? { features: payload.features } : {}), ...(Array.isArray(payload.remote) ? { remote: payload.remote } : {}) };
