...
export async function automationsRequest(request, { workspace, cli, invoke = cliAutomation }) {
  ...
  // `key` is the row's qualified id (local/<id>, <member>/<id>) or a bare local id.
  ...
  const envelope = await invoke(cli.bin, { kind, action, ...(keyed ? { id: request.key } : {}), workspaceDir: workspace.scope });
