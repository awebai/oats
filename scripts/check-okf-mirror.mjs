...
// --generate/--finalize replace ONLY capabilities/oats-okf and the inventory.
...
export const CAPABILITY_PATH = "capabilities/oats-okf";
...
function manifestFrom(text) {
  ...
  assert.deepEqual(Object.keys(manifest).sort(compare), ["capabilities", "compatibility", "description", "package", "version"], "new distribution surfaces require explicit mirror review");
  ...
  assert.deepEqual(manifest.capabilities, [CAPABILITY_PATH], "standalone must export ONLY capabilities/oats-okf");
...
function validateInventory(inventory) {
  assert.equal(inventory.schemaVersion, 1, "unsupported OKF inventory schema");
  assert.equal(inventory.distributionPath, DISTRIBUTION_PATH);
  assert.equal(inventory.capabilityPath, CAPABILITY_PATH);
...
export function checkOkfPayload(capabilityRoot, inventory) {
  validateInventory(inventory);
  const actual = payloadEntries(capabilityRoot);
  ...
  const capability = JSON.parse(readFileSync(join(capabilityRoot, "oats.json"), "utf8"));
  assert.equal(capability.capability, "oats.okf");
  assert.equal(capability.version, inventory.version);
  return inventory;
}
...
export function checkOkfMirror({ repoRoot = REPO_ROOT } = {}) {
  ...
  return checkOkfPayload(join(repoRoot, CAPABILITY_PATH), inventory);
}
...
export function materializeOkfGitPayload(destination, { repoRoot = REPO_ROOT } = {}) {
  ...
  const mirror = realpathSync(join(repoRoot, CAPABILITY_PATH));
  ...
  assert.ok(!contained(mirror, resolvedDestination) && !contained(resolvedDestination, mirror), "payload destination and source mirror must not overlap");
  mkdirSync(destination, { recursive: true });
  plainDirectory(destination);
  assert.deepEqual(readdirSync(destination), [], "OKF payload destination must be empty");
  cpSync(join(repoRoot, CAPABILITY_PATH), join(destination, CAPABILITY_PATH), { recursive: true, verbatimSymlinks: true });
  ...
  return checkOkfPayload(join(destination, CAPABILITY_PATH), inventory);
}
...
export function generateOkfSourceInventory(standaloneRoot) {
  ...
  const inventory = {
    ...
    entries: payloadEntries(join(distribution, CAPABILITY_PATH)),
  };
  ...
  return checkOkfPayload(join(distribution, CAPABILITY_PATH), inventory);
}
...
function commitPayload(source, commit) {
  return gitTree(source, `${commit}:${DISTRIBUTION_PATH}/${CAPABILITY_PATH}`).map(({ mode, type, oid, path }) => {
...
function finalSourceInventory(standaloneRoot, { finalTag, finalCommit, repository = SOURCE_REPOSITORY } = {}) {
  ...
  inventory.snapshotSha256 = snapshotHash(inventory);
  return checkOkfPayload(join(source, DISTRIBUTION_PATH, CAPABILITY_PATH), inventory);
}
...
function syncInventory(standaloneRoot, repoRoot, makeInventory) {
  ...
  const source = realpathSync(standaloneRoot);
  const destination = join(repo, CAPABILITY_PATH);
  assert.ok(!contained(destination, source) && !contained(source, destination), "source and mirror must not overlap");
  ...
  for (const [path, type] of [[destination, "directory"], [output, "file"]]) {
    let stat;
  ...
  try {
    ...
    cpSync(join(source, DISTRIBUTION_PATH, CAPABILITY_PATH), staged, { recursive: true, verbatimSymlinks: true });
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    ...
    console.log(JSON.stringify({ ok: true, version: inventory.version, entries: inventory.entries.length, snapshotSha256: inventory.snapshotSha256, sourceHead: inventory.source.head, sourceDirty: inventory.source.dirty, release: inventory.release }));
  } catch (error) {
