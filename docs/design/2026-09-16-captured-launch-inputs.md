...
The initial executable contract is deliberately bounded to a command already exported by an exact selected capability. No new package resolver is introduced, and an arbitrary host executable path is not treated as retained runtime authority. A selected capability can export a native runtime entrypoint; executable validity, required dependency roots and runtime-specific loading still need qualification before actual start. This is not a general native-runtime installer or a replacement for runtime package acquisition.
