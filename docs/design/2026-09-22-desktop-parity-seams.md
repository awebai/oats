...
Lead: K1, K4, K5 (readiness/policy shape), K6 preview/apply plumbing, K7/K8 projections, dispatch contract for additive-capability views — proposed as Decisions where they change contracts, implemented in small PRs otherwise. P1: **`oats.forge`** (forge-neutral; GitHub backend first; defined in the oats repo) — **Decision ACCEPTED** (`agents/oats-expert/soul/knowledge/decisions/p1-oats-forge-capability-and-additive-view-dispatch.md`): official additive capability + typed additive-view dispatch `oats view <cap>:<name>` + native credential custody (`gh`). Implementation: kernel view contract (lead) then `capabilities/oats-forge/` (lead); slice 2b consumes. Desktop engineer: all slices, Desktop IPC review items (detach, open-in-editor).
