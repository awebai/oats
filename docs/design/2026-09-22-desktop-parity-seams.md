...
> **Ownership amendment (2026-09-22, human):** contract-dependent slices are not waits — the team implements the contracts. Kernel seams may be assigned to the Desktop engineer under lead review (the `lib/`/`bin/` lane rule is lifted per assigned seam). Current split: **K1 → engineer**; **K4, P1 Decision, then K3/K5 → lead**. Kernel PRs: full root `npm test` + DTO in `docs/desktop-cli-api.md` in the same PR.
