...
## Staged official OKF pairing

This cut stages **oats.okf 2.1.0**, requiring **OATS >=0.24.0**, at the existing
`oats-package` source path. The catalog's intended `v2.1.0` ref is not itself
publication evidence. Mirror provenance remains **pending** until the release owner
verifies the actual published tag and accepted commit through the existing finalizer.
Run installed acceptance on the release-stamped0.24 artifact, not the still0.23.2
source manifest with this newer provider floor. No compatibility gate is bypassed.
