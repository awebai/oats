...
four tests that pass on macOS and fail on Linux (three launch test fixtures
that resolved the real harness binaries on the test process PATH, and one
capture-lock test where Linux reused the inode of an unlinked directory).
The tag stays fixed and unpublished. This release carries the same content
...
- Capture lock (source only; the installed capture service is unchanged):
  ...
  so a replacement that reuses the unlinked directory's inode (Linux) is
  recognised by device and inode and left alone.
