...
resolves it each time the agent starts, and no command, receipt or answer
ever carries the value. Previews, roster answers and spawn answers redact
environment values. A configuration cannot name the kernel's own launch
...
`oats session restart` (and `session start` with a selection) checks the
requested launch before asking the current harness to stop: recipe,
executable, references, capability preparation, runtime packages. It sends
...
existing remote homes keep their saved route, and older kernels are refused
before anything is sent or stopped.
