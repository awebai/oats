...
- **Package souls** (feature `package-souls`). A package may ship souls as
  ...
  - The spawn fetches the soul at the locked commit and verifies it against the
    lock's digest (`E_PACKAGE_INTEGRITY { why: "soul-digest" }`).
    `instance.json.workspace.soul` records `package: { id, version, commit,
    digest, path }`, and the soul id is `package:<id>#<soul>`. `oats status`
    shows the soul as `moved` once the package pin moves.

## Changed

- **`oats-local.yaml` `souls.disabled` is enforced at spawn.** A listed soul is
  refused with `E_SOUL_DISABLED { name, qualifiedName, entry }`. Before, the
  list only counted souls in the `oats sync` report. An entry is a bare name
  (every soul of that name) or a qualified name (`oats.okf/knowledge-harvester`,
  `<member>/<soul>`).
