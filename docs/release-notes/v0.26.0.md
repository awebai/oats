...
- **Local souls and `local-agents/`.** A soul is a member repository's
  ...
  - **The kernel no longer runs `soul-scaffold` hooks.** They ran when
    `oats create` wrote a soul. A manifest may still declare the hook; nothing
    runs it.
...
- **Retire capability-agent instances before upgrading.** Instances homed
  ...
  from that directory. Retire them with the 0.25 kernel first, or accept that
...
- **`oats inspect`, `oats readiness` and `oats operation run` read the
  ...
  - The probe's integers say the kernel can answer the v2 shapes. Dispatch
    on each payload's own integer: a classic scope still answers v1 until the
    classic chain is removed.
  - The `readiness-verify` feature is no longer advertised: a workspace
    deployment refuses `--verify-signatures`.
