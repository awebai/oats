...
- **`oats-config.yaml` and the scope configuration chain** (lead decisions
  ...
  `docs/oats-config.schema.json` stays as a record of what 0.25 read. Each key:
...
- **A soul may name several team labels** (teams contract,
  ...
  - The merged messaging payload, `OATS_TEAM_LABEL` and `OATS_TEAM_ID` stay the
    ...
    `OATS_TEAMS_SOURCE` (`live` | `recorded`). All three are set in every hook
    and home command, and `teams` + `teamsSource` sit beside `settings` on a
    provider check's stdin. They are empty when a home's teams are unknown. A
    provider leaves a joined team only when the source is `live`.
