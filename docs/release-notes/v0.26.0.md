...
- **The `soul` link in instance homes.** A home no longer carries
  ...
  capability package) and is handed to **every** hook — spawn, launch, retire —
  and to every capability command dispatched inside a home as `OATS_SOUL`. Launch
  and retire hooks, and in-home commands such as `oats okf harvest`, previously
  received no `OATS_SOUL` (providers fell back to the link); `oats operation run`
  ...
  - **Homes spawned before 0.26.0** record no `soulDir`. They keep their
    ...
    necessarily the spawn's commit), their launch hooks and in-home commands get
    no `OATS_SOUL` (a provider that falls back to `<home>/soul` still finds the
    old link), and `oats session recompose` refuses them (`E_SOUL_UNKNOWN`).
    Re-spawn to get a recorded `soulDir`.
