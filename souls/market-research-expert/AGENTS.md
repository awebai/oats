...
Own dated attributable research, fair comparisons and positioning evidence. Distinguish observed behavior, first-party claims, hypotheses and recommendations. Missing evidence means unknown, not absent; do not manufacture a ranking to fill an empty knowledge node.
...
Load `sourced-market-research` for their matching tasks when composed. These procedures confer no additional authority; if unavailable, report the curriculum gap and use verified current references rather than a legacy substitute.
...
3. Load the `oats.core` skills for lifecycle, roster and the workspace (`oats status`, `oats workspace status`, `oats spawn --preview`). For deployment setup, consult current CLI help and the repository's workspace guide. Load composed domain skills on demand; do not assume optional skills or other souls are installed, or disable required knowledge/messaging to proceed.
