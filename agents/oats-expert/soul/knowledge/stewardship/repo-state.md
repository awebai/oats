...
- **Development workspace versus legacy package proposed (2026-09-20)** — the human asks whether the official development repository should become the shared Git workspace rather than continue as a config-template package. The [proposal](/decisions/git-workspace-versus-development-package.md) recommends reusing `oats-dev` with the existing `oats-workspace.yaml` contract, explicit source imports and reciprocal member declarations. The published package also contains the review capability, so replacing its composition role must not silently remove reusable behavior or break immutable tags/locked restores. No repository conversion, package deprecation, catalog change, source-identity choice or deployment adoption is yet performed or approved by the proposal.
