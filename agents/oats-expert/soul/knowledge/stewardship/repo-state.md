...
- **Canonical model documentation and next Desktop wave (2026-09-19)** — human-approved README introduces open, decentralised, provider-independent agent teams, capabilities, ephemeral and long-running instances. The existing `docs/knowledge-theory.md` is the detailed canonical account of the centralised per-soul default, capability-owned learning/placement, promotion doctrine and reviewed speciation/context-reuse direction. [The recorded decision](/decisions/flexible-knowledge-and-instance-expertise.md) narrows external-only placement to the default, not a kernel rule. No implementation, source-layout or bootstrap authority is silently added. The original untracked design notes remain preserved. The human explicitly directs documentation to main, then a fresh Desktop engineer with historical notes to implement supplied designs and new/Git-based flows; this supersedes the previous scheduling hold, not runtime/provider safety or evidence limits.
