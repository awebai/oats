...
- **Kernel release preflight (2026-09-19)** — source CI at `0999f4a8` reports41 failures:39 are explicit provider-floor refusals under the still-unstamped0.23.2 source version; the actual release lane stamps0.24.0 before testing. Two separate Desktop server integration assertions still expect the old0.24 ceiling, despite the approved0.25 ceiling. Their owner has a narrow test-only correction request preserving0.23/0.24 acceptance and0.25 rejection; no runtime, floor or gate weakening. Do not create the kernel tag with those known stale assertions unresolved.
