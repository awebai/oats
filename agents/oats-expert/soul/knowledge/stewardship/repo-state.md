...
- **Shared helper/input producer returned (2026-09-17)** — independent review of `be2460c5` found a P1: its new-helper policy verifier admits an extra choice-less capability file block when that owner declared no instruction/helper contribution. The semantic guard probe accepted the malformed contribution despite eight passing focused tests. A narrow successor must account for every capability-sourced block/omission and add a real publication-path regression while preserving primary/literal historical reads. One reviewer owns producer closure and separately scoped schema/provider pairing; native/public/Herdr review stays separate. `f43996d` reader approval remains valid, and delivered source checkpoints are unaffected. Provider successor `57aaec7` now supplies exact schema synchronization, repository-validator changes and owner-reported paired evidence; producer corrective `59d5bd18` also exists. Both require their scoped independent verdicts: manifest/pair success against be does not waive its P1, and native `5d95175d` / public clarification `105cce24` remain separately reviewed ancestry.
- **Provider execution-reader / helper-input pairing (2026-09-17)** — reader `f43996d` and separate provider declaration/input candidate `0f809af1` are delivered for review/pairing, not main-ready. The latter's old vendored closed schema rejects the new declaration sites; kernel owner must supply the exact committed producer/schema before byte-exact schema synchronization and paired qualification. The immutable input prevents circular waiting without weakening schema or inventing a fork. Developer evidence is initial 8/9 plus one affected diagnostic recheck, not a fresh full pass. Existing `ec5d767` source-main checkpoint remains unchanged; reviewers own the new verdicts, and release floor/native/helper/privacy qualification stays open.
- **Native start candidate review (2026-09-17)** — `2a3d785c` implements a retained core API over the existing native transaction, with inert-process evidence, not public helper/Pi/model qualification. The native reviewer returned lifecycle-acquisition and same-ID restart P1s, the peer's endpoint P2, and a fixture-portability P3. Corrective `dd4eb82a` now has scoped independent approval for all four closures (four new tests). Its parent public bridge `bbde8c6e`, real Pi/provider execution and Herdr parity remain separate review/acceptance work. This candidate is separate from approved admission/curriculum/request slices and is not in their staged integration; no whole-native-head or release approval is implied.
