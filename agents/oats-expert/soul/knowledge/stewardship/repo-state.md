...
- **Managed runtime and remaining lifecycle qualification (2026-09-17)** — code integration, conflict review, shared helper-policy correction, provider pairing and the actual combined gate are complete, not new review queues. Managed Pi roots/contributions, actual retained primary/helper model execution and public wake/retire/recovery remain separate unfinished work. Bounded public SDK evidence at Pi 0.85.1 did not establish nonambient nonempty extension loading; it is not categorical Pi incapability. A genuinely zero-plugin profile is a distinct question and may not silently drop selected requirements. Proposed exact runtime-bundle approval in the existing ledger, sequential launch-contribution admissions and staged retirement remain human-pending contracts, not implemented permission or qualification. No private SDK import, ambient discovery fallback, replayed completed hooks or persisted secret delivery is authorized.
...
- Integrated native/provider source: framework merge `cd938860` plus approved docs `e0bccef0`, paired provider `86bff1a`, delivered to both mains on 2026-09-17. Actual combined3/3, standalone285/0/5 and framework2704/0/3 plus all eight gates pass; original failed run and scoped evidence limits preserved. No release/deployment claim (see delivery-log).
