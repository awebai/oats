...
- **Standalone OKF direct-main delivery review (2026-09-16)** — implementation delivery may use coordinator-reviewed direct main pushes in the provider repository; runtime Git knowledge publication remains PR-only. Candidate `388182c` is returned for captured worker/completion selection, binding/identity safety, owner-registry lookup and conditional checkout-filter issues despite passing targeted tests. Fixes belong to the provider, with a generic retained-helper interface supplied by the kernel; no kernel harvester or ambient fallback. Planned minimum/version metadata and real consumer qualification remain publication gates. No new provider source-main delivery, tag or activation is claimed yet.
