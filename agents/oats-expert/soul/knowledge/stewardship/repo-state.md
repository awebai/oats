...
- **2026-09-22 — OATS 0.24.6 published.** `oats catalog [--json]` (read-only effective official catalog for the Desktop Capabilities view) and Desktop parity slice 1a (compact relation guides, engine-owned ⌘F/⌘N, rebind-aware hints). CI pull-request workflow checks out full history. S8 plan and seams recorded in `docs/design/2026-09-22-desktop-parity-seams.md`; lifecycle/enrollment/permission/auto-PR decisions taken under delegation.
- **2026-09-22 — OATS 0.24.5 (PR43, lead-implemented on human authorisation).** Retire recovery derives the branch from the worktree; captured launch treats `ifInstalled` runtime rows as satisfied by absence and refuses hard rows attributed; helper-injection refusal attributed; `onboard --workspace` reads the catalog from the workspace revision. Rule: `prepare` never ends in a bare refusal after selection.
- **2026-09-21 — OATS 0.24.4 published (kernel-first wave).** Provider fixed reasons now cross the binding wire (exact match against manifest `binding.reasons` or reviewed bundled aweb 1.11.0 / OKF 2.1.2 lists; free text still dropped; check `result.problems` same rule); optional `binding.keys` accepted shape-only (enforcement 0.25); `inspect --request` accepts prepare's `operator`/`launch`/`helperLaunches`/`mode`/`allowLocalPaths` as `ignored`, so one request file serves both commands; CLI renders problem `key`. Decisions: provider-problem-reasons-cross-the-wire, operator-bindings-ownership. Providers declaring the new fields floor on `>=0.24.4` (validator was closed on ≤0.24.3). Workspace source policy fixed earlier the same day (`teams: {private: per-human}`, editions `teams: []`, 906b1558) and imports re-pinned.
- **2026-09-21 — OATS 0.24.3 published** with the five second-operator `prepare` seams fixed (attributed provider problems naming the missing item, typed v3 trust guidance, `--artifact-set` in help, typed absent-deployment hold, one request file for inspect/prepare + `--emit-prepare-request`); the recorded second-operator pair is preserved as fixtures and stays identical by design (both refused on missing OKF runtime settings). Two CI flakes (transient `.git/objects/maintenance.lock` in byte-snapshot walks) hardened.
- **2026-09-21 — OATS 0.24.2 published: `oats onboard`, explicit `oats.core`, aweb 1.11.0 pairing.** The distribution model decided on 2026-09-20 is now shipped end to end: `oats.framework` 1.1.1 (tag) exports `oats.core`/`oats.setup`/`oats.knowledge-theory` and is the official catalog entry; soul creation writes an explicit removable `oats.core`; `oats onboard` bootstraps an `oats-setup-expert` holding both capabilities (verified from the npm tarball); declared capabilities suppress the legacy kernel skills. aweb 1.11.0 adds the portable binding interface and is pinned by the catalog and all six framework soul editions; its `check` qualifies HOME-route custody for input-capable Claude/Codex only and reports `needs-configuration` for strict-Pi print. The independent second-operator gate (Juan's machine) proved discovery/resolution/approval from public sources and found five `prepare` usability seams, deliberately NOT in 0.24.2 (target 0.24.3). Tag recut once before publish for bundled-copy version parity — lesson recorded.

- **2026-09-20 evening — OATS 0.24.1 published; official capabilities live; five experts exported and imported.** `v0.24.1` (kernel/Pi/Desktop) published with the public inspection seam, home-route captured custody and the OKF 2.1.1 pairing. `oats.framework` 1.1.0 (tag `oats-framework/v1.1.0`) ships `oats.core`, `oats.setup` and `oats.knowledge-theory`; the catalog lists it with capability aliases, and `oats create` now writes an explicit, removable `oats.core` into every new soul (declared ⇒ legacy kernel skills are not composed). All five framework expert editions live under `souls/`, declare `oats.core` from the same repository payload, are exported by `oats.yaml` and imported by `oats-workspace.yaml` at their published commit; a live `oats inspect --request` against published `main` resolves them. Late evening: all seven member indexes on main (access granted); `awebai/oats-knowledge` is public with the 25-concept curated base accepted on `main` (PR #1). Remaining: `oats-setup-expert` onboarding (D3), aweb adapter (1.11.0), second-operator fresh-deployment gate and fresh-reader proof (Juan's side), legacy roster/knowledge cutover, Desktop parity (hosted on Juan's machine).

- **2026-09-20 afternoon — redesign delivery wave.** PR23 (framework `oats-workspace.yaml`/`oats.yaml`, transitional `souls/oats-expert`), PR24 (`oats inspect --request` public read-only inspection seam; human-ordered removal of the kernel setup skill), PR26 (`docs/official-marketplace.md` + catalog policy pointer) merged; stale validate anchor fixed. Member `oats.yaml` indexes merged to main in oats-okf, oats-aweb, oats-authoring, oats-jira. The living cross-stream status is the [program board](https://github.com/awebai/oats/blob/main/docs/design/2026-09-20-redesign-program-board.md); in flight: OKF consumer fix (oats-okf PR4), D1 `oats.core`/`oats.setup` packaging, D2 explicit default + HOME-route custody fix, S6 five soul editions. Blocked on the human: push access to oats-dev/oats-linear; `oats-knowledge` visibility; Desktop engineer runtime.

- **Forward record correction reviewed and accepted (2026-09-19)** — `0060dbe8b0c8b470083d9a14c78ef4653d567b9a` restores exactly four record files from `635fc7e6` to their approved `3f51acf3` postimages. Independent full-diff, blob and whole-tree checks confirm `packages/record` is again `9137bba602ec04603dbfb9d51b81359c5a8dd0ce`; all other paths retain the prior main bytes. Net change from the approved baseline is only the independent `docs/layers.md` reach clarification. The human chose forward correction and explicitly authorised continuing from the repaired history: held `54b07ee6` remains an ancestor, but its active delta is removed and its scope hold is NOT lifted. The integrating maintainer reports 223/223 record tests; the reviewing maintainer independently verifies exact source equality and passes the bounded scaffold/layout/copied-resource/normal-retirement probe (1/1, no native runtime/backend). Normal fast-forward preserves pending stewardship edits. No history rewrite, release/tag change or new optimisation approval.

- **Held-patch integration incident, now corrected (2026-09-19)** — the `lead` merge into `635fc7e6` introduced both held `54b07ee6` ancestry and its four-file normalized delta, stable patch-id `21b53623067540c09d632b49d2f81ae1b931eda6`. The integrating maintainer confirmed no superseding approval existed; reported green tests did not lift the scope hold. The explicit forward-only human disposition and independently verified `0060dbe8` correction above close the repair/adoption blocker, not the separate question of whether that optimisation should ever be approved. Original evidence remains preserved; no runtime defect or erased history is asserted.

- **Canonical OATS and knowledge explanation published (2026-09-19)** — `edd82f82` is delivered to main, verified by remote ref and GitHub file readback. README introduces open, provider-independent specialised teams; the existing `docs/knowledge-theory.md` is the detailed canonical account. Operational guides and [the accepted decision](/decisions/flexible-knowledge-and-instance-expertise.md) distinguish the centralised per-soul default from capability-owned alternative placement/learning, and short- from long-running instances. Twelve Markdown files only; strict OKF 89 concepts / zero errors or warnings, 56 local links/anchors and diff check pass. No runtime, source-export, bootstrap or deployment changes follow from this documentation delivery.
...
- **S2 — FIRST SECOND-OPERATOR PUBLICATION (2026-09-21)**: after oats.framework 1.1.3 / aweb 1.11.2 (helperInjection declared), a fresh outside operator published an executable resolution from the public definition alone (no `launch`). OKF 2.1.2 `check` then refuses `provider-not-qualified` with no reason (code-only check phase folding ≥4 causes) → OKF 2.1.3 (P). Pi `launch` blocked by the captured-path `ifInstalled` defect → 0.24.5 (L).
- **S2 — operator amended verdict (2026-09-21, superseded above)**: knowledge binds to the public base from a fresh machine and messaging normalizes, but no resolution can publish: behind `responsibleHuman`, helper composition refuses because `oats.core` and `oats.aweb` ship `inject` without `helperInjection` (OKF adopted the contract; siblings did not). Packaging, not operator input. Fix in flight: oats.framework 1.1.3 (core `inherit`) + aweb 1.11.2 (`omit`) + release check + attributed early refusal; then unchanged re-run.
- **0.24.4 provider wave (2026-09-21) — DELIVERED**: OKF 2.1.2 v2.1.2 (`f02a0d9a`), aweb 1.11.1 (`d404da76`), oats.framework 1.1.2 (`6f98c7c9`), pins and imports moved (`08c68ece`); lead's tarball probe shows knowledge binding with `wider: []` present and the OKF named reason on the wire. Open: independent operator's unchanged re-run, then `responsibleHuman` (Juan) → first published second-operator resolution → OKF `check` probe. Original scope was: OKF 2.1.2 (oats-okf PR5: named-setting diagnostics, source-derived owned-bindings filter fixing the `wider` deadlock, `binding.reasons`; still needs floor `>=0.24.4` + `binding.keys ["stores.","write.default"]`) and aweb 1.11.1 (manifest-only: reasons + keys, floor `>=0.24.4`) to be merged/released by the lead, then `oats.framework` 1.1.2 (P's held skill-text patch), catalog + six edition pins, import re-pin, and the second operator's unchanged re-run with `wider: []`. `binding.keys` kernel enforcement (overlap refusal, filtered forwarding, stray-key attribution) deferred to 0.25 (L).
- **Distribution direction accepted: official capabilities and marketplace (2026-09-20)** — the human directs that the kernel's operational skills become two official capabilities shipped from this repository: `oats.core` (skills `oats-operate`, `oats-souls`, the "you run on OATS" injection), declared **explicitly** on every soul by default and removable by the user, never silently injected; and `oats.setup` (former `oats-config`/`oats-packages` and workspace adoption guidance), held by an `oats-setup-expert` soul that workspace onboarding creates and instantiates. The `instance-boundary` injection stays kernel-owned. The official marketplace is the reviewed `package-catalog.json` list in this repository — listing by approved PR is the safety gate, including for external packages; fundamentals plus the two new capabilities are the first entries. Recorded in the [decision](/decisions/official-capabilities-oats-core-setup-and-marketplace.md) and as work packages D1–D4 in the [plan](https://github.com/awebai/oats/blob/main/docs/design/2026-09-20-workspace-and-portable-adoption-plan.md). Lanes are assigned after the current phase1 PRs are integrated; nothing is packaged, de-ambiented or onboarded yet. This also resolves the setup-skill question: no kernel skill is recreated; setup content belongs to the capability.

- **Adoption-first necessity checkpoint (2026-09-20)** — the human questions why a supposedly supported architecture needs runtime changes. The lead narrows the work to declarations, actual adoption and focused proof first. A new read-only `inspect --request` surface is not yet established as necessary for the existing prepare/approve/scaffold/start path, so that runtime WIP is parked pending a concrete blocker. A separate real released-package gap is confirmed: `oats.aweb`1.10.3 declares no binding interface and the captured broker refuses such a provider. The current provider draft still unconditionally reports unqualified native readiness, so a codec/refusal scaffold is not a usable messaging implementation. The human explicitly confirms that capabilities not ready for the new infrastructure should be fixed. The provider lane is therefore authorised to implement the minimum usable adaptation through existing contracts and legitimate native authority, while isolating any genuinely new authority decision; a refusal-only scaffold is not completion. Metadata delivery and existing-path tests proceed. No runtime/provider WIP has been integrated or qualified by this checkpoint.

- **Phase1 implementation started under explicit human GO (2026-09-20)** — framework-hosted workspace is accepted, with `oats-dev` retained as development capabilities. Three existing implementation lanes receive disjoint work: workspace/source declarations and cross-repo root indexes; kernel/public onboarding/lifecycle; canonical knowledge/messaging provider readiness. The [plan](https://github.com/awebai/oats/blob/main/docs/design/2026-09-20-workspace-and-portable-adoption-plan.md) fixes ownership, and the [source proposal](/decisions/portable-role-editions-and-bootstrap.md) now records the bounded `souls/oats-expert/` transitional edition permission. Developers implement from a clean current base and deliver exact reviewed increments; the lead supervises, integrates and owns actual deployment gates. No new reviewer agents, blanket old-branch merges or repeated full matrices. Assignments are not delivered code or completed workspace adoption; corpus/five-role cutover remains phase2.

- **Workspace first, knowledge and experts second (2026-09-20)** — the human explicitly sets this delivery order. The [execution plan](https://github.com/awebai/oats/blob/main/docs/design/2026-09-20-workspace-and-portable-adoption-plan.md) covers a real Git workspace, per-repository exports/backlinks, a usable retained portable-source path and fresh local adoption before the centralised knowledge/five-role cutover. Full Desktop parity follows. A bounded read-only inventory confirms the workspace/member indexes are not yet published in the inspected maintained repositories, so released machinery is not deployment adoption. The [rebuild decision](/decisions/expert-souls-and-knowledge-rebuild.md) records the sequence; source identities, visibility, exact profiles and any new authority remain explicit choices. No repository conversion or knowledge migration has occurred from writing this plan.

- **Framework-hosted workspace choice accepted (2026-09-20)** — the human explicitly selects `oats` as workspace host and directs implementation while preserving `oats-dev` as development capabilities. The [accepted decision](/decisions/git-workspace-versus-development-package.md) uses separate `oats-workspace.yaml` and `oats.yaml` contracts, with explicit reciprocal admission where the framework also participates as a member. Public source consumers must not inherit that development workspace implicitly. Existing package payloads/tags/restores remain intact; actual declarations, provider readiness and deployment are work to deliver, not implied by accepting the choice.
...
- **Desktop wave remains authorised, sequence revised (2026-09-20)** — canonical documentation is on main and the requested fresh Desktop engineer/historical-note handoff remains part of the plan. The latest human direction puts usable Git-workspace/portable adoption and the centralised five-expert knowledge cutover before full Desktop design/new-functionality parity. This supersedes the September19 Desktop-next ordering, not runtime/provider safety or evidence requirements. Historical notes are context, not copied identity, credentials or unfinished-operation authority; no completed implementation is claimed here.
...
- **Fresh deployment and role cutover remain distinct (2026-09-19)** — the0.24.0/2.1.0 artifacts are published and their installed smoke passes, but workspace re-provisioning, adaptation of the preserved five-role/external-KB/public-helper assets and real private-provider/team acceptance are not proven by that smoke. A read-only deployment handoff confirms the preserved role declarations still use the legacy shape and lack an advertised source export index; preservation is not valid portable publication. The human must select the accepted Git publisher/export identity and reviewed immutable reference before that owner is incarnated. The [parallel source-edition proposal](/decisions/portable-role-editions-and-bootstrap.md) remains proposed, not silently adopted. Private/aweb qualification, accepted KB/PR writer custody and any explicitly limited nonmessaging pilot remain separate choices. Deployment-specific paths/processes stay with the deployment. The September20 sequence now requires workspace/portable adoption and knowledge cutover before full Desktop parity; actual infrastructure adoption remains separately unproven.
- **Portable setup skill documentation drift (2026-09-19)** — released `skills/oats-portable-setup/SKILL.md` still describes standalone context and launch inputs as not public, while the0.24 router and closed preparation validators already accept `--request`, `standaloneContextKey`, `launch` and exact `helperLaunches`. The operator handoff follows those actual contracts. A bounded published-guidance correction is proposed; no new runtime API, scope expansion or source-layout approval follows from discovering the stale paragraph.
- **Remaining feature and rollout boundaries (2026-09-19)** — the real print-mode start/replay/restart/directory-learning slice does not qualify interactive Pi, required nonempty managed plugins/contributions, scheduled or always-on captured harvesting, public captured wake/retire/recovery, private messaging/onboarding or Git-PR knowledge delivery. Requirements may not be dropped to make a profile eligible. [Native authentication](/decisions/harness-native-authentication.md), strict selected Pi curriculum and the [external witness](/decisions/captured-session-storage-identity.md) remain separate contracts. Proposed runtime-bundle grants, contribution/secret delivery and broader retirement/bootstrap remain human-pending; no credential wrapper, ambient fallback, secret persistence or new authority follows from these tests. Runtime Git knowledge publication stays PR-only. The current sequence places workspace/portable adoption and knowledge cutover before full Desktop parity, without implying either is already complete; historical migration remains deferred, and the original first-cut target was missed rather than reset.
...
- **v0.24.8 probe findings (2026-09-22)**: (a) fresh `oats init --knowledge oats.okf` + `create` → soul lacks `okf.json`; okf required spawn hook ENOENTs, spawn rolls back. Fix in oats-okf template or `create`. (b) readiness `trusted` fails for `oats.core` after `oats trust oats.framework --all-capabilities` — verify readiness trust source vs package-level trust.

- **OKF 2.1.3 bug list (deferred with harvest, per human direction)** — (a) `check` per-cause reasons (E_CONFIRM/E_BASE vs E_OWNER narrowing, second operator holds the request shapes); (b) **retire leaves the per-source workspace schedule definition `okf-<source-id>` enabled** (second-operator observation on 0.24.6 classic, 2026-09-21): the retire hook's final capture sets `status.retired` and `auto=false` in the source's status.json but never issues `oats schedule disable|remove okf-<id>`; a later host-scheduler install would pick up definitions for retired sources. Fix belongs in the OKF retire hook (disable — or remove — the definition after a certified final capture; idempotent; never on an incomplete capture). Operator workaround `oats schedule remove okf-<id>` is safe; per-source `schedule.json` remaining is intended.

- **Collaboration-guide factual review (2026-09-19)** — a supplied draft is returned before publication: it treats legacy in-soul OKF as the current default, config/locks as universally machine-private, executable trust as exclusively uncommitted, repository AGENTS.md as automatically composed, and PR delivery/personal instances as eliminating shared-work conflicts. Current contracts permit reviewed configuration, exact locks and template bases in Git-backed scopes; classic lock approval is not a credential and may survive exact restore. Template adoption metadata proves a recorded baseline, not original authorship. Instruction inheritance must be explicit and verified; messaging directory presence is not process liveness or responsiveness. A warning that onboarding is unverified does not make incorrect contract statements safe. Exact revised diff and operator approval remain outstanding; no guide implementation or verified onboarding claim is added.

- **Supported co-location and multi-operator guidance (2026-09-19)** — [capability-owned placement](/decisions/flexible-knowledge-and-instance-expertise.md) is accepted, but an official co-located OKF2.x profile has no assigned delivery or committed ETA; 2.1.0 does not qualify it. Public visibility, sharing a Git repository and retaining writable `soul/knowledge` paths are different requirements. Multi-operator guidance must separate Git-shared definitions/accepted knowledge from each operator's config, trust, native auth, instances and working state. A derived onboarding recipe is not a witnessed run; missing recorded template adoption does not prove how a config was originally authored. A collaboration guide needs review and actual operator validation before being advertised as a verified setup path. No mandatory OATS control plane does not mean selected providers use no services. These are documentation/qualification gaps, not authority to migrate existing deployments or a promised release date.
