...
- **Combined native/provider candidate assembled locally (2026-09-17)** — the coordinator integrated provider `86bff1a` locally and applied all four independently approved native/public conflict resolutions to the `280d0532` + `6b836ba1` merge. Exact before/stage/after identities and patch applicability matched; all resolved files equal the reviewed incoming blobs, preserving accepted fresh-source additions. The knowledge-log union preserves both histories. Standalone provider gate passes **285 tests, zero failures, five skips**, source unchanged. The first combined harness was returned before execution for four custody/coverage gaps; a separately frozen R1 now has static approval with mandatory externally verified runner custody, disjoint output and exact dependency provenance. The actual frozen-tree combined invocation is next, not yet a passing result or main/release delivery. Inert native/seeded continuation evidence will remain distinct from live model, worker, retirement and privacy qualification.
- **Shared helper/input producer P1 closed; integration pending (2026-09-17)** — the assigned reviewer accepts exact `be2460c5` plus correction patch `a29db161…`, resulting tree `0c031114e965b08ac25b29ab456f48eedcf42c91`. The same three-file correction is committed as `59d5bd18`, but its unrelated inherited native/API2 ancestry is not covered by this verdict. Every capability-sourced new-helper block/omission now requires the exact retained owner policy; primary and literal historical reads remain unchanged. The real publication-path regression independently passes 1/1, with syntax and six-schema checks. Original be remains historically returned; its parallel no-findings review never erased the demonstrated blocker. No code integration or release follows automatically.
- **Provider execution-reader / helper-input pairing scoped-approved (2026-09-17)** — reader `f43996d` remains approved. `57aaec7` independently closes the old `0f809af1` schema/validator defect but was itself returned for executing the obsolete producer pin. Repository-only successor `86bff1a` closes that sole pairing-identity P2: its actual fixture executes the accepted corrected producer tree and independently passes 2/2. Historical returns remain immutable. Distributed package bytes are unchanged through the schema/pin successors; paired retained runs are explicitly seeded, not launched workers. This permits coordinator integration/gating, not source-main/release/native activation. The isolated corrected-producer/API1 pair must not be relabelled as execution against a later combined API2/native tree. Existing ec5 source-main and release/private-provider holds remain distinct.
- **Native Herdr/tmux start/restart and public PB1 independently approved (2026-09-17)** — `5d95175d` generalizes the existing transaction/adapters with closed tagged endpoints, allocated receipt identities, exact tuple/index/pending custody and same-ID uncertain-allocation holds; four independent targeted tests and generated schemas pass. API2 availability is not host readiness. Separate `6b836ba1` closes PB1 by refusing direct helper-ID public sessions before request reading/effects; SOURCE plus exact helper selection remains required, while helper scaffold/internal native use is unchanged. One independent new negative passes; earlier dd NS1–NS4 closures stay closed. These are separate exact deltas, not whole ancestry, main delivery or real backend/model qualification. Managed Pi contributions, public wake/retire/recovery and real provider/worker acceptance remain open; retirement proposal `45c4c95f` is documentation, not implemented retirement.
- **Fresh lifecycle/admission and onboarding review (updated 2026-09-17)** — exact `257c4b96` plus the cumulative fix-only v2 patch has independent closure of FA1–FA4/IC2; the prior ten passing tests and two new residual tests are preserved as separate evidence. Stale-row and post-hook reporting findings are closed, not outstanding. The fresh workspace-presence fix, portable resource files, request leaf and pinned public fixture/docs are independently accepted at their named slices through `f9292253` (14 selected tests, no findings), not whole historical-migration ancestry. Actual composition/request-router hookup through `7a9c25d2` has scoped review acceptance; the direct-ingress case is independently accepted and its dedicated regression passed in the merged-tree gate. Fresh directory witnesses `f92ca993` plus `bba25ca4` docs now have independent reviewer acceptance and two independently passing cases. The context-adapted ingress patch is independently accepted; it and original `7a9c25d2` were mechanically applied with exact hashes, and the approved-base merged-tree gate passes 30/30, including scaffold/layout/normal retirement. The approved core admission/curriculum/request subset is delivered through `c1ff882b` after all eight gates; the high-level fresh facade and directory-witness slices have now also completed their separate reviewed eight-file integration and 19-case combined gate. No whole branch or hostile-concurrency guarantee is implied. Retained launch inputs/no-launch fixtures still do not qualify actual runtime or private providers; general migration remains deferred.
- **Standalone OKF direct-main delivery review (2026-09-16)** — coordinator source-main delivery is authorized; runtime Git knowledge publication remains PR-only. Successor `97f2af3` closes the original worker/locator/alias/identity/registry/filter findings; `b92a824` independently closes the residual pre-registration harvest gate. Captured helper launch still correctly refuses until the kernel execution contract is qualified. One full standalone gate at `b92a824` reports 259 pass, one worker-index byte-preservation failure, three expected native opt-in skips. Successor `ce8980ba` closes the ordinary-index byte regression but was returned for split-index metadata writes and special-file blocking. Exact `ec5d767` now independently closes both residuals (ten selected tests, no findings), with patch/base applicability verified. The complete standalone gate at that exact head finished with 149 pass, 121 fail and four skips, with archived source unchanged. The reviewer confirmed all 121 failures share the harness's repository-contained temp-placement cause; two unchanged affected tests pass in a physically non-Git root. The corrected complete run at unchanged `ec5d767` passed **270 tests, zero failures, four skips**, with archived source unchanged and all runtime paths outside Git; failed evidence is preserved. Independent c24 wire review and continuous-range source-checkpoint consolidation are accepted; exact `ec5d767` is now delivered on provider main by a normal fast-forward push. Release/activation and fuller generic execution/native/provider claims remain explicitly withheld. Version/floor, actual helper launch and live qualification remain separate release gates.
