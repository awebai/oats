...
- **Standalone OKF direct-main delivery review (2026-09-16)** — coordinator source-main delivery is authorized; runtime Git knowledge publication remains PR-only. Successor `97f2af3` closes the original worker/locator/alias/identity/registry/filter findings; `b92a824` independently closes the residual pre-registration harvest gate. Captured helper launch still correctly refuses until the kernel execution contract is qualified. One full standalone gate at `b92a824` reports 259 pass, one worker-index byte-preservation failure, three expected native opt-in skips; a narrow provider-owned fix is in progress without weakening that assertion. Source-main remains held on the failed integration gate; version/floor, actual helper launch and live qualification remain separate release gates.
