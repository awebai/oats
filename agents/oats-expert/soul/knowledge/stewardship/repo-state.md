...
- **Managed runtime and remaining lifecycle qualification (2026-09-18)** — code integration, conflict review, shared helper-policy correction, provider pairing and the actual combined gate are complete, not new review queues. Managed Pi roots/contributions, actual retained primary/helper model execution and public wake/retire/recovery remain separate unfinished work. Bounded public SDK evidence at Pi 0.85.1 did not establish nonambient nonempty extension loading; it is not categorical Pi incapability. The owner's distinct zero-plugin consumer experiment used actual public SDK session/runtime construction, reload, new-session creation and one prechecked owned switch with unchanged selected curriculum and no model/account/network operations. This supports a minimal SDK-host integration path, not real model/TUI/native readiness or unrestricted history access. Exact retained eligibility, explicit host/model/credential binding and before-read history custody are the next bounded implementation/review work; selected requirements may not be dropped. Proposed exact runtime-bundle approval in the existing ledger, sequential launch-contribution admissions and staged retirement remain human-pending contracts, not implemented permission or qualification. No private SDK import, ambient discovery fallback, replayed completed hooks or persisted secret delivery is authorized.
