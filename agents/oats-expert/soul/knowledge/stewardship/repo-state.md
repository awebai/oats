...
- **Claude Code/Codex native-launch policy clarified (2026-09-18)** — the human explicitly requires the complete resolved OATS instance home, skills, capabilities and lifecycle setup, but permits normal outside native context/skills for those two harnesses. No OATS-only visibility adapter or Pi dependency is required. Permission bypass remains explicit user opt-in, never an unattended-launch default. The [accepted decision](/decisions/claude-codex-native-launch.md) narrows the old universal strict-curriculum target without weakening composition/source/history custody. Exact three-file regression/docs change `09f4a5f0` is locally integrated as `39587ea1`; six parent checks pass on the assembled core, without a runtime patch. Full resolved home composition remains required; renderer/planner checks are not native loading, permission-UI or complete captured-worker qualification. The superseded alternative handoff was not integrated.

- **Reviewed local host/worker candidate, not a release (2026-09-18)** — framework runtime candidate `26e81054` includes Herdr20/22 adaptation, the explicit native-auth Pi SDK host, verified helper launch summary, packaging/Desktop compatibility, PH1 retry correction and the reviewed five-library record guard chain. Record candidate `8fcf5e85` was RETURNED for a demonstrated open-time ancestor-redirect read before descriptor/name binding; exact `e0c3232c` closes RV1. Both scoped deltas are integrated, with36/36 parent record checks and a separate1/1 scaffold/layout/normal-retirement probe passing. Original failed probes remain preserved; no record-bin/parser/store redesign or unrelated ancestry was imported. Separate focused checks pass: adapter10, combined inert20, packaging/Desktop25, four-entry inventory3 and PH1/host9; actual npm inventory is not installed-runtime proof. Standalone OKF candidate `3701f74` follows reviewed `85b9ec0` (16 focused checks, including actual admitted public helper scaffold/staging/repeated-prepare without native launch) with one bounded selector-hygiene correction and its separate passing unit. Worker child calls now also discard legacy OATS instance aliases while preserving native auth/profile and Git/SSH helper context. Its worker opens only through an admitted persistent-instance operation, not raw/null-intent/scheduled or always-on lifecycle harvesting. Gate payload/selector/failure-status findings are closed, but the real gate remains partial until actual process success and SDK final outcome are observed. Last verified source mains remain framework `1de7c31c` and provider `86bff1a`; no new source delivery, model/backend success, package version/tag/install or release is implied. Earlier complete-suite evidence belongs to its recorded pins, not this newer candidate.
- **Integrated-review completion wave and record exception (2026-09-18)** — three implementation lanes continue, with the maintainer owning combined product/correctness/security review; both reviewer instances retired normally with evidence preserved and aliases unavailable for reuse. The human explicitly approved the [five-library capture-enforcement exception](/decisions/captured-session-storage-identity.md). The record guard source is now reviewed and locally integrated. Lifecycle still owns the kernel consumer update needed to permit approved v1-to-v2 expected-inventory mutation during authority callbacks, the obsolete absent-support test premise, and truthful native/SDK outcome observation; its coupled test must run with actual record-v2 rather than skip. Provider will consume that exact observer in the real gate. Recorder-bin/parsing/store redesign and broader pending contracts remain excluded. Preserve the single complete-pipeline version2 advertisement and pinned synchronous APIs; do not activate writer-only support. No safety contract, unqualified feature or live permission is waived by delivery pressure.
- **Managed runtime and remaining lifecycle qualification (2026-09-18)** — code integration, conflict review, shared helper-policy correction, provider pairing and the actual combined gate are complete, not new review queues. Managed Pi roots/contributions, actual retained primary/helper model execution and public wake/retire/recovery remain separate unfinished work. Bounded public SDK evidence at Pi 0.85.1 did not establish nonambient nonempty extension loading; it is not categorical Pi incapability. The owner's distinct zero-plugin consumer experiment used actual public SDK session/runtime construction, reload, new-session creation and one prechecked owned switch with unchanged selected curriculum and no model/account/network operations. This supports a minimal SDK-host integration path, not real model/TUI/native readiness or unrestricted history access. Exact retained eligibility, explicit host/model selection and before-read history custody are the next bounded implementation/review work; selected requirements may not be dropped. The human has now [settled harness authentication](/decisions/harness-native-authentication.md): launch the already authenticated harness using normal native auth, including native OAuth behavior where applicable. The proposed OATS credential view/auth-file argument/API-key-only restriction is superseded, not a prerequisite or approved implementation. Resource isolation must not replace the user's native auth context. The initial host contract was returned because path-only native location receipts cannot prove persistent session-root identity. The human has now [approved the narrow external witness](/decisions/captured-session-storage-identity.md) in existing kernel custody; the five-library source/codec now has scoped review closure, while kernel consumer/outcome integration and actual native qualification remain pending. The old credential-view/provider-allowlist finding applies to the superseded auth design, not current native-harness policy. No unchanged-contract or live-runtime approval is implied. Proposed exact runtime-bundle approval in the existing ledger, sequential launch-contribution admissions and staged retirement remain human-pending contracts, not implemented permission or qualification. No private SDK import, ambient discovery fallback, replayed completed hooks or persisted secret delivery is authorized.
- **Coordinated release and real acceptance (2026-09-17)** — source-main delivery does not update installed deployments. The planned breaking 0.24.0 floor/version alignment, provider mirror/catalog publication, fresh re-provisioning, actual workers/learning and real private-provider acceptance remain open. Both tmux and Herdr must pass applicable primary/helper start/restart/wake/retire/recovery qualification; inert dispatch, hook ABI and seeded continuation do not prove those live behaviors. Existing metadata is not new compatibility evidence. Runtime Git knowledge publication remains PR-only; general historical migration stays deferred.
