...
- **Fresh lifecycle/admission and onboarding review (updated 2026-09-17)** — exact `257c4b96` plus the delivered fix-only patch passes ten selected tests; retry references, outcome rendering and current view receipts are scoped closed. Two residual static P2s are returned: index-only failure reporting can overwrite a newer lifecycle status, and a post-hook classification read can bypass observed-fact reporting. The fresh workspace-presence fix, portable resource files, request leaf and pinned public fixture/docs are independently accepted at their named slices through `f9292253` (14 selected tests, no findings), not whole historical-migration ancestry. Actual composition/request-router hookup through `7a9c25d2` is delivered source undergoing separate review. Retained launch inputs/no-launch fixtures still do not qualify actual runtime or private providers; general migration remains deferred.
- **Standalone OKF direct-main delivery review (2026-09-16)** — coordinator source-main delivery is authorized; runtime Git knowledge publication remains PR-only. Successor `97f2af3` closes the original worker/locator/alias/identity/registry/filter findings; `b92a824` independently closes the residual pre-registration harvest gate. Captured helper launch still correctly refuses until the kernel execution contract is qualified. One full standalone gate at `b92a824` reports 259 pass, one worker-index byte-preservation failure, three expected native opt-in skips. Successor `ce8980ba` independently closes that ordinary-index byte regression (one selected test passes), but is returned for two new copy-boundary gaps: native split-index reads freshen original backing-file timestamps, and synchronous copying can block on a special source file. Both are static source findings, not new executed reproductions; the provider owns a narrow correction without assertion weakening or metadata restoration. No repeated full-gate pass or source-main delivery is claimed; version/floor, actual helper launch and live qualification remain separate release gates.
