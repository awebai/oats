...
- **Workspace home recommendation revised (2026-09-20)** — after considering reuse of `oats-dev`, the human asks about hosting the workspace in `oats` while preserving `oats-dev` as development capabilities. The [proposal](/decisions/git-workspace-versus-development-package.md) and execution plan now prefer this arrangement: separate `oats-workspace.yaml` and `oats.yaml` contracts can coexist in the framework repository, with explicit reciprocal admission where it is also a member. Framework/source consumers must not inherit the publisher's development workspace implicitly. A separate workspace repository is optional if independent permissions/lifecycle are needed. The package, immutable tags and old restores remain intact; no declaration, catalog, deployment or migration change has been performed by revising the recommendation.
