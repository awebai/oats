...
- **Fresh lifecycle/admission and onboarding review (2026-09-16)** — candidate `257c4b96` is returned for replaced-home publication after custody failure, retry-reference loss, unsettled optional-hook success reporting, outcome/ledger mismatch, stale receipt mirrors and remaining active legacy curriculum. Onboarding `4d244ee2` is returned for inconsistent supplied-workspace presence handling. Scoped tests passed; these are real implementation holds, not human approval waits. Implementation owners are correcting them with explicit resource-file ownership split. Retained launch inputs and public no-launch fixtures are later source work, not actual runtime/private-provider qualification; automatic historical migration remains deferred.
