...
- **Release pair integrated locally, not published (2026-09-19)** — approved provider metadata `f20f8e5` selects2.1.0 and kernel floor>=0.24.0 without production-code/protocol changes; parent manifest validation and exact metadata case1/1 pass. Exact framework mirror/catalog/assertion/notes commit `b01f6a3b` is integrated as `c9ba5798`. Its mirror subtree is identical to approved standalone Git objects; all38 inventory entries, wrappers and alias were independently checked, all25 preimages matched, and four parent current-metadata/actual-npm-byte checks pass. Inventory remains pending until actual provider-tag verification. A controlled release export stamps0.24.0 into all three package manifests/locks without changing tracked kernel versions; its version probe, shipped-JS syntax and real cross-checkout installed kernel/adapter smoke with OKF2.1 pass. The initial bare-export missing-dependency failure remains separately recorded; declared production dependencies were installed before the successful remaining checks. Final assembled scaffold/layout/copied-resource/normal-retirement1/1 also passes. Last verified source mains are still framework `1de7c31c` and provider `86bff1a`; final publication, installed published-artifact verification and deployment are not yet complete. Earlier full-suite results remain tied to their original pins.
