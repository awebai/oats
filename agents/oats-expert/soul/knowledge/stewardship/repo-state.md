...
- **Kernel release preflight correction closed (2026-09-19)** — source CI at `0999f4a8` reports41 failures:39 are explicit provider-floor refusals under the still-unstamped0.23.2 source version; the actual release lane stamps0.24.0 before testing. The other two stale Desktop server integration assertions are corrected in exact test-only `c97ddf00`, reviewed and integrated as `7352c16d`. Parent affected cases2/2 pass through real HTTP/fake CLI, preserving0.23/0.24 acceptance and0.25 rejection. No runtime, floor, version or gate weakening; no remaining stale-band blocker. The full tag lane remains required before publication.
- **Kernel release publication pending (2026-09-19)** — reviewed metadata/mirror pairing is on source main and OKF2.1.0 is published as recorded above. Parent metadata1/1 and current-metadata/actual npm-byte4/4 checks pass; all38 inventory entries/wrappers match approved immutable source. Actual provider-tag finalization changes only provenance/wrapper-mode metadata, not the verified mirror payload. A controlled0.24.0 release export, without tracked kernel version bumps, passes version/shipped-JS checks and real outside-checkout kernel/adapter installation with OKF2.1. Final assembled scaffold/layout/copied-resource/normal-retirement1/1 passes. The initial bare-export missing-dependency failure and older bundled2.0 smoke remain separate evidence. Tag-driven kernel/Pi/Desktop publication, installed published-artifact verification and deployment are next; no prior full-suite result is relabelled as the newer release.
